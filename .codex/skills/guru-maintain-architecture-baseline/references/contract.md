...
identifies the profile, stage, task, current baseline, constitution, project change
contract, requirement/behavior authority, and freshness. It never carries an
impact decision, change path, contribution decision, project-check result,
review conclusion, semantic pass, or typed route.
...
exactly one change path, the task-owned contribution, current project-check
results, and the stage-specific review state. The deterministic runtime only
validates this separation, identity/freshness binding, and route consistency.
...
Projects declare their own check descriptor, command, and semantics. The
descriptor binds check identity/version, entrypoint, applicable scope,
rule/decision/GAP refs, result-contract identity, and freshness source. The generic result
contains check identity/version, applicability, rule/decision/GAP refs,
...
the task's real dependency. The runtime validates shape, binding, and route
consistency only. Every descriptor/result binds at least one current rule,
decision, or GAP identity. A blocking failed or unverified check cannot support
`baseline_current`; a non-blocking evidence gap remains explicit and cannot be
used to close a GAP, approve an exception, or claim architecture completion.
