...
def _get_task_status(trellis_dir: Path, input_data: dict) -> str:
    ...
    if not active.task_path:
        return (
            ...
            "In Guru Team projects, issue-backed, task-like, or file-changing requests mandatory invoke "
    ...
    if task_status == "planning":
        ...
        if missing_complex:
            next_bits.append(
                f"complete {', '.join(missing_complex)}, then display links to "
                "prd.md, design.md, and implement.md before start"
            )
        else:
            next_bits.append(
