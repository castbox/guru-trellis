...
`minimal_handoff`, the current package closure is 13-by-52, the deferred global
workflow closure is 12 invokes, 46 exits, and 27 targets, and the Stage 0
...
readiness; publication `ready` points to active `guru-finalize-task`, while the
global invocation/order remains fail closed until #119 activates the Finish
family. The production migration identity and three-Skill/11-exit membership
...
minimal exits. They state that `ready` targets active `guru-finalize-task`,
whose global Finish-family invocation/order is deferred to #119;
...
global workflow markers remain 12 invokes, 46 exits, and 27 targets until #119.
The separate
...
They separately identify the deferred global workflow projection as 12 invokes,
46 exits, and 27 targets. #119 owns Finish-family invocation/order, combined
acceptance, and #115 closure; #132 owns physical upstream overlay cleanup.
Finalizer documentation must not present package activation as either of those
