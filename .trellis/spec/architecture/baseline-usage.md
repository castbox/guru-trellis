...
- status：`active`
- source binding：reviewed #247 contribution `architecture-contribution-247-remove-issue-scope-ledger-v1` + accepted `ADR-009` + inherited immutable `.49` authority；current graph 23 Skills / 97 exits / 78 commands；framework source `castbox/Trellis@a2003296b4c4ce46c50d72ead3b2ec9c317f69fc` / CLI `0.6.17` / `pnpm@10.32.1` / extension `0.6.16-guru.41`；released `v0.6.16-guru.1` 不包含 #247 candidate；#305 target authority 不变（精确 revision 由包含本 authority 的 Git commit/tree identity 绑定；promotion-created Gate 与远端动作状态不进入本 projection）
