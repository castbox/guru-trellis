...
This is the execution and validation plan for the current #305 Phase 1 Requirements continuation. It is not an
implementation plan for changing the Guru runtime. The live task status remains `in_progress` from a historical
activation, but `REQ-REV-133..142` and the latest user decision keep the current continuation in Phase 1 and stop it
after fresh Design gate closure. #311/#312 are merged into the selected base; PR #317 platform-selection preservation is
rebound as material current capability evidence; Requirements/Design/Test/Architecture/task projections cover the
complete 84/34/23/13/50/73 set. Fresh Requirements semantic, Strict technical, fresh Design review and deterministic
closure gates have passed; `evolution_refactor_eligible` is current. Runtime/workflow/Skill/schema/script/preset/overlay changes, stock
mutation, installation, migration, provider action and Release remain outside this continuation.
...
- current as-built Test remains `docs/test/versions/current-main-0.6.5-guru.42/` at selected-base
  `5650df47…` and supplies current capability context plus PR #317 focused platform-matrix and PR #318
  caller-inventory consistency evidence, not target execution PASS; prior selected bases `736ef333…`, `9f560ec1…`, `a41b8a34…`, `.41`/`3efcce72…` and
  `.40`/`d907fcc5…` are historical comparison evidence.
...
2. Freshly read selected-base `.42` RDT/Architecture/inventory and PR #317/#318 evidence, verify `.41...42`,
   `a41b8a34...9f560ec1` and `736ef333...5650df47` are fact-only, classify `9f560ec1...736ef333` as a material platform-selection advance,
   reclassify all merged results as current-base capabilities, and compare them against target requirements and
   normal-path fixtures.
3. Complete `REQ-REV-133..142` across Requirements main/NFR/inventory and the 52 UC / 84 REQ / 34 NFR / 23 current
   capability / 13 target delta / 50 fixture closure. #311 is folded into `CUR-CAP-013/014/017/018/019`, #312 into
   `CUR-CAP-012`, and PR #317 into `CUR-CAP-013/014/017`; no fifth core capability, duplicate
   `TARGET-DELTA-014`, new fixture or new Design responsibility remains.
...
7. Revise Design/Test/Architecture/task projections for the full current capability set. Allocate `EVO-DES-070..073`
   ...
   failure lifecycle; map `EVO-REQ-084` / `EVO-NFR-034` into existing Publish/Finish/Projection responsibilities and
   the installed-publication fixture; preserve all 50 fixture mappings without changing Requirements semantics.
...
9. Run deterministic set/link/table/YAML/JSON/consumer/recovery closure for 84/34/23/13/50/73, 39 public Skills,
...
Current sequence result: steps 1-10 are complete for the planning candidate; fresh Design review and deterministic
closure returned zero P1/P2/P3 findings and zero high-risk open questions. This establishes planning eligibility only.
The pre-`REQ-REV-142` `DES-REV-001..052` result and task activation are historical evidence only and authorize no
Phase 2/runtime action.
...
| closed ID sets | Requirements 84, NFR 34, candidate Design 73, current capabilities 23, target deltas 13, fixtures 50 | presence alone does not prove semantic adequacy or current Design review |
...
| Architecture impact | task contribution is `fresh_design_review_passed` / `not_promoted`; selected-base current Architecture is input, while shared current and ADR index remain unchanged | no accepted ADR/shared-current promotion |
| semantic Requirements review | fresh full-document semantic and Strict technical review passed for the final 84/34/13/50 candidate with P1/P2/P3 findings and high-risk open questions all zero | `requirements_ready_for_design` / `requirements_trace_ready_for_design` current; no Design execution in this continuation |
| semantic Design review | pre-`REQ-REV-142` review is stale historical evidence; fresh full-document review and deterministic closure passed for the complete 73-responsibility/50-fixture candidate with P1/P2/P3 findings and high-risk open questions all zero | `design_ready_for_delivery_planning` / `fresh_design_review_passed` / `evolution_refactor_eligible`; no runtime/Test execution claim |
...
- Repository Requirements remains the only upstream authority in `docs/requirements/evolution/`; its current
  `requirements_ready_for_design` identity is bound to `REQ-REV-011..142`. The selected post-#311/#312 base is
  bound to `5650df47…` / `.42`; merge/lifecycle facts are current, `.42`, `a41b8a34...9f560ec1` and
  `736ef333...5650df47` additions remain fact-only, while `9f560ec1...736ef333` is a material platform-selection advance. Latest stable current is
  `.2/.38/CLI 0.6.15`; `.3/.39` remains an unverified successor. The #305 authority rebind, merged-behavior
  reconciliation and Requirements-stage zero-loss projection are synchronized; fresh Requirements dual review and
  deterministic closure are current. Every prior Design review binding remains stale historical evidence, while the
  revised exact candidate is `design_ready_for_delivery_planning` / `fresh_design_review_passed` /
...
slices. Requirements convergence, fresh Design review and Design deterministic closure are current.
Those suggestions are non-actionable until a future independently authorized delivery intake. Each later
...
The eventual #305 planning delivery route, commit scope and PR action are selected only after a future fresh Design
gate and a later separately authorized workflow entry. The merge-bound fresh Requirements gate is current in this
continuation. Each external side effect still requires its own current exact-action plan. No commit/push/PR is part of
