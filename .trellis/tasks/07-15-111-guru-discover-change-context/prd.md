...
- Skill 必须读取 open GitHub issue candidates 并记录候选身份、live facts digest、匹配理由与 AI 观察。
...
- Malformed JSON、缺失 index、index shape 错误、symlink/path boundary 错误必须进入按 repo-relative path 排序的 invalid evidence，不得泄露原始内容或本机绝对路径。
...
- Task 创建后，持久化调用必须把同一 canonical snapshot 写入 `{TASK_DIR}/context-discovery.json`，并校验 expected snapshot digest、query digest、archive manifest digest、base HEAD 与 live issue/draft identity；此阶段只接纳新 task 目录内的 tracked dirty paths 和 gitignored runtime mapping。
...
- Artifact、example、manifest 与公共文档不得携带 secret、private data、workspace journal、本机绝对路径或业务私有运行态。
...
- 权威 `Docs SSOT Plan` 位于 `design.md` 第 11 节，执行 checkpoint 位于 `implement.md` 第 2 节。
...
- [ ] AC4：Duplicate evidence 只记录 candidates/reasons，不作 reuse/new target 终局决定。
...
无。Live issue、官方 Trellis 文档、当前 durable docs/code/tests 与三个归档候选已经提供进入设计和实现规划所需证据。
