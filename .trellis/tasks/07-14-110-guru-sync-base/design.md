...
tool-free route
  ...
  -> evidence finalization: workflow lease transfer | standalone cleanup
  -> synced | skipped | blocked
```

Global workflow 只拥有 mandatory invocation、route transition 与 typed-exit consumer。Skill package 独占 selected-base resolution、sync procedure、AI review dimensions、evidence lifecycle、workflow resolution lease 与 standalone behavior。
...
- Result evidence 完成 AI Gate 与 validator 消费后必须在所有 terminal path 删除；执行期间出现 `blocked` 时同样必须清理已经创建的 result evidence。`blocked`、`skipped` 与 standalone `synced` 还必须删除 resolution evidence，然后才能返回 typed exit。
- Workflow `synced` 不删除 resolution evidence，而是把 exact external path、`resolution_sha256` 与 lease state 作为非持久化 handoff 转移给唯一 workflow consumer；该动作是 evidence ownership transfer，不是未定义残留。
- 同一 workflow lease 必须传给后续每次 `prepare-task --resolution-file ... --expected-resolution-sha256 ...`。Planner、issue creation、worktree creation 与 task creation 不得重新解析出第二份 provenance，也不得提前释放 lease。
- `sync-base --release-resolution-evidence --resolution-file <path> --expected-resolution-sha256 <digest>` 是确定性 release executor。它必须先校验 lexical/resolved path 位于 repository 外、component/symlink boundary 安全、现存文件 raw bytes digest 匹配，再删除并确认路径不存在；路径已由外部临时目录清理时只能返回结构化 `already_released`，不得删除其它对象。
- Workflow 在 Phase 0 成功创建 task 后释放 lease；当 route 阻塞、用户放弃、session 明确终止或新 `guru-sync-base` invocation 取代旧 resolution 时，也必须先释放旧 lease。需要用户确认后继续同一 Phase 0 route 时 lease 保持 active。
- Resolution lease path/digest 不得写入 tracked task artifact、`.trellis/.runtime/**`、shared cache、public README/example 或 review evidence。缺失或已释放 lease 不能继续 `prepare-task`，必须从新的 `guru-sync-base` invocation 重新开始。
...
| Exit | Consumer | 证据 |
| --- | --- | --- |
| `synced` | workflow route `guru-discover-change-context` | AI-reviewed、validator-passed fresh result；workflow mode 还携带 active resolution lease handoff |
...
Workflow `synced` consumer 必须持有 resolution lease 并把同一 file/digest 传给 Phase 0 的每次 `prepare-task`。Phase 0 terminal route 必须调用 `sync-base --release-resolution-evidence`；处于用户确认挂起状态时不是 terminal route，不得提前释放。`synced` 之后的 prepare/intake block、用户放弃与 superseded invocation 同样属于 terminal cleanup；Skill 自身的 `blocked` exit 在返回前完成内部 cleanup，不产生 lease obligation。Global workflow 只消费 typed exit 与 release obligation，不重新判断 selected base 或 evidence 充分性。
...
6. 每次 prepare 调用只读同一 active resolution lease；最终 task mutation 成功后由 workflow 调用 deterministic release executor。任何 prepare blocked/abort route 在停止前释放 lease，retryable human-confirmation route 保留 lease。
...
- Resolution release 拒绝 repo-internal、symlink、digest mismatch 与 unrelated path；active content 只能成功删除一次，重复 release 返回 `already_released`，missing lease 不能继续 prepare。
...
- Clean throwaway marketplace init/preview/switch、preset apply、standalone sync、真实 workflow `synced -> prepare-task` planner/mutation guard/release、`trellis update`、workflow/preset reapply、零 evidence 残留与零 sidecar。
...
- Task delta merge：四级 resolution、三方 equality、workflow resolution lease、terminal cleanup ownership、typed exits、prepare reuse、distribution 与验证矩阵必须进入 durable contracts。
...
- `sync-base` 增加 additive `--release-resolution-evidence` mode，不改变既有 command id、schema id 或 typed exit id；workflow mode 的 `synced` handoff 新增 lease obligation，standalone 行为保持返回前清理。
...
| Resolution lease 与 terminal cleanup | 第 5.2、7、8 节 | 第 3、4、6 步 | AC8、AC11 |
