...
- `build_closeout_plan()` / `closeout_plan_errors()` / `validate_closeout_plan()` / committed plan readers。
- `prepare_closeout()`，供 dry-run和formal复用。
- `record_marketplace_machine_evidence()`，分离semantic reason与machine identity。
- 状态识别只保留由 `cmd_finish_work()`、active recovery 与 archived recovery 实际调用的 stage-specific resolver；不得保留只有测试调用的聚合 state resolver。
- content push、marketplace evidence、draft PR、final projection、archive transaction 与 ready transition 由唯一 closeout primitives 承担。
- `build_final_archive_projection()`、`execute_archive_metadata_transaction()`、`ensure_closeout_pr_ready()`。
- `cmd_finish_work()`只做阶段编排和结构化错误包装。
- `cmd_publish_pr()`只保留兼容性 fail-closed handler；旧 normal/recovery publish pipeline、summary URL tail、独立 marketplace validator/recorder 与 recovery command builder 全部删除。

Bash wrapper继续只转发参数，不承载状态判断。`publish-pr.sh` 转发后只能得到指向 `trellis-finish-work` 的兼容性拒绝，不得进入第二套 publish executor。
...
### 9.1 Round 18 scope cap

- 跨月保护只负责在 official archive move 前比较 immutable plan destination 与官方当前 destination。发生不一致时，task 保持 active，并由同一 finish entry 重新 prepare；不得移动 task，不实现跨月迁移或通用时间抽象。
- `after_archive` hook 采用 unsupported-by-contract 门禁。prepare 阶段只做确定性配置解析与“是否为空”判断；非空、不可解析或无法证明为空时 fail closed。companion 不执行 hook、不分析 hook 语义、不把 hook mutation 纳入 allowlist。
- tracked artifact bytes、Git mode、staged/dirty/untracked allowlist 必须在 official move 前与 evidence commit 逐项绑定；post-move continuity 只用于证明 move 结果，不得首次发现 pre-move 可检测错误。
- exact committed archive recovery 的 plan authority 固定为 current commit blob，与 archived working tree 是否保留 task context 无关。
...
- 当前 `cmd_finish_work()` 已直接编排新 closeout transitions，不再调用旧 `cmd_publish_pr()`。
- 旧 publish handler 及其 transitive-exclusive helper 在仓库生产入口中不可达，却仍保留完整 publish/recovery/summary-tail 行为。
- 旧测试通过手工构造 `from_finish_work` / `recovery_after_finish_work` 字段绕过真实 parser，形成测试自证。
- `resolve_closeout_state()` 无生产调用者，只有单元测试调用。

### 13.2 收敛方案

1. 保留 `publish-pr` command name 与 shell wrapper，Python handler 只调用 invocation guard 并返回固定 fail-closed 错误。
2. 删除旧 handler 专属的 PR body generator、marketplace evidence recorder/validator、recovery command、open PR recovery、summary rewrite 与 metadata tail commit helpers；被其他生产 command 使用的共享 helper不得误删。
3. 删除无生产调用的 aggregate state resolver及对应测试。
4. 删除直接调用 dormant handler 的测试；新增/保留一个通过真实 parser/main入口验证兼容性拒绝的测试。
5. 保留 `cmd_finish_work()` production failure matrix、mixed-case/identity/path/recovery regression和installed initial/update smoke。
6. 使用 AST call graph 与 `rg` 双重证明：删除后不存在旧 helper引用、测试字段或第二套 publish/recovery路径。

### 13.3 文档合同

Durable workflow、README 与 companion specs 中“`publish-pr.sh` 保持低层 executor”的表述必须改为“保留的兼容性阻断路径”。用户入口、恢复入口与真实 executor均固定为 `trellis-finish-work`。
