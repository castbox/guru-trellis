...
1. 为runtime locator、完整identity、common-dir pointer、用户cache与迁移增加测试。
...
7. 在隔离临时repo中执行main checkout apply -> linked worktree wrapper lifecycle，重放 `EXT-222-001`。
8. 执行 `guru-check-task`、创建reviewed commit、Branch Review、publication、PR与merge。
9. 合并后更新#222 prerequisite并从fresh `origin/main`重新冻结candidate；旧发布证据全部作废。
