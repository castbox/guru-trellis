...
3. **Fail closed**：直接 `finish-work.sh` 默认失败，除非调用方能证明来自显式 `trellis-finish-work`；直接 `publish-pr` 默认失败，除非调用方能证明来自 `finish-work` 或显式 recovery/debug override。
...
`cmd_finish_work()` 当前在 archive、journal、metadata commit 后构造 namespace 调用 `cmd_publish_pr()`。原始问题是 `publish-pr` CLI 子命令和 `finish-work` 内部调用之间没有可验证区别，因此 AI 可以在 `trellis-continue` 阶段误调用 `publish-pr` 并完成 push/PR。

用户补充证据显示，`trellis-continue` 还可能直接链式调用 `finish-work`。因此新的设计必须同时区分：

- 显式 `trellis-finish-work` 用户/会话入口；
- `finish-work.sh` companion helper；
- `finish-work` 内部调用 `publish-pr`；
- recovery/debug 直接 publish。
...
trellis-continue
  ...
  -> stop and wait for explicit trellis-finish-work

trellis-finish-work
  -> call finish-work.sh --from-trellis-finish-work
  -> check review gate
...
```text
finish-work.sh
  -> if not from explicit trellis-finish-work:
       fail before gate/archive/journal/push/PR checks
```
...
- `from_finish_work`：Python 内部 marker，不作为 CLI flag 暴露；由 `cmd_finish_work()` 传入。
- `--recovery-after-finish-work`：显式恢复/debug flag；允许 CLI 直接调用。

`finish-work`：

- `--from-trellis-finish-work`：显式 finish entrypoint intent marker；只由 `trellis-finish-work` skill/prompt/command overlay 传入，不得加入 `trellis-continue`。

### 新增校验函数

新增 `validate_finish_work_invocation(args)`：

- 如果 `args.from_trellis_finish_work` 为真，则通过。
- 否则抛出 `WorkflowError(exit_code=2)`，错误说明：
  - `finish-work` 必须来自显式 `trellis-finish-work`；
  - `trellis-continue` 不能链式执行 closeout；
  - 需要 `--from-trellis-finish-work` intent marker。
- 校验放在 `cmd_finish_work()` 最前面，确保不会 archive、写 journal、push 或创建 PR。
...
- 不调用 `publish-pr`；
- 不调用 `finish-work`；
- Branch Review Gate 后停止，等待显式 `trellis-finish-work`。
...
- 调用 `cmd_finish_work()` 不带 `--from-trellis-finish-work`，断言 `WorkflowError` 且不会进入 archive/journal；
...
- 风险：已有脚本或 AI 直接运行 `finish-work.sh --json` 会失败。
  - 缓解：所有 `trellis-finish-work` overlay 改为传 `--from-trellis-finish-work`，错误 payload 明确 required entrypoint 和 intent flag。
