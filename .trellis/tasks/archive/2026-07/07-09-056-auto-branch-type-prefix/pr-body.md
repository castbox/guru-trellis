...
- 尚未对当前分支的公开 `gh:castbox/guru-trellis/...#ref` 跑完整 throwaway install，因为 Branch Review Gate 发生时分支尚未 push，不能把已发布 stable tag 或本地 checkout 验证冒充为当前 ref 验证。
- 分支 push 后或 release tag 前，应使用指向当前 ref 的 `TRELLIS_WORKFLOW_SOURCE` 补跑 `verify-throwaway-install.sh`，作为 release readiness 证据。
...
- 当前 HEAD：`9fae97fba1967bbe80c44a97b6cc80354a5baa67`，相对 reviewed head 仅新增 review/gate metadata。
...
- 当前 PR 限制：当前 ref 的完整 throwaway install 尚未验证；该限制已保留在 Review Gate observation 和本 PR body 中。
