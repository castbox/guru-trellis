...
profile input。三个 production corpora 的真实 native execution 同时要求 shared native
adapter 只为本任务三个 production Skill ids 完成 owner staging；该扩展不得改变 #147
schema、runner、grader policy、adapter protocol 或 expected-exit 边界。
...
- [production eval adapter owner-staging proposal](https://github.com/castbox/guru-trellis/issues/146#issuecomment-5053205362)
  绑定 proposal `9cb7d74836af661328de1dc5f0e6740840a7a8fbd02e0028a6813a6bde73ebc5`
  与 action `82052b7d1f9d04cf49a6ec0b4a3980b71cb6d83e96aef8f796b30acee1fae4ae`；
  用户已精确确认以 `accepted_current` 纳入 #146，并授权发布该 exact issue comment、按
  GitHub authority -> fresh context -> planning/ledger update -> fresh planning approval 顺序
  更新当前 task。
...
- `origin/main` 的 shared native adapter 只实现 `guru-sync-base` 与五个 Stage 0 owner
  builders；三个 production corpora 虽然 runner exit `0`，但 case 均返回
  `execution_error` 与 `owner staging is not implemented for <skill-id>`，不能满足 R6/AC7。
- 当前 task 已按用户授权的一次性 R16 recovery 修正 post-commit task-local context re-entry：
  selected-base freshness 与 task HEAD/worktree freshness 分层校验，并已用 snapshot
  `3f40c78c012252820080e0e15f87c7e0da3a1f6629f2b795356da67d2eecd0f3`
  正式替换 prior `41680b6c441af235e9db2cc4c17958a55800fb6ac6955d9263027802b348ccea`。
...
- Base freshness 与 task worktree freshness 必须独立校验：`pre_task` 继续要求 current HEAD
  必须匹配 base-sync decision HEAD；task mode 继续验证 selected base local/remote refs、remote repo
  与 `task.json.branch`，但 task HEAD 和 dirty state 只由 R15 `task_worktree_state` 精确绑定，
  不得把正常 post-commit task HEAD 错判为 `base_head_stale`。
...
- Shared native adapter 的 owner staging 扩展范围精确为
  `guru-approve-task-plan`、`guru-check-task`、`guru-create-task-commit` 三个 stable Skill ids；
  其它 owner dispatch 与 unsupported 行为保持不变。
- 每条 production path 必须在隔离 fixture 中复用现有 recorder/checker/builder 构造
  checker-passed owner state，再真实调用对应 public wrapper；actual exit 只能来自 wrapper
  checked result。
- Adapter request 不得新增或传递 `expected_exit` 作为 route input；runner 继续只在 wrapper
  返回 actual exit 后选择 output schema，并由 grader 事后比较 `expected_exit`。
- Canonical `trellis/skills/guru-team/adapters/eval/native_adapter.py` 与 preset-installed mirror
  必须 byte-identical；不得修改 #147 eval schema、runner、grader policy、adapter protocol、
  runner evidence contract、semantic owner 或 human-review boundary。
- 三个 production corpora shared-native execution、#145 Stage 0 6 Skills/24 exits、九份
  corpora byte identity、clean throwaway、pre-migration upgrade、`trellis update`、preset reapply
  与零 `.new`/`.bak`/drift 必须全部回归。
...
| R17 | `confirmed_scope_expansion` | comment 5053205362 与精确 proposal/action confirmation | `design.md` 2、8.3-8.4、10-11、13；`implement.md` Step 5、7-8 | AC7、AC9、AC10、AC13、AC22 |

R7、R12-R17 分别承接 comments 5042976037、5044522718、5045340918、5047220259、
5049566946、5050065847、5053205362 的 current authority；七个 `accepted_current` 语义均已由精确用户
...
- Requirement impact：本任务改变 production public Skill I/O、consumer projection、
  ...
  eval coverage、production native adapter owner staging、clarify_scope routing、active-task
  owner-checker/worktree binding、formal context snapshot replacement 与 upgrade contract，
  必须在同一 Phase 2 中同步 durable specs 与 public README。
...
- [ ] AC21：不同 bytes 的 task context snapshot 只有在 exact expected prior 与新 snapshot 全部
  ...
  `context_ready`。
- [ ] AC22：Shared native adapter 只为三个 production Skill ids 构造 checker-passed owner
  staging，并真实执行对应 public wrappers；三个代表 corpora 不再返回 owner-staging
  `execution_error`，actual exit 不由 `expected_exit` 驱动，#147 schema/runner/grader/protocol/
  evidence contract 与 #145 Stage 0 6×24 行为保持不变。

## 9. 完成条件

AC1-AC22 全部获得命令、测试或 semantic review 证据，Docs SSOT reconciliation 完成，
