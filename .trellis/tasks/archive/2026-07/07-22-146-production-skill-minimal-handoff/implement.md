...
- [x] 用户已授权一次性 R16 recovery exception，只用于恢复当前 post-commit task-local
  context re-entry；canonical/installed runtime 与定向 regression 已完成，未修改 adapter、
  P2-F002 或其它 product implementation。
- [x] Fresh context 已正式替换：snapshot
  `3f40c78c012252820080e0e15f87c7e0da3a1f6629f2b795356da67d2eecd0f3` supersede
  `41680b6c441af235e9db2cc4c17958a55800fb6ac6955d9263027802b348ccea`，绑定 task HEAD
  `e3efc635e36039f0db94a9d24eddad676ad7fe7b` 与 worktree-state digest
  `0bd71fb1ebfc0f82bcd73decb7f4d2e30acdfca59c99d77b772824db48871b16`。
- [x] 旧 planning approval `2b6917cc2891185a403c31b668c53179d6bae2233429287fa96a6ca8186727d8`、
  Phase 2 `c0f91fad9e457cf19665c7219353aa734e6af6f1d1796b2aa5ef5465cf1fc9b0`、
  Branch Review gate `113b5c75cfd8bbb2e0dc938f56f364bc3f4ede461f6783f9a67c502946573fea`
  与 rollup `2a28ac1e6933ffb973e1857b64ce84d00b8ef6c20a88e81b855480a7c4d6a4f1`
  均标记为 `stale`；旧 task commit 只保留历史，不作为 R17/P2-F002 验收证据。
- [ ] 本轮更新三份 planning 与 ledger，完成 contract-wording/plan semantic review，并在用户
  查看链接后取得 fresh post-planning approval。该 approval 前不得继续 production adapter、
  P2-F002、Phase 2、finding-fix commit 或 Branch Review closure。
...
- [ ] R12-R17 的六组 proposal/action 已作为 `accepted_current` 绑定 comments
  `5044522718`、`5045340918`、`5047220259`、`5049566946`、`5050065847`、`5053205362`，并写入
...
- [ ] 保留本轮 recovery 的 freshness 分层：`pre_task` current HEAD继续绑定 base-sync
  decision；task mode继续校验 selected-base local/remote refs、remote repo与task branch，但
  task HEAD/dirty state只绑定 `task_worktree_state`。补齐 source/installed/upgrade 回归证据。
...
- [ ] Shared native adapter 只为 `guru-approve-task-plan`、`guru-check-task`、
  `guru-create-task-commit` 增加 isolated owner staging，复用现有 recorder/checker/builder并
  真实调用 wrapper；不得新增 generic fallback。
- [ ] 分别执行 `approved-initial`、`passed-initial`、`revision-required` 代表 case，断言不再
  返回 owner-staging `execution_error`；request不携带 `expected_exit`，actual exit只来自
  wrapper checked result。
...
#147 schema、runner、grader policy、adapter protocol、semantic ownership、human review boundary
与run evidence无修改。
...
- [ ] 比较 canonical/installed native adapter bytes与 executable mode；验证 preset inventory、
  update/reapply与throwaway都分发同一三个 production owner-staging branches。
...
- [ ] 执行所有 profiles、11 exits、Stage 0 `keep_current_open_issue` regression 与 #145
  6×24 regression。
- [ ] 执行三个 production corpora 的 shared-native owner staging + 真实 wrapper cases，并
  验证 `expected_exit` 不进入 adapter request、owner builder或route selection。
...
### Step 10. Finding closure 与 fresh final-release review

- [ ] Fresh Phase 2 `passed` 后，由 `guru-create-task-commit` 创建新的 finding-fix commit；旧
  commit、Phase 2 与 review evidence 继续只保留为 stale history。
- [ ] Round 001 finding owner `review_146_final_r1` 必须在 current HEAD 上完成 P1-F001 与
  P2-F002 的闭环审查并返回零 findings；若该 technical agent 已 terminal failed、stale 或
  unfinished，必须按 workflow 记录完整 replacement chain，再由 replacement closure agent
  完成零 finding 闭环。
- [ ] Closure reviewer 不得成为 final reviewer。闭环后必须 dispatch 一个从未出现在 earlier
  review rounds 的全新 `最终放行审查代理`，覆盖 `origin/main...HEAD` 完整 diff、Docs SSOT、
  distribution/upgrade、deployment/safety 与全部 R1-R17/AC1-AC22。
- [ ] 主会话保留并扩展既有 `agent-assignment.json`、`reviews/*.md`、`review.md` 与
  `review-gate.json`，不得覆盖 Round 001 metadata；只有 closure-before-final、fresh final
  reviewer、current HEAD、零 P0-P3 findings 全部满足时记录 passing Branch Review Gate。
- [ ] `trellis-continue` 在 Branch Review Gate 后停止，不执行 push、PR 或 `finish-work`。
...
- 三个production Skill owner-staging shared-native execution + #147 request/protocol不变证明
...
- Owner-staging gate：native adapter只dispatch三个production Skill ids，复用checker-passed
  owner state并真实调用wrapper；#145 6×24与#147 schema/runner/grader/protocol无回归。
...
- Review-lifecycle gate：Round 001 finding owner已在current HEAD完成零finding闭环，随后由
  未参与earlier rounds的全新final-release reviewer完成完整diff放行，旧metadata被保留。
...
| RP12 | production owner staging要求修改#147 schema/runner/grader/protocol或以expected_exit选择route | 停止adapter方向并重新进入Scope Change Gate；不得stub output、降级execution claim或扩大generic dispatch |
...
- [ ] 同 finding owner closure与全新 final-release review依次完成，独立 Branch Review覆盖
  `origin/main...HEAD`完整diff并通过。
