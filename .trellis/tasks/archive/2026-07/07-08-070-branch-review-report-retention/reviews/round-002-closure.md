# Issue #70 Branch Review Gate 问题闭环审查原始报告
...
- 闭环目标：`round-001-final-release.md` 中的 P3 finding
- 上一轮审查 HEAD：`fa7d0574230f6773dc319af604929f9cb17b2cfa`
- 问题发现代理：`019f3fd7-da1e-7b81-96df-ca19846d3797`
- findings_count：0
...
- `docs/requirements/requirement-main.md` 对 issue #70 原始报告 + 汇总 + digest ledger 语义的同步。
...
- 为确认没有同范围回归，查看了当前 `requirement-main.md` 中 Branch Review、原始报告、汇总、`review_rounds[]`、`review_reports[]`、#61 顶层 artifact 表边界相关表述。
...
- Phase 3 finish / publish 不再描述为只产出 `review.md`，而是每轮保留 task-local `reviews/*.md` 原始报告，最终 `review.md` 作为汇总入口并链接原始报告。
- 历史来源新增 issue #70，明确 `agent-assignment.json.review_rounds[]` 与 `review-gate.json.verification_evidence.review_reports[]` 追溯原始报告 digest，并保留 #61 顶层 artifact 表默认只列 final `review.md` 的边界。
- 能力表已将旧的“审查报告必填”表述更新为“原始报告加最终汇总必填”，说明每轮 AI / human review 判断写原始 Markdown 报告，顶层 `review.md` 是最终汇总。
- Review gate recorder 描述已改为固化 final `review.md` digest 与 raw `review_reports[]` digest。
- Sub-agent assignment ledger 描述已包含原始报告 path / sha256 / size / modified_at。
- 默认 sub-agent mode 执行边界和 issue 索引已更新为 `reviews/*.md` 原始报告 + final `review.md` 汇总。
...
- `git diff fa7d0574230f6773dc319af604929f9cb17b2cfa..HEAD -- docs/requirements/requirement-main.md`：通过，确认上一轮指出的旧语义已改为原始报告 + 汇总 + digest ledger 语义。
