...
### R7. Production semantic eval assets

1. Evals 必须定义 clean install 后真实 public entry、real wrapper 和十个 profiles，
...
5. 本 Issue 不执行 live GPT-5.6 Sol matrix；deterministic gate 必须覆盖 corpus、runner、
   real wrapper、sandbox、no-model/fake production dispatch 与 fail-fast contract。
6. 只有另行完成并绑定 exact model、prompt、package、matrix 与 corpus identity 的 live
   run 才能形成模型行为证据；本 Issue、PR 和发布结论必须明确该证据缺失，不得声称
   pressure matrix、模型稳定性或永不复发已经通过。
7. 不同于实现 Owner 的 independent reviewer 必须检查全部十 profile contracts、真实
   production entry 的 deterministic/no-model evidence、非关键词判断、压力 corpus
   完整性和发布结论边界。
...
4. 官方 live 文档把完整升级定义为两步：`trellis upgrade` 把全局 CLI 升到发布版本，
   ...
   `trellis update --dry-run` 与适用的 `trellis update --skip-all` /
...
5. `trellis upgrade` 只能在 disposable npm prefix/container 中运行并验证 upgrade 前后
   ...
   output 明确报告 `MIGRATION REQUIRED` 时执行；`--skip-all` 保留已有修改并以
   非交互方式继续，验证流程不得使用 `--force`。
...
- [ ] #113 F-001 与 #236 攻击式 scanner 变体及 paired legitimate
      correctness/security cases 在 corpus、schema、route 和 no-model/fake-production
      contract 中形成完整相反分类，不使用关键词 classifier。
- [ ] 本 Issue 不执行 live production model matrix；PR 与发布结论明确缺少 live
      GPT-5.6 Sol pressure/stability evidence，不把 deterministic/no-model pass 写成模型通过。
