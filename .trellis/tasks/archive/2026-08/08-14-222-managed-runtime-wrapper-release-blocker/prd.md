...
- shared、Codex、Claude、Cursor eval adapter 的 `native_adapter.py` 第二跳同样必须通过
  adapter 所属 checkout 的 managed resolver，不得重新执行 PATH `python3`。
...
- Branch Review finding 修复后的 candidate bytes 不得继续使用 `0.6.5-guru.29`；
  extension revision 前进到 `0.6.5-guru.30`。尚未创建的 repo tag 仍为
  `v0.6.5-guru.6`。
...
- [ ] PATH Python 缺少 `jsonschema` 时，shared source/installed validator 与 eval/compat
  wrapper 均使用对应 checkout 的受管 Python并通过；PATH 完全没有 `python3` 时，四个平台
  adapter 的 source/installed 入口仍通过各自 checkout 的 managed resolver 启动。
...
- [ ] manifest、README、release notes 与 stable source 映射一致为 repo tag `.6`、extension
  revision `.30`、official Trellis CLI `0.6.5`。
