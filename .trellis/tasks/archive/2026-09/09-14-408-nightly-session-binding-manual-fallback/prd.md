...
- 原始 Guru source lock 位于 `trellis/presets/guru-team/source/trellis-source.json`，commit 为 `a2003296b4c4ce46c50d72ead3b2ec9c317f69fc`，CLI 为 `0.6.17`。
...
- 原始现场的 Git branch、official task metadata 与两份 Guru mapping 一致，但 `get_context.py` 显示无 current session task。提交 `b0b9446a` 已完成指定 Fork 采纳、session 回归及手动 Git/GitHub 边界；这不证明下节正常 Guru 创建链已完整验证。
...
| R408-03 | 同一明确 session 从 primary 经正常 Guru 创建入口建立 linked-worktree task 后回到 primary，继续解析同一 active task | 覆盖真实 public authoring、创建器、双端 mappings、boundary 与受控激活；`task.py current`、`get_context.py`、SessionStart、workflow-state 均指向该 task；裸 Git/官方 task-store probe 仅证明底层 session，不替代 Guru 链 |
| R408-04 | 保留旧完整 lifecycle、现有 Skill/exit/Finalizer/Publication/Merge 与 task/worktree 语义 | 正常 Agent 根据实际可见合同即可调用当前 owner，不依赖 eval runtime、手算私有绑定或手工拼接 task/mappings；现有 package、finish-family 与 installed 链通过；无新节点/Skill/exit/recovery checkpoint |
...
本节细化既有 R408-03/04，不新增手动恢复能力。参考现场中，正常 Intake 调用困难后转为手工创建，遗漏 worktree 内 mappings，直到 Phase 2 才暴露；后续补 mapping 只作为缺陷证据，不是本任务方案。

当前复核事实：真实 installed 六步 wrapper 重放已返回 `created`，创建器原本就双端写 mapping；但该 fixture 预先计算私有数据且未覆盖激活，不能证明正常 Agent authoring 闭合。已复现的缺口为：Agent 投影文档中的脚本/示例相对路径不存在；Clarification recorder 未落实所声明的 digest 派生；Readiness recorder 要求 caller 预先给出两个私有 gate digest。

修复必须使正常输入与既有 recorder 职责一致，并用当前正常入口补齐创建/激活回归；不恢复旧 `scope` aggregate，不将手工修 mapping 标准化。产品范围无新增问题；本轮实现细化尚未执行。正式发布、Issue closure 与 cleanup 仍为独立操作。
