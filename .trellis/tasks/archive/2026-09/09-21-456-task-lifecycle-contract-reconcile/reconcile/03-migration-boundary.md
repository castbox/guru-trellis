...
| `443-SUCCESS-DTO` | Bind output schemas and interface DTOs | Bind package owner | Public I/O design 11 Bind rows | Bind package activation and #434 graph | yes | Phase D443 Bind migration | All five Bind success outputs validate the same `TaskLifecycleDTO + resume_target` shape. |
| `443-EXPLICIT-MODE` | Bind interface/schema/runtime | Bind package owner | Session explicit-task contract | Context-less create/resume route and graph activation | yes | Phase D443 then Phase E434 | Bind interface/runtime tests emit the declared `explicit_task_mode` DTO; workflow consumption is owned only by `443-CONSUMER-ROUTERS`. |
...
| `443-MAPPING-DEPENDENCY` | Bind runtime legacy mapping readers/writers | Bind package owner | Retired mapping model supplied by #454 substrate | Any target Bind runtime | yes | Phase C + Phase D443 | Bind package search and tests show no active reader/writer depends on task/workspace mappings or path/branch/HEAD fields. |
| `443-CONSUMER-ROUTERS` | canonical/dogfood workflow Bind router declarations | #434 workflow owner | new Bind interface | Production routing | yes | Phase E434 graph activation | The activation candidate maps every declared Bind router to one consumer; workflow bytes are not changed by Phase D package migration. |
| `443-REGISTRY-SELECTOR` | registry Bind selector entry | #434 activation owner | Phase D443 package-ready identity | Selector flip/integrated state | yes | Phase E434 graph activation | The single activation transaction selects only the reviewed Phase D443 package identity. |
| `443-PROJECTIONS` | canonical Bind package/interface/schema and package-owned projection source templates | Bind package owner | migrated canonical package | Active installed/platform publication | yes | Phase D443 Bind migration | Canonical package contracts and projection source templates identify one package-ready byte set; no installed/platform target is written or used as Phase D proof. |
...
| `436-REACTIVATE-PLANNING` | Reactivate planning output/schema | Reactivate package owner | target DTO/branch/session outcome | Planning router activation | yes | Phase D436 then Phase E434 | Planning output validates the target DTO and declares the router-facing exit without owning workflow routing. |
| `436-REACTIVATE-LEGACY-EXITS` | Reactivate interface/schemas/runtime | Reactivate package owner | target exit closure | Any alias or old route | yes | Phase D436 Reactivate migration | Reactivate package interface, schemas, runtime, and tests expose no legacy direct requirements/implementation/evidence-refresh exits; workflow edge retirement belongs to Phase E434. |
| `436-REACTIVATE-NEW-EXITS` | Reactivate interface/output schemas | Reactivate package owner | design 11 Reactivate rows | New graph routing | yes | Phase D436 then Phase E434 | All target Reactivate exits validate their package DTOs and expose stable exit identities; #434 alone maps them to consumers. |
...
| `436-CLOSURE-NEW-CONFLICT` | Closure interface/output schema/runtime | Closure package owner | external-change conflict contract | Conflict recovery route | yes | Phase D436 then Phase E434 | Live-state mismatch tests emit the declared external-change-conflict DTO and exit identity; #434 alone maps that exit to Closure semantic re-entry. |
...
| `436-FINISH-EXITS` | Finish interface/output schemas and exit identities | Finish package owner | design 11 Finish rows | Closure refresh/manual cleanup routes | yes | Phase D436 then Phase E434 | Target Finish exits validate package DTOs and stable exit identities; downstream route ownership remains outside the Finish package row. |
...
| `436-RESOURCE-OWNERSHIP` | shared common-dir resource ledger schema/runtime and ownership resolution | #454 substrate owner | single common-dir ledger | Any package-local second ledger | yes | Phase C + Phase D436 | Substrate tests show one common-dir lifecycle ledger and one ownership-resolution authority; package consumers do not create a second ledger. |
...
- #454 substrate owns identity, generation, source/target relations, branch association/rebind, live checkout resolution,
  session storage, resource ledger, and shared DTO definitions. Phase C rows contain only these substrate surfaces.
- #443 owns Bind semantic behavior and package contract, not branch/checkout/resource authority. Phase D443 owns only
  canonical Bind package/interface/schema/runtime and package-owned source projections.
- #436 package owners retain their semantic judgments and consume substrate; Finish seals inventory, Cleanup deletes resources.
  Phase D436 owns only package contracts, schemas, runtimes, and package-local semantic behavior.
- #434 owns only workflow invocation, typed-exit routing, targets/stops, registry selector flips, active manifest
  inventory, installed/platform projection bytes, and atomic activation. These Phase E surfaces are never completion proof
  or affected files of a Phase C/D package row.
- The single #434 activation owner alone flips registry selectors, rewrites the active manifest inventory, and publishes
  active installed/platform projection bytes; package owners produce package-ready canonical contracts without activating
  those surfaces.
