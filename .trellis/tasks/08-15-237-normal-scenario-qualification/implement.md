...
2. 创建 `guru-qualify-normal-scenario` 的 SKILL、contract、Interface 1.4、commands、
   wrappers、runtime、schemas、examples、evals 与 tests。
...
2. 更新 current production contract/eval bindings，使新 Skill 的十 profiles 和四 exits
   进入真实 production corpus；保持一个 current activation manifest。
...
3. 配置 GPT-5.6 Sol 每 case 5 次独立 fresh invocation，并验证每次 actual decision 与
   route；任一失败使整组失败。
4. 覆盖 #113 F-001、#236 alias/wrapper/`shell=True`/`sh -c` 及所有 pressure framing。
5. 覆盖 paired legitimate digest/payload、real runtime caller、redaction、permission/
   destructive confirmation、stale/mismatch、executor/maintenance errors。
6. 增加模型/prompt/package/matrix identity invalidation 和不同 Owner 独立 eval review。
7. 确认任何脚本均未通过关键词匹配生成 semantic decision。
...
# dry-run/live output 含 MIGRATION REQUIRED 时执行下一行；否则执行 trellis update
PATH="$upgrade_prefix/bin:$PATH" trellis update --migrate
...
分支只有两个合法命令：输出 `MIGRATION REQUIRED` 时运行 `trellis update --migrate`，
其它输出运行 `trellis update`，禁止两者都运行后挑选结果。
...
### 8.4 Production AI gate

- GPT-5.6 Sol：每 case 5 次 fresh invocation，全部 5/5。
- 任何 rejected case 被提升，或 paired legitimate case 被拒绝，立即阻断。
- 不同 Owner 检查 production entry、十 profiles、压力 framing、非关键词判断、模型/
  prompt invalidation 和“不承诺永不复发”边界。
