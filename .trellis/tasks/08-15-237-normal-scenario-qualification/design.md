...
四个平台 adapter 必须读取已安装 package、执行真实 `scripts/invoke.sh` public entry、
形成可验证 trace。Runner 在 native boundary 外评分；不能用 mock classifier、关键词
扫描、schema-only success 或手写 result 替代 AI judgment。

### 11.3 Model invalidation

Eval evidence 绑定 model id、关键 prompt/corpus identity、package identity 与 case matrix。
...
- 同一 clean/existing throwaway project 随后执行 `trellis update --dry-run`。只有输出
  `MIGRATION REQUIRED` 才执行 `trellis update --migrate`，其它情况执行
  `trellis update`。随后执行 marketplace workflow `--create-new` preview、active switch、
