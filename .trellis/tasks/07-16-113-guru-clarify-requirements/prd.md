...
- `active_task_scope_change` 返回 `clear` 或 `new_task` 时，`scope_proposals[]` 必须包含一个或多个 terminal proposal；terminal decision 闭集为 `accepted_current`、`related`、`followup`、`new_task`、`out_of_scope`、`mechanism_removed`、`mechanism_replaced`。空数组或 `pending` 必须失败并阻塞 progression；`new_task` 还必须包含 `decision=new_task`。
...
每个 active-task final proposal，无论分类为 `accepted_current`、`related`、`followup`、`new_task` 还是 `out_of_scope`，都必须满足以下合同：

- 存在 proposal-digest-bound exact 用户决策证据；普通“继续”、task 创建确认、planning approval 或 review confirmation 不得替代该证据。
- 当前 `issue-scope-ledger.json.scope_decisions[]` 中存在唯一 exact structured decision trail，绑定 proposal 决策、用户证据、live GitHub comment/body authority 及其 `updated_at`、task update 前 context digest、三份 planning document、完整 planning approval、review state、stale downstream gates、interrupted resume target 与 re-entry owners。
- GitHub authority mutation 后必须返回 `refresh_context`。Re-entry 必须证明 context snapshot 的 `generated_at` 不早于 live authority `updated_at`，随后完成 task-local planning/ledger update 与 fresh planning approval；task-local update 由当前文件 digest、完整 approval、review/stale identities 直接校验，不强制第二次 context snapshot digest 变化。全部绑定重新匹配后，`accepted_current` 才能进入 planning review，其余分类才能恢复 exact interrupted progression，`new_task` 才能进入 #112 的 side-effect-free draft route。
- `planning-approval.json` 不得只按文件 hash 判断存在性。Checker 必须复用 schema 1.2 完整 validator，验证 explicit post-planning confirmation、passed ambiguity review、固定受控词扫描、空 `unchecked_normative_hits[]`、七个审查维度以及三份 planning artifact 的 path/hash/size 绑定；两行占位 planning 文档、最小 JSON 或其它不完整 evidence 必须失败。
...
- `mechanism_removed` 与 `mechanism_replaced` 是 proposal terminal disposition，不是 current/related/followup/new-task/out-of-scope scope classification。两者要求 `optional_mechanism_origin=true`、`confirmation_ref=null`，不得进入 `scope_decisions[]` 或触发 GitHub/task authority mutation；`clear` 当且仅当 payload 中全部 proposal 已 terminal。若替换机制形成独立产品价值，必须创建新的 scope proposal 并按五类 scope classification 重新确认。
...
- Active-task 的五类 scope classification 必须把最终 decision trail 写入当前 task 的 ledger/planning/review evidence；mechanism disposition 只保留在当前 result 的 `scope_proposals[]`，两者均不得写其它 task directory。
- `scope_decisions[]` 是 active-task final classification 的 task-local structured SSOT；每条 trail 必须能由 runtime 从当前 ledger 解析并与 result 中的 `decision_trail` exact 匹配。
...
- [ ] AC12：Active-task `clear/new_task` 拒绝空或非 terminal `scope_proposals[]`；五类 scope classification 均要求 exact 用户证据与当前 ledger structured trail，`mechanism_removed/replaced` 作为无需 confirmation/trail 的合法 terminal disposition 可进入 `clear`；完整 schema 1.2 planning approval 和真实三份 planning 文档必须通过 validator，占位 JSON、占位文档、缺失 trail、stale authority 或未完成 authority-before-context-before-task-update 顺序均失败并阻塞。
