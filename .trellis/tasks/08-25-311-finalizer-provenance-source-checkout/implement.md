...
- [ ] 从当前 source finding-fix 生成新 clean candidate，在现有 fixture worktree 上重新安装并完成
...
- [x] 旧 candidate 的 `guru-check-task` 曾因 representative closeout 尚未授权返回 `blocked`；授权后
  ...
  regression。当前完整 worktree candidate 必须重新执行 fresh Architecture Phase 2 与
  `guru-check-task`，不得复用旧 checkpoint。
