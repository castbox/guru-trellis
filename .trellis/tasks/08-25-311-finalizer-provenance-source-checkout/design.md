...
## 6.1 Standalone Verifier Failure Evidence

`verify_trellis_compatibility_matrix.py` 继续拥有 matrix execution。它在异常边界投影一个 bounded
failure object，必须区分 pre-matrix、matrix-cell、post-matrix，并在适用时携带 `cell_id`、稳定 command
label、exit code 与 bounded output tail。`verify-throwaway-install.sh` 只透传该单一结构化终态。

`guru-verify-extension-installation` outer owner 在 temporary source/install root cleanup 前解析该终态，
将 failure facts 与既有 command hash/size 一起放入 standalone execution evidence。schema 禁止任意额外
字段；credential、认证 remote、token 与环境 secret 不得进入 failure detail。解析失败本身必须形成
明确的 `unparseable_failure_output`，不得静默退化成只有 hash/size。

该 evidence owner 与 Finalizer lifecycle 隔离：Finalizer 不 import/call verifier，verifier failure facts
不进入 Finalizer preview、transaction、gate、publication 或 Merge handoff。成功路径、capability 与 asset
inventory contract 保持不变。
...
- recovery ordering：首次 preview 先分类 exact existing PR，无 PR/remote branch 的 `prepared` state 再
  推断 provenance reprepare；post-bind transaction 继续先行；remote/plan/scope drift 保持阻断。
...
### 7.4 Standalone verifier failure evidence

通过 focused fixture 令 compatibility matrix 在确定 cell/command 失败，断言 outer verifier 在清理
temporary workspace 后仍返回结构化 failure facts；无 cell 的 pre-matrix failure 必须显式返回 null/
not-applicable，而不是伪造 cell。测试不得执行完整 live matrix，也不得对 `cdc55ca9` 进行第 4 次
throwaway。
...
- `trellis/skills/guru-team/packages/guru-verify-extension-installation/**` 的 failure evidence contract、
  schema、example 与 focused tests
- `trellis/presets/guru-team/scripts/{bash/verify-throwaway-install.sh,python/verify_trellis_compatibility_matrix.py}`
  的结构化 failure projection
...
| matrix failure 被临时目录清理抹除 | cleanup 前投影 bounded structured failure facts，并用 focused fixture 验证 |
...
- verifier failure evidence shape 已经 scope qualification 并由用户接受；若后续要求改变 public exit、
  Finalizer transaction state、verifier lifecycle ownership 或 Issue close scope，立即返回 Phase 1。
