...
5. #144/#145/#147 的已交付合同是本次实现基线；本任务只新增三包 migration、9/35
   ...
   binding、formal snapshot replacement 与三个 production Skill owner-staging 修复。
...
| Shared native eval adapter | `trellis/skills/guru-team/adapters/eval/native_adapter.py` | 只为三个 production Skill ids 构造隔离 owner state，并执行真实 public wrapper |
...
### 8.4 Production owner staging

Shared native adapter 在 request/profile schema 已验证后，只对
`guru-approve-task-plan`、`guru-check-task`、`guru-create-task-commit` dispatch 对应的
production owner staging builder：

```text
validated public input + isolated fixture
  -> reuse current production recorder/checker/builder
  -> materialize checker-passed task-local owner state
  -> call the selected package public wrapper
  -> return wrapper actual typed-exit DTO
```

Adapter 不读取 `expected_exit`，不根据 corpus expected result选择 route，也不生成 semantic
judgment。`expected_exit` 保持 runner/grader 的事后断言。Canonical adapter 与 installed mirror
同步；#147 的 eval schema、runner、grader policy、adapter request/response protocol、run evidence
contract、semantic owner 与 human-review boundary 均保持不变。

Owner staging 的 production allowlist 精确为上述三个 Skill ids。#145 的 Stage 0 6 Skills/24
exits 使用既有 staging 路径并执行完整回归；未知或未支持 Skill 继续返回现有 unsupported/
execution error，不得被 generic production fallback 接受。
...
### 9.6 Base freshness 与 task worktree freshness 分层

`context_live_base_errors()` 按 mode 分开验证两组 identity：

- `pre_task`：selected base local/remote refs、remote repo、current branch 与 current HEAD 必须
  同时匹配 base-sync decision；HEAD 前进返回 `base_head_stale`。
- task mode：selected base local/remote refs 与 remote repo 继续匹配 base-sync decision，
  current branch继续匹配 `task.json.branch`；task HEAD、dirty paths、status/mode/rename/content
  identity 只与 9.4 的 `task_worktree_state` 比较。正常 post-commit task HEAD 不再与 base
  decision HEAD 比较。

本次一次性 recovery 只修改 canonical/installed shared runtime 的上述 task-mode condition，
并补 `ChangeContextDiscoveryTests` 证明旧 snapshot 在 HEAD 前进后返回
`task_worktree_state_stale`、刷新 worktree state 后通过、`pre_task` 仍返回
`base_head_stale`。Recovery 随后正式写入 fresh snapshot
`3f40c78c012252820080e0e15f87c7e0da3a1f6629f2b795356da67d2eecd0f3`，其
`superseded_snapshot_sha256` 为
`41680b6c441af235e9db2cc4c17958a55800fb6ac6955d9263027802b348ccea`。该 bootstrap
完成不构成 production adapter、P2-F002 或其余 finding-fix implementation approval。
...
3. 实现 authoring-seed field partition/no-overwrite merge、deterministic candidate builder、
   ...
   snapshot replacement，以及三个 production Skill 的 isolated owner staging。
4. 添加三份 corpora、production manifest、9/35 closure tests，并执行三个 production
   shared-native wrapper cases 与 #145 Stage 0 6×24 全回归。
...
- Context freshness 分层保持 selected-base refs 与 task branch 校验：只有 `pre_task` 将 current
  HEAD 绑定 base decision，task mode 将 task HEAD/dirty state 绑定 private
  `task_worktree_state`。
...
- Shared native adapter 只新增三个 production owner staging branches；#147 schema、runner、
  grader、protocol、run evidence 与 expected-exit semantics 保持 byte/behavior compatibility。
...
- Production owner staging 无法复用现有 recorder/checker/builder 或要求修改 #147 contract：
  停止 adapter 方向并重新进入 Scope Change Gate，不用 `expected_exit`、stub output 或降级
  execution claim 绕过真实 wrapper。
...
R12-R17 都是在受支持正常路径中复现并经精确 proposal/action confirmation 纳入的
...
replacement，以及 8.4 的三个 production owner-staging branches。R7 是正常 public wrapper
路径中已复现的 ordinary correctness expansion。
...
- `.trellis/spec/workflow/quality-guidelines.md`：source/installed/negative/distribution gates 与
  production native owner-staging 真实 wrapper regression；
...
validator、production owner staging 与 tests 后，implementation agent 必须把最终 field/profile ids、validation
...
| C10 | 固定locator执行exact-prior formal replacement，记录optional superseded digest，并分离base/task freshness | 删除/改名旧snapshot；versioned archive/current pointer；task HEAD绑定base decision | 最小修复active task authority刷新与post-commit re-entry，同时保留base refs、task worktree、历史读取、重复执行结果一致性与public API | `true/false` |
| C11 | Shared native adapter只为三个 production Skill ids复用owner recorder/checker/builder并真实调用wrapper | 删除/降级native execution；修改#147 runner/schema/grader；独立prerequisite后拆分验收链路 | 直接关闭R6/AC7的同一受支持执行缺口，同时保持#147 ownership与actual-exit边界 | `true/false` |
