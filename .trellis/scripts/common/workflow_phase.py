...
def resolve_effective_platform(platform: str, config: dict) -> str:
    ...
    Default is ``sub-agent``. Codex sub-agents run with ``fork_turns="none"``
    isolation, so main-session dispatch prompts must include
    ``Active task: <task path>`` and sub-agents fall back to
    ``task.py current --source``. Invalid / missing values also fall back to
    sub-agent. Set ``codex.dispatch_mode: inline`` only as an explicit
    downgrade.
