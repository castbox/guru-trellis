...
CONSUMERS = {
    ...
    "mechanism_revision_required": {"kind": "workflow", "id": "guru-solution-mechanism-mechanism-router"},
    "blocked": {"kind": "stop", "id": "solution-mechanism-qualification-blocked"},
}
OUTPUT_SCHEMAS = {
...
def record_value(package_root: Path, authored: object) -> dict:
    ...
    decisions = {item["decision"] for item in authored["candidate_results"]}
    has_scope = "scope_confirmation_required" in decisions
    has_mechanism = bool(decisions & {"mechanism_removed", "mechanism_replaced"})
    if "blocked" in decisions:
        expected_exit = "blocked"
    elif has_scope and has_mechanism:
        raise CommandError("schema_mismatch", "typed_exit", "Return one unambiguous exit for the complete candidate set.")
    elif has_scope:
        expected_exit = "scope_confirmation_required"
    elif has_mechanism:
        expected_exit = "mechanism_revision_required"
    else:
        expected_exit = "classified"
    if exit_id != expected_exit:
        raise CommandError("schema_mismatch", "typed_exit", "Match the AI-selected exit to the complete candidate decision set.")
...
def typed_output(package_root: Path, recorded: dict) -> dict:
    ...
    continuation_id = "solution-mechanism:" + recorded["input_sha256"][:24]
    if exit_id == "classified":
        ...
        output = {"exit_id": exit_id, "handoff_profile": "solution_mechanism_scope_confirmation", "mode": public_input["mode"], "target_locator": public_input["target_locator"], "resume_target": RESUME_TARGETS[public_input["profile"]], "continuation_id": continuation_id, "candidate_refs": list(public_input["candidate_refs"])}
