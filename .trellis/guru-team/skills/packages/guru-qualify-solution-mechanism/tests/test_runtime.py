...
class SolutionMechanismQualificationRuntimeTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        (self.repo / "AGENTS.md").write_text("solution mechanism authority\n", encoding="utf-8")
    ...
    def semantic(self, profile: str = "implementation_discovery", exit_id: str = "classified", decision: str = "qualified_current", mode: str = "workflow") -> dict:
        ...
        return {"schema_version":"1.0","skill_id":"guru-qualify-solution-mechanism","public_input":self.public_input(profile, mode),"candidate_results":[{"candidate_ref":"candidate-1","decision":decision,"reason":"Current AI-owned scope-first conclusion.","witness":witness}],"ai_review_gate":{"status":"passed","reviewed_candidate_refs":["candidate-1"],"summary":"The complete candidate set was reviewed."},"typed_exit":exit_id,"consumer":copy.deepcopy(consumers[exit_id])}
    ...
    def test_four_typed_exits_and_fixed_consumers(self) -> None:
        ...
        self.assertEqual("solution_mechanism_scope_confirmation", scope["handoff_profile"])
    ...
    def test_runtime_validates_closed_exit_aggregation_without_semantic_judgment(self) -> None:
        with self.assertRaises(CommandError) as raised:
            self.invoke(self.semantic(exit_id="classified", decision="scope_confirmation_required"))
        self.assertEqual("schema_mismatch", raised.exception.code)
        self.assertEqual("typed_exit", raised.exception.field_path)
    ...
    def test_fail_closed_shape_identity_and_freshness_cases(self) -> None:
        ...
        stale_planning = self.semantic(profile="phase2_candidate_set")
        (self.repo / ".trellis/tasks/current/prd.md").write_text("changed planning\n", encoding="utf-8")
        with self.assertRaises(CommandError) as raised:
            self.invoke(stale_planning)
        self.assertEqual("stale_identity", raised.exception.code)
