...
class WorkspaceTest(unittest.TestCase):
 def setUp(self):
  self.tmp=tempfile.TemporaryDirectory();self.parent=Path(self.tmp.name);self.repo=self.parent/"repo";self.remote=self.parent/"remote.git";subprocess.run(["git","init","-q","--bare",str(self.remote)],check=True);subprocess.run(["git","init","-q","-b","main",str(self.repo)],check=True);self.git("config","user.name","Test");self.git("config","user.email","test@example.invalid");shutil.copytree(PACKAGE.parents[4]/".trellis/scripts",self.repo/".trellis/scripts",ignore=shutil.ignore_patterns("__pycache__","*.pyc"));(self.repo/"seed").write_text("seed\n");self.git("add","seed", ".trellis/scripts");self.git("commit","-qm","seed");self.git("remote","add","origin",str(self.remote));self.git("push","-qu","origin","main");self.head=self.git("rev-parse","HEAD");self.configure();self.plan=self.make_plan()
 ...
 def workspace_state(self,workspace):
  paths=[workspace/Path(self.plan["side_effects"]["task_artifacts"][0]).parent/"task.json",self.repo/".trellis/.runtime/guru-team/workspaces/027-workspace.json",self.repo/".trellis/.runtime/guru-team/tasks/027-workspace.json",workspace/".trellis/.runtime/guru-team/workspaces/027-workspace.json",workspace/".trellis/.runtime/guru-team/tasks/027-workspace.json"]
 ...
 def test_real_workspace_mutation_check_and_invoke(self):
  pp=self.write("plan.json",self.plan);self.assertEqual(self.plan,record.run(PACKAGE,{},["--root",str(self.repo),"--input",str(pp)]));result,checked=self.execute_and_check();workspace=(self.parent/"repo-worktrees/027-workspace").resolve();self.assertTrue(workspace.is_dir());task_dir=Path(self.plan["side_effects"]["task_artifacts"][0]).parent.as_posix();mapping=json.loads((self.repo/".trellis/.runtime/guru-team/tasks/027-workspace.json").read_text());self.assertEqual({"schema_version":"1.0","workspace_slug":"027-workspace","workspace_path":str(workspace),"task_artifact_dir":task_dir},{key:mapping[key] for key in ("schema_version","workspace_slug","workspace_path","task_artifact_dir")});self.assertTrue((workspace/".trellis/.runtime/guru-team/tasks/027-workspace.json").is_file());boundary=PACKAGE.parents[0]/"guru-finalize-task/runtime/legacy.py";boundary_result=subprocess.run([sys.executable,str(boundary),"check-workspace-boundary","--root",str(workspace),"--task",task_dir,"--json"],text=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE);self.assertEqual(0,boundary_result.returncode,boundary_result.stderr);self.assertEqual("ok",json.loads(boundary_result.stdout)["status"]);self.assertNotIn(str(self.parent.resolve()),json.dumps(result));env=self.write("invoke.json",{"result":checked});self.assertEqual({"exit_id":"created"},invoke.run(PACKAGE,{},["--root",str(self.repo),"--invocation",str(env)]))
 ...
 def test_checker_rejects_every_task_payload_field_drift(self):
  result,_=self.execute_and_check();workspace=self.parent/"repo-worktrees/027-workspace";task_path=workspace/Path(self.plan["side_effects"]["task_artifacts"][0]).parent/"task.json";expected=json.loads(task_path.read_text());pp=self.write("checker-task-drift-plan.json",self.plan);rp=self.write("checker-task-drift-result.json",result)
  for field in ("name","title","status","creator","scope","worktree_path"):
