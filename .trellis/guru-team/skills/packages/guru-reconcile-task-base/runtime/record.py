...
from common import POST_REVIEW_PROFILES, checkpoint_path, digest, objective_identity, output_for, parse, read_json, repo_root, validate_json, validate_public, validate_result
...
def run(package_root: Path, command: dict, argv: list[str]) -> dict:
    ...
    if exit_id == "review_continuity_required":
        if public["profile"] not in POST_REVIEW_PROFILES or receipt is None:
            raise CommandError("schema_mismatch","reconciliation_result","Post-review continuity requires the checked local reconciliation result.")
        if any(receipt[key] != value for key,value in {
            "task_ref":public["task_ref"],"prior_task_head":public["task_head"],"old_base_head":public["old_base_head"],"new_base_head":public["new_base_head"],"branch_review_commit":public["branch_review_commit"],"candidate_tree_sha256":gate.get("route_payload",{}).get("candidate_tree_sha256")}.items()):
        ...
        if receipt is not None:
            raise CommandError("schema_mismatch","reconciliation_result","Only review_continuity_required consumes a reconciliation result.")
        current_head=public["task_head"]
    objective_identity(repo,public,expected_head=current_head)
    output=output_for(public,gate,exit_id,task_head=current_head); validate_json(output,package_root/f"schemas/public-{exit_id.replace('_','-')}-output.schema.json","typed_output")
    result={"schema_version":"2.0","skill_id":"guru-reconcile-task-base",**{k:copy.deepcopy(v) for k,v in public.items() if k != "task_head"},"prior_task_head":public["task_head"],"task_head":current_head,"semantic_gate":gate,"reconciliation_result":receipt,"typed_output":output}
