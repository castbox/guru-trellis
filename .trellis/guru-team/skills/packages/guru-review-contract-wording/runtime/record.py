...
def run(package_root:Path,command:dict,argv:list[str])->dict:
 ...
 if exit_id=="pass" and (gate.get("status")!="passed" or unchecked or auth.get("revisions")):raise CommandError("schema_mismatch","typed_exit","Pass requires a passed Gate, no revisions, and no unchecked hits.")
 if exit_id=="content_changed" and (gate.get("status")!="passed" or not auth.get("revisions") or unchecked):raise CommandError("schema_mismatch","typed_exit","Content changed requires a passed Gate, revisions, and no unchecked hits.")
 if (exit_id=="blocked") != (gate.get("status")=="blocked"):raise CommandError("schema_mismatch","typed_exit","Blocked exit and blocked Gate must be equivalent.")
