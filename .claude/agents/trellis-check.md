...
- In Phase 2 check mode, run `.trellis/guru-team/scripts/bash/check-planning-approval.sh --json --task <task-path>` and stop if it rejects a missing, stale, legacy, or non-approved owner result. Do not interpret the planning owner's private checkpoint fields; the main workflow reruns the semantic owner and auto-consumes its mapped exit. In Branch Review mode, reread current authority and planning documents as review scope instead of running planning recorder/validator scripts.
