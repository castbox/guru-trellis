...
python3 -c 'import json, sys; source = json.loads(sys.argv[1]); installed = json.load(sys.stdin); assert source["status"] == installed["status"] == "passed"; expected={"invoke_markers":12,"exit_markers":46,"target_markers":27,"planned_ids":[]}; assert all(source["facts"][key] == installed["facts"][key] == value for key,value in expected.items())' "$SOURCE_SKILL_VALIDATION_JSON" <<<"$INSTALLED_SKILL_VALIDATION_JSON"
...
contracts["public_input_schema_ids"] = [
    ...
    if not value.startswith(("guru-extension-", "guru-finalize-", "guru-production-"))
]
...
python3 -c 'import json, sys; source=json.loads(sys.argv[1]); installed=json.load(sys.stdin); expected={"invoke_markers":12,"exit_markers":46,"target_markers":27,"legacy_ids":[],"planned_ids":[]}; assert source["status"] == installed["status"] == "passed"; assert all(source["facts"][key] == installed["facts"][key] == value for key, value in expected.items())' "$PRE146_SOURCE_VALIDATION_JSON" <<<"$PRE146_INSTALLED_VALIDATION_JSON"
for skill_id in \
  guru-approve-task-plan \
  guru-check-task \
  guru-clarify-requirements \
...
for skill_id in guru-approve-task-plan guru-check-task guru-create-task-commit guru-finalize-task guru-review-branch guru-review-task-publication guru-verify-extension-installation; do
  PRE146_EVAL_JSON="$(
