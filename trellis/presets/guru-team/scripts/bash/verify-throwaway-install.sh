...
python3 -c 'import json, sys; payload = json.load(sys.stdin); assert payload["status"] == "ok"; assert payload["checkpoint"] == "initial-install"; assert payload["exit_family_count"] == 23; assert len(payload["six_step_transcript"]) == 6; assert payload["workspace"]["actual_exit"] == "created"; assert payload["workspace"]["checker_status"] == "passed"' <<<"$INITIAL_PHASE0_TRANSCRIPT_JSON"
...
python3 -c 'import json, sys; payload = json.load(sys.stdin); assert payload["status"] == "ok"; assert payload["checkpoint"] == "update-reapply"; assert payload["exit_family_count"] == 23; assert len(payload["six_step_transcript"]) == 6; assert payload["workspace"]["actual_exit"] == "created"; assert payload["workspace"]["checker_status"] == "passed"' <<<"$UPDATED_PHASE0_TRANSCRIPT_JSON"
