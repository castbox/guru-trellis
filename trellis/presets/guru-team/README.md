...
`0.6.5-guru.19`；已发布 stable source 仍是 `v0.6.5-guru.2`。Preset 将 active package
...
shared/Codex/Claude/Cursor descriptor、可执行 wrapper、shared native runtime，以及 package-local `evals/**` 的
...
`.new`/`.bak` 检查。Adapter wrapper 必须从 descriptor 路径执行，并从 `PATH` 检测
documented native command；installer 不写本机 executable override。普通 Skill invocation
