...
If a reapply reconstructs the same stable installed state after omitting
top-level `installed_at` / `source` and per-file `installed` / `unchanged`
action labels, it preserves the previous manifest bytes. Any other managed
install fact change records the current timestamp and source. The raw `apply.sh`
