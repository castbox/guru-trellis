...
Use `--finding` or `--findings-file` when the review has P0/P1/P2/P3 findings. P0/P1/P2 block finish-work. A passing gate must include a Chinese `--summary`, at least one concrete `--evidence` line, and `--review-report` pointing at task-local `review.md`; `--reviewer` is identity metadata only. `review-branch.sh` records and validates the prior review; it is not the reviewer.
