...
Before draft PR create, finish-work writes and commits task-local `pr-readiness.json.publish_inputs`. It binds repo, base/head branch, Branch Review Gate HEAD, exact title, `pr-body.md`, body SHA-256, `draft=true`, reviewed source, and `closeout_plan_digest`. Retries validate these committed inputs and the plan before resolving the exact repo/head/base PR identity.
