...
`check-context-discovery`。Exits 是 `context_ready` ->
workflow route `guru-clarify-requirements`、`refresh_base` -> `guru-sync-base`、`blocked` ->
`change-context-blocked`。该 route 是现有 Phase 0 clarification/check-env/prepare-task
continuation，不激活 #113 package；source/installed validator 同时解析 active Skill consumer
及唯一 workflow/stop target markers。
