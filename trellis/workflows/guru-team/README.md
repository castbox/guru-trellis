...
stable 是 `.2`；本分支 canonical `.13` 在 merge/tag/远端验证完成前不是 stable source。
...
- `ready` -> planned `guru-create-task-workspace`
...
#112 激活前，`ready` 必须在 missing-Skill gate stop fail closed，不得回退
`guru-full-task-intake-chain`、`check-env` 或 `prepare-task`。只有后续 active
`guru-create-task-workspace` 才能把同一 checker-passed bytes 持久化到 direct active task 的
...
remote-default provenance. GitHub, worktree, and task mutation boundaries each get an
independent adjacent guard; the task guard runs after workspace/identity setup
and before `task.py create`.

Resolution/result facts remain stdout-only. Planner and every mutation guard
consume the preceding post-sync digest, rerun the shared resolver/sync core,
and return the next post-sync digest; there is no
