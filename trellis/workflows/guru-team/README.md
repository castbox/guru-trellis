...
`trellis-continue` 不得 push 分支、创建 PR 或调用 `publish-pr`。PR 发布只发生在
`trellis-finish-work` 成功 archive task、记录 journal、提交允许的 Trellis
metadata 之后。`publish-pr.sh` 默认拒绝普通直接调用；只有 `finish-work.sh` 的内部
调用或 finish-work 已完成后的显式 recovery/debug flag 可以进入 publish 检查。
