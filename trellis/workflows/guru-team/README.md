...
三包、十个 profiles 与 11 exits。当前 active closure 为 9/35 且 `legacy_skill_ids=[]`；
...
三条 semantic package handoff 使用 additive target-owned
`skill_input_authoring_seed`：planning revision self-reentry、Phase 2 passed 到 initial
commit、commit revision self-reentry。Producer 只投影 minimal seed，caller AI 编写 target
...
中的九个 packages 已维护 canonical corpora 并覆盖全部 35 exits/profile；Stage 0 的
