...
issue、worktree、branch、Trellis task，也不会在未确认 `source_issue` 时写
`.trellis/guru-team/handoff.json`。没有传入 source issue 时，它只在 stdout JSON
中输出 `proposed_issue`、duplicate candidates、base branch、branch name、
workspace path、`create_task_command` 和 `handoff_written: false`。
