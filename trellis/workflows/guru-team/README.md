...
`trellis-continue` 写完 `review.md` 与 `review-gate.json` 后必须停止，不提交这些
metadata、不 push、不创建 PR；`finish-work` 负责 archive、journal、剩余 metadata
commit 和自动 publish。`finish-work` 只容忍 reviewed HEAD 之后的 Trellis
metadata-only commit，任何代码、配置、脚本、schema、CI/CD、部署或 preset 变更都要
重新过 Branch Review Gate。
