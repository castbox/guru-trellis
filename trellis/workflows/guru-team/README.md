...
stable 是 `.2`；本分支 canonical `.19` 在 merge/tag/远端验证完成前不是 stable source。
...
extension version 是待发布的 `0.6.5-guru.19`，已发布 stable source 仍是
`v0.6.5-guru.2`。
...
`1.3+minimal_handoff`。#144 不迁移九个 production payload，它们全部保持 legacy；
#145/#146 负责后续 migration。两版均保留 `workflow` / `standalone` mode id 和必填
`judgment_mode`，并以
...
仍为 legacy；#145/#146 完成后续迁移与 coverage closure。四个 descriptor 分别绑定
可执行 `shared.sh|codex.sh|claude.sh|cursor.sh`；wrapper 从 `PATH` 检测
`guru-team-shared-eval|codex|claude|cursor-agent`，组装平台专用非交互 argv。Runner 在 native
