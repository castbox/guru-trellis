...
def install_current_contract_schemas(skills_root: Path) -> None:
    ...
    shutil.copyfile(
        SKILLS_ROOT / "schemas/skill-interface-1.5.schema.json",
        schemas / "skill-interface-1.5.schema.json",
    )
    shutil.copyfile(
    ...
    shutil.copyfile(
        SKILLS_ROOT / "schemas/skill-registry-1.4.schema.json",
        schemas / "skill-registry-1.4.schema.json",
    )
    shutil.copyfile(
...
class SourceValidationTests(unittest.TestCase):
    ...
    def test_production_registry_activates_current_public_skills(self) -> None:
    ...
    def test_active_task_commit_contract_consumes_phase2_passed_dto(self) -> None:
        ...
        self.assertIn(
            "current reviewed-content identity still matches",
            phase2_result["freshness"],
        )
        self.assertNotIn(
            "planning_approval",
            commit_interface["modes"]["workflow"]["entry_precondition_ids"],
        ...
        for path in active_sources:
            ...
            self.assertIn("compact owner-result `decision_trail`", normalized, path)
    ...
    def test_ai_first_owner_contracts_do_not_persist_authorization_or_routine_handoffs(
        ...
        self.assertEqual(len(owner_ids), 15)
        forbidden_keys = {
            "agent_assignment",
            "authorization",
    ...
    def test_public_docs_report_current_finalization_package_state(self) -> None:
        ...
        for path in readmes:
            ...
            self.assertRegex(
                text,
                r"(?:15 Skills\s*/\s*54(?: package)? exits|15/54|十五个 active packages 共声明\s*54)",
            )
            self.assertRegex(
                text,
                r"(?:14/52/31|14 invokes?\s*/\s*52 exits?\s*/\s*31 targets?)",
            )
        ...
        for path in durable_docs:
            text = path.read_text(encoding="utf-8")
            self.assertIn("fifteen active", text, path)
    ...
    def test_finalizer_durable_docs_distinguish_package_and_global_closure(self) -> None:
        ...
        for path in durable_docs:
            with self.subTest(path=path):
                text = path.read_text(encoding="utf-8")
                normalized = " ".join(text.split())
                self.assertIn("fifteen active Skills and 54 external exits", normalized)
                self.assertIn(
                    "nine target-owned `skill_input_authoring_seed` handoffs",
                    normalized,
                )
                self.assertIn("14 invokes, 52 exits, and 31 targets", normalized)
    ...
    def test_finalizer_transaction_and_legacy_schema_match_installed(self) -> None:
        ...
        self.assertEqual(
            private_artifacts["finalization_transaction"]["persistence"],
            "ignored_runtime",
        )
        self.assertEqual(
        ...
        transaction_path = (
            SKILLS_ROOT
            / "packages/guru-finalize-task/schemas/finalization-transaction.schema.json"
        )
        transaction_installed_paths = [
            REPO
            / ".trellis/guru-team/skills/packages/guru-finalize-task/schemas/finalization-transaction.schema.json",
            REPO / ".agents/skills/guru-finalize-task/schemas/finalization-transaction.schema.json",
            REPO / ".codex/skills/guru-finalize-task/schemas/finalization-transaction.schema.json",
            REPO / ".claude/skills/guru-finalize-task/schemas/finalization-transaction.schema.json",
            REPO / ".cursor/skills/guru-finalize-task/schemas/finalization-transaction.schema.json",
        ]
        transaction_bytes = transaction_path.read_bytes()
        Draft202012Validator.check_schema(json.loads(transaction_bytes))
        for path in transaction_installed_paths:
            self.assertEqual(path.read_bytes(), transaction_bytes, path)

        legacy_package_source = (
            SKILLS_ROOT / "packages/guru-finalize-task/schemas/closeout-plan.schema.json"
        )
        legacy_package_bytes = legacy_package_source.read_bytes()
        for path in [
            REPO / ".trellis/guru-team/skills/packages/guru-finalize-task/schemas/closeout-plan.schema.json",
            ...
            self.assertEqual(path.read_bytes(), legacy_package_bytes, path)

        legacy_runtime_source = (
            REPO / "trellis/workflows/guru-team/schemas/closeout-plan.schema.json"
        )
        legacy_runtime_installed = (
            REPO / ".trellis/guru-team/schemas/closeout-plan.schema.json"
        )
        self.assertEqual(
            legacy_runtime_installed.read_bytes(),
            legacy_runtime_source.read_bytes(),
            legacy_runtime_installed,
        )
    ...
    def test_public_readmes_match_canonical_extension_version(self) -> None:
        ...
        top_level_readme = (REPO / "README.md").read_text(encoding="utf-8")
        self.assertEqual(
            re.findall(r"(?<!v)\b\d+\.\d+\.\d+-guru\.\d+\b", top_level_readme),
            [],
        )
    ...
    def test_contract_discovery_returns_only_current_interface_contracts(self) -> None:
        action = runtime.build_skill_contract_discovery(self.root, "guru-example-action")
    ...
    def test_contract_discovery_validation_failures_use_distinct_stable_codes(self) -> None:
        ...
        action["interface_schema_id"] = "guru-team-skill-interface-unknown"
    ...
    def test_registry_interface_schema_mismatch_fails_closed(self) -> None:
        registry = self.read_registry()
        action = next(item for item in registry["skills"] if item["id"] == "guru-example-action")
        action["interface_schema_id"] = "guru-team-skill-interface-unknown"
        self.write_registry(registry)
        self.assertTrue(any(
            "unknown interface_schema_id" in item
    ...
    def test_extension_registry_contract_mismatch_fails(self) -> None:
    ...
    def test_semantically_loose_current_interface_schema_fails_identity_check(self) -> None:
        schema = self.root / "schemas/skill-interface-1.3.schema.json"
    ...
    def test_registry_schema_identity_and_constraint_matrix_fail(self) -> None:
        schema = self.root / "schemas/skill-registry-1.4.schema.json"
        schema.write_text(json.dumps({"type": "string"}), encoding="utf-8")
        ...
        shutil.copyfile(SKILLS_ROOT / "schemas/skill-registry-1.4.schema.json", schema)
    ...
    def test_eval_corpus_discovery_contract(self) -> None:
        ...
        with self.assertRaises(runtime.WorkflowError) as unknown:
            runtime.build_skill_eval_discovery(self.root, "guru-missing-skill")
        self.assertEqual(unknown.exception.payload["code"], "unknown_skill")
...
class ProductionDistributionTests(unittest.TestCase):
    def test_active_packages_install_to_all_selected_roots(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(result["status"], "ok")
            self.assertEqual(result["active_ids"], ["guru-approve-task-plan", "guru-check-task", "guru-clarify-requirements", "guru-create-task-commit", "guru-create-task-workspace", "guru-discover-change-context", "guru-finalize-task", "guru-merge-task-pr", "guru-review-branch", "guru-review-change-request", "guru-review-contract-wording", "guru-review-task-publication", "guru-select-workflow-mode", "guru-sync-base", "guru-verify-extension-installation"])
            for root in (".agents", ".codex", ".cursor", ".claude"):
                ...
                normalized_clarification = " ".join(clarification_text.split())
                self.assertIn("compact owner-result `decision_trail`", normalized_clarification)
                self.assertIn("closed scope-only 2.0 document", normalized_clarification)
                self.assertIn("reject any schema mismatch before normalization", normalized_clarification)
                ...
                self.assertNotIn("scope_decisions[]", clarification_text)
...
class IntakePublicInvocationTests(unittest.TestCase):
    ...
    def test_source_verifier_shared_adapter_uses_source_checkout(self) -> None:
        run_root = Path(self.temp.name) / "source-verifier-eval"
        process = subprocess.run(
            [
                str(
                    REPO
                ...
                "--adapter", "shared",
                "--case", "source-repository-verified",
                "--run-root", str(run_root),
                "--json",
            ],
            cwd=REPO,
        ...
        result = json.loads(process.stdout)
        self.assertEqual(result["status"], "passed", result)
        self.assertEqual(result["cases"][0]["actual_exit"], "verified")

    def test_verifier_has_no_finalizer_projection_or_not_required_exit(self) -> None:
        interface = json.loads((
            self.repo
        ...
        self.assertTrue(interface["public_contracts"]["projections"])
        self.assertTrue(interface["public_contracts"]["consumer_inputs"])
        self.assertTrue(all(
            item["consumer"]["kind"] == "stop"
            for item in interface["public_contracts"]["consumer_inputs"]
        ))
        current_contract = {
            "modes": interface["modes"],
            "external_exits": interface["external_exits"],
            "input": interface["public_contracts"]["input"],
            "outputs": interface["public_contracts"]["outputs"],
            "consumer_inputs": interface["public_contracts"]["consumer_inputs"],
            "projections": interface["public_contracts"]["projections"],
        }
        serialized = json.dumps(current_contract, sort_keys=True)
        self.assertNotIn("not_required", serialized)
        self.assertNotIn("guru-finalize-task", serialized)
        self.assertTrue(all(
            item["id"].startswith("legacy_")
            for item in interface["schemas"]
            if "not_required" in item["id"]
        ))
