...
class ArchitectureBaselineContractTest(unittest.TestCase):
    ...
    def project_check(self, public_input, *, blocking=True, status="pass"):
        check = {
            "schema_version": "2.0",
    ...
    def test_no_impact_is_lightweight_and_creates_no_contribution_or_adr(self):
        ...
        self.assertNotIn("contribution_locator", json.loads(result.stdout))
    ...
    def test_task_locator_is_bound_and_bootstrap_current_uses_caller_identity(self):
    ...
    def test_branch_review_requires_independent_committed_full_diff(self):
        ...
        selected["review"] = {
            "status": "reviewed",
            "independent": True,
            "committed_diff_identity": "origin/main...abcdef",
        }
        result = self.invoke(public_input, selected)
        self.assertEqual(result.returncode, 0, result.stderr)
    ...
    def test_promotion_required_needs_current_reviewed_contribution_and_successor(self):
        ...
        selected = self.architecture_selection(
            ...
            review={
                "status": "reviewed",
                "independent": True,
                "committed_diff_identity": "origin/main...0123456789abcdef",
            },
        )
        result = self.invoke(public_input, selected)
        self.assertEqual(result.returncode, 0, result.stderr)
    ...
    def test_review_and_project_check_conditions_are_closed_and_duplicated_consistently(self):
        ...
        pending_review = {"status": "pending", "independent": False, "committed_diff_identity": ""}
        self.assert_schema_valid(semantic["properties"]["review"], pending_review)
        invalid_pending = {"status": "pending", "independent": True, "committed_diff_identity": "origin/main...head"}
        self.assert_schema_invalid(semantic["properties"]["review"], invalid_pending)
    ...
    def test_contribution_closes_adr_promotion_and_review_conditions(self):
        ...
        reviewed_promoted["review"] = {
            "status": "reviewed",
            "independent": True,
            "committed_diff_identity": "origin/main...reviewed-head",
        }
