...
def test_cleanup_requires_confirmation_and_current_finish_receipt(tmp_path):
    ...
    public={"profile":"finish_success","source_exit":"success","mode":"standalone","task_ref":".trellis/tasks/demo","archive_ref":".trellis/tasks/archive/2026-09/demo","finish_ref":FINISH_REF,"resources":[{"kind":"runtime","locator":".trellis/.runtime/guru-team/demo.json"}]}
    ...
    pending=json.loads(subprocess.run(command,text=True,capture_output=True,env=env,check=True).stdout); assert pending=={"exit_id":"remaining_resources","task_ref":".trellis/tasks/demo","archive_ref":".trellis/tasks/archive/2026-09/demo","finish_ref":FINISH_REF} and runtime.exists()
...
def test_cleanup_rejects_unbound_or_current_worktree(tmp_path):
    ...
    public={"profile":"finish_success","source_exit":"success","mode":"standalone","task_ref":".trellis/tasks/demo","archive_ref":".trellis/tasks/archive/2026-09/demo","finish_ref":FINISH_REF,"resources":[{"kind":"worktree","locator":str(repo)}]}; semantic={"profile":"finish_success","mode":"standalone","owned_resources":public["resources"],"route":{"typed_exit":"cleaned"}}
