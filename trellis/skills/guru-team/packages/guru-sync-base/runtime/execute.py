...
from common import authority_checkout, clean, digest, git, head, repo, resolution
...
def run(package_root, command, argv):
    ...
    base=before["selected_base"]; remote=before["remote"]; authority=authority_checkout(root,base); local=f"refs/heads/{base}"; tracking=f"refs/remotes/{remote}/{base}"; local_before=head(authority,local)
    git(authority,"fetch","--no-tags",remote,f"refs/heads/{base}:{tracking}"); remote_after=head(authority,tracking)
    authority=authority_checkout(root,base)
    if local_before!=remote_after:
        if subprocess.run(["git","merge-base","--is-ancestor",local,tracking],cwd=authority,stdout=subprocess.PIPE,stderr=subprocess.PIPE).returncode:
            raise CommandError("git_failed","repository.base","The selected local base cannot fast-forward to the fetched remote base.",4)
        git(authority,"merge","--ff-only",tracking)
    authority=authority_checkout(root,base); after=head(authority)
    if not clean(authority) or after!=remote_after: raise CommandError("stale_identity","repository.head","Rerun from current clean Git state.",3)
