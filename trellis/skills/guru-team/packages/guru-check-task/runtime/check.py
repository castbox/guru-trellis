...
from common import checkpoint,content_identity,dirty_paths,git,load,parse,rel,root,task,validate_candidate_classifications
from runtime.io import CommandError
from runtime.schema import validate_json
def run(package_root:Path,command:dict,argv:list[str])->dict:
 p=argparse.ArgumentParser(add_help=False);p.add_argument("--root");p.add_argument("--task")
 a=parse(p,argv);repo=root(package_root,a.root);td=task(repo,a.task);path=checkpoint(repo,td,"phase2-check.json");value=load(repo,package_root,str(path),"artifact");validate_json(value,package_root/"schemas/phase2-check.schema.json","artifact");validate_candidate_classifications(value)
