...
from common import checkpoint,content_identity,dirty_paths,git,load,parse,rel,root,store,task,validate_candidate_classifications
...
def run(package_root:Path,command:dict,argv:list[str])->dict:
 ...
 if auth.get("schema_version")=="5.0":value=auth
  ...
  value={"schema_version":"5.0","skill_id":"guru-check-task","task_ref":rel(repo,td),"phase2_capture_commit":git(repo,"rev-parse","HEAD"),"reviewed_content_sha256":content_identity(repo),**auth,"reviewed_paths":paths}
 validate_json(value,package_root/"schemas/phase2-check.schema.json","input");validate_candidate_classifications(value);path=checkpoint(repo,td,"phase2-check.json")
