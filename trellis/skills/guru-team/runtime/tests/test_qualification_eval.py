...
class QualificationEvalTests(unittest.TestCase):
    ...
    def test_five_fresh_requests_hide_expected_classification(self) -> None:
        ...
        self.assertEqual(result["cases"][0]["passed_invocations"], 5)
        self.assertTrue(
            all(
...
head = subprocess.run(
    ["git", "rev-parse", "HEAD"], cwd=protocol["owner_repository"], text=True,
...
candidate_result = {
    "candidate_ref": candidate_ref,
    "decision": decision,
    "reason": "Fresh fake-Codex fixture authored the declared semantic decision after reading installed authority.",
    "witness": {
...
""",
        ...
        for adapter_request, adapter_response, wrapper_call in zip(
            ...
            for payload in (adapter_request, native_request):
                ...
                self.assertNotIn("input_profile_id", payload)
