...
派生日期：2026-09-10；source：`trellis/skills/guru-team/registry.json` 与各 package `interface.json`；source baseline：containing Git object + inherited immutable `.47` authority + reviewed #392 contribution；精确 revision 由包含本 authority 的 Git commit/tree identity 绑定，正文不自引用可变 HEAD；provenance：`code_recovered` + inherited reviewed authority + #392 current release mapping and serialized promotion contract。
...
- Architecture current authority 是 `docs/architecture/README.md` / `current-main-0.6.5-guru.48` / `active`；设计宪法是 `docs/architecture/00-foundation/design-constitution.md` / `guru-trellis-design-constitution-v1` / `current`。
...
## #392 current release capability boundary
