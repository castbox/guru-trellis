...
完整继承 immutable `.59` 业务合同，当前增量为 #454 C3 checkout acquisition/live resolution substrate；当前 registry 保持 32 packages / 142 exits / 102 commands并增加一个 planned ID，production workflow 保持 22 mandatory invokes / 98 exits。

`.60` 完整继承 `.59`，吸收 reviewed #454 C3 contribution；RDT 与 Architecture current 均为 `.60/active`。本次只提升已验证的 C3 slice；promotion-created diff 必须重新通过 fresh Phase 2、Task Commit、完整 Branch Review 后才能进入 Publication。
...
- `D454-05`：`LifecycleContractError` 提供稳定 code/field/remediation/details；runtime 不写 task/session/mapping，不导入 Fork private code，不修复 metadata，不选择 semantic route，也不持久化 validation result。
