...
完整继承 immutable `.55` 业务合同，当前增量为 #436；旧 pin、release mapping、计数及 promotion/evidence 叙述只绑定其明确历史版本。
...
当前 inventory 完整继承 `.55` 并增加 reviewed #436 contribution；source 为 live registry/interfaces/commands 与 containing Git object/tree identity。五个 lifecycle packages 为 active/deferred，production workflow graph 尚未由 #434 激活。
...
| `guru-review-branch` | `archived_review_passed`, `passed`, `continuity_passed`, `implementation_required`, `scope_confirmation_required`, `blocked` |
| `guru-review-task-publication` | `archived_ready`, `ready`, `return_to_task_work`, `blocked` |
...
| `guru-merge-task-pr` | `review_refresh_required`, `merged`, `phase2_reentry_required`, `merge_blocked`, `closure_mismatch` |
...
- Registry 标记 26 个 package 为 `active`；22 个 workflow-integrated，三个 Delivery packages 为 `deferred`，`guru-verify-extension-installation` 为 `standalone_only`；合计 114 个 external exits / 96 commands；业务 workflow 保持 22 invokes / 98 exits。
