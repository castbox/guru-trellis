...
## #247 Issue Scope Ledger retirement design

- `D247-01`：Workspace owner 删除 ledger authoring、artifact declaration、writer、checker reader、
  schema/example/eval/test；created output只保留 official task identity与唯一 consumer必需数据。
- `D247-02`：Clarification、Planning、qualification、Phase 2、Commit、Branch Review各自 fresh读取
  current requirement、planning、source、Git与diff，不传递 `scope_ledger_path`、`primary_issue` 或
  Issue classification arrays。
- `D247-03`：Publication owner从 current requirement、reviewed full diff、validation、target/default
...
需求主定义为 [R408-01..08](../../../requirements/versions/current-main-0.6.17-guru.52/requirement-main.md)；
...
- `D408-03` Stop/manual boundary：canonical workflow 拥有自动失败停止和独立手动请求的全局合同，
  ...
  能力并验证该项结果；不增加 Skill/exit/executor。D247-03..05 只定义 Guru lifecycle 的 closure
  责任；独立手动操作既不消费为 Guru gate，也不补造 Finalizer/archive/recovery state。
...
完整旧 lifecycle、23/97/78、developer/ledger retired-zero 和 ledger-free recovery 继承 `.50`。
