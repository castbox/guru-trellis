...
派生日期：2026-09-05；source：`trellis/skills/guru-team/registry.json` 与各 package `interface.json`；source baseline：containing Git object + inherited `.43` authority + reviewed #240/#348 contributions + #332 promotion；精确 revision 由包含本 authority 的 Git commit/tree identity 绑定，正文不自引用可变 HEAD；provenance：`code_recovered` + #260 exact matrix evidence + #283 architecture contract evidence + #290/#295 reviewed evidence + #240/#348 merged review evidence + #332 current release authority。
...
- Registry 标记 23 个 package 为 `active`；22 个 workflow-integrated，`guru-verify-extension-installation` 为 `standalone_only`；合计 97 个 external exits / 81 commands。
...
- Architecture current authority 是 `docs/architecture/README.md` / `current-main-0.6.5-guru.44` / `active`；设计宪法是 `docs/architecture/00-foundation/design-constitution.md` / `guru-trellis-design-constitution-v1` / `current`。
...
- extension candidate 为 `0.6.15-guru.40`；`.44` 是 RDT/Architecture knowledge identity，不构成已发布
...
`.44` 在 `.43` 上建立 current release authority projection，并消费已进入 `main`、已独立审查且随本次
serialized promotion 提升的 #240/#348 authority，使 current graph 与 live 23 public Skills / 97 external exits /
81 commands 一致。新增 current owners 为 `guru-qualify-solution-mechanism` 与
`guru-restore-archived-task`；它们的 implementation/public interfaces 已分别由 PR #346/#351 合入，
本 promotion 不重新实现这些能力，也不改变 Evolution target。
