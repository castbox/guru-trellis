...
完整继承 immutable `.56` 业务合同，当前增量为 #443；旧 pin、release mapping、计数及 promotion/evidence 叙述只绑定其明确历史版本。
...
当前 inventory 完整继承 `.56` 并增加 reviewed #443 contribution；source 为 live registry/interfaces/commands 与 containing Git object/tree identity。`guru-bind-task-session` 为 active/deferred，production workflow graph 尚未由 #434 激活。
...
- live registry 的支持平台声明包含 `shared`、`codex`、`cursor`、`claude`、`opencode`；canonical/installed/platform package bytes 由 preset inventory 与专项 distribution tests 校验。#443 pre-promotion evidence只覆盖当时的Shared/Codex/Cursor/Claude projection，OpenCode由当前#452 combined diff独立验证。
