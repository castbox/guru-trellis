...
派生日期：2026-09-06；source：`trellis/skills/guru-team/registry.json` 与各 package `interface.json`；source baseline：containing Git object + inherited `.44` authority + reviewed #332 original-entry correction contribution；精确 revision 由包含本 authority 的 Git commit/tree identity 绑定，正文不自引用可变 HEAD；provenance：`code_recovered` + #260 exact matrix evidence + #283 architecture contract evidence + #290/#295 reviewed evidence + #240/#348 merged review evidence + #332 current release and wrapper convergence authority。
...
- Registry 标记 23 个 package 为 `active`；22 个 workflow-integrated，`guru-verify-extension-installation` 为 `standalone_only`；合计 97 个 external exits / 77 commands。
...
- Architecture current authority 是 `docs/architecture/README.md` / `current-main-0.6.5-guru.45` / `active`；设计宪法是 `docs/architecture/00-foundation/design-constitution.md` / `guru-trellis-design-constitution-v1` / `current`。
...
- extension candidate 为 `0.6.15-guru.40`；`.45` 是 RDT/Architecture knowledge identity，不构成已发布
...
wrapper 是正向回归样本；current graph 与 live 23 public Skills / 97 external exits / 77 commands 一致。
