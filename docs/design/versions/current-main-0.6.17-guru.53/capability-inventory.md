...
当前 inventory 完整继承 `.52` 并增加 reviewed #418 contribution；source 为 live registry/interfaces/commands 与 containing Git object/tree identity。#408 的无 graph 增量为继承历史，#418 增量如下。
...
- Registry 标记 23 个 package 为 `active`；22 个 workflow-integrated，`guru-verify-extension-installation` 为 `standalone_only`；合计 100 个 external exits / 78 commands；业务 workflow 为 22 invokes / 98 exits。
...
当前总数为 23 Skills / 100 exits / 78 commands；业务图为 22 invokes / 98 exits。
原 Merge 四出口、Finalizer 四个普通 profile 和 #247 ledger-free/Restore 保持。
