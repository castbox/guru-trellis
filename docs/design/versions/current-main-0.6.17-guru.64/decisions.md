...
- `DDEC-046`（accepted）：35 DTO catalog 与 identity/source/result runtime 是 C2 的单一 package-neutral substrate；TaskId、TaskRef、generation 与 source/Delivery relations 使用 closed schema/runtime value domain。
- `DDEC-047`（accepted）：Fork official task/session primitives 独占 framework identity/persistence，Guru substrate 不创建 second store、identity index、workspace mapping reader、alias、dual-read 或 dual-write。
- `DDEC-048`（accepted）：D0 将 pre-review committed reconcile、exact Task Commit candidate、full Branch Review 与 bounded continuity 分为不同 owner；continuity 不得用于首次 review，base pair 不成为 durable identity。
- `DDEC-049`（accepted）：#454 C2+D0 以 expected `.58` promotion 为 `.59` current，但保持 package/runtime inactive boundary；C3-C7、D443、D436 与 E434 未完成，registry 32/142/102 与 production 22/98 不变。
- `DDEC-050`（accepted）：C3 checkout DTO/runtime 使用 call-local facts 与单一 live validator；candidate conflict、selection、adopt/provision、rollback/recovery 均为 closed behavior，不引入 durable checkout authority或第二 error vocabulary。
- `DDEC-051`（accepted）：planned Skill ID 与 active package 分离。C3 只预留 `guru-ensure-task-checkout` metadata；E434 必须在一个 activation boundary 中创建完整 package、selector、workflow、graph 与 installed/platform projection。
- `DDEC-052`（accepted）：C3 output-loss recovery使用 owner-private Git-admin marker补足不可重建的transaction origin；该marker只服务单一acquisition owner，不成为task/session/branch/resource authority或兼容层。
- `DDEC-053`（accepted）：#454 C4 复用 `ADR-015` 与 C2/C3 substrate，在 Git common-dir 建立唯一 TaskBranchBinding writer，并通过窄 OwnershipPort 与 C5 current ownership 对齐。Epoch/revision 与 reviewed expected HEAD 是 mutation/recovery freshness authority；candidate label 不是。两个新 owner IDs 保持 planned，E434 独占 package/selector/workflow/installed/platform activation。
- `DDEC-054`（accepted）：C5 official session adapter 不复制 store，唯一 resource ledger 实现 C4 窄 port 与 Finish/Cleanup responsibility；active missing 以 caller-owned 保守恢复，terminal missing 人工选择。同一 remote incarnation 只按 ancestry 推进，不引入 strict selector、第二 store 或 ledger-wide token；E434 独占 planned ID 激活。
## #454 C6/C7 `.64` Decision

`DDEC-055`（accepted）：以 reviewed C6/C7 contribution 和 expected `.63` 为 source，将非激活
task creation/activation input 与 C7 subtraction 提升为 `.64` current。保留 Fixed Fork task/session、C3
checkout、C4 branch、C5 resource 的单一 writer；六个 IDs 仅 planned，旧生产图直到 E434 原子切换仍有效。
`ADR-015` 足以承接 identity/extension boundary，本次不新增 ADR。promotion-created diff 仍需 fresh gates。
