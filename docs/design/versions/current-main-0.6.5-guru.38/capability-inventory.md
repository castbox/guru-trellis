...
派生日期：2026-08-21；source：`trellis/skills/guru-team/registry.json` 与各 package `interface.json`；source baseline：`2d34abfc…` + #283 architecture convergence task delta（精确 revision 为当前 Git HEAD）；provenance：`code_recovered` + #260 exact matrix evidence + #283 reviewed contract/installation evidence。
...
- Architecture current authority 是 `docs/architecture/README.md` / `current-main-0.6.5-guru.38` / `active`；设计宪法是 `docs/architecture/00-foundation/design-constitution.md` / `guru-trellis-design-constitution-v1` / `current`。
...
- extension candidate 仍为 `0.6.5-guru.37`；`.38` 是 RDT/Architecture knowledge identity，不构成 version bump、tag 或 Release。
