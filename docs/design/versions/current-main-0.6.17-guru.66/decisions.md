...
## #454 D443 `.65` Decision

`DDEC-056`（accepted）：以 reviewed D443 contribution 与 immutable `.64` 为 source，将 path-free Bind major
提升为非激活 current package。C3-C5 substrate 与 Fixed Fork schema-2 adapter 持有任务/checkout/branch/
ledger/session 事实；Bind 只拥有 route 判断与 pointer 写入。五个成功出口保留，新增 explicit mode，blocked
投影 ReasonDTO。E434 独占 active selector、production router 和 installed/platform 原子切换。

## #454 D436 `.66` Decision

`DDEC-057`（accepted）：五个 canonical terminal lifecycle package major 只在 source 层完成 Completion、
Closure、Finish、Cleanup、Reactivate 的单写组合；Closure transaction 与 Finish seal 分别保持 Issue disposition
及 archive/resource authority。Finish 的 cleanup head 是 PR head，不是目标 merge SHA；后者只验证 archive。
`ADR-015` 继续拥有 identity/extension boundary，E434/#434 独占 active selector、graph 和 installed/platform
切换，`.66` promotion 不构成 Release proof。
