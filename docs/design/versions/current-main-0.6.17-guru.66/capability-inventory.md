...
- `guru-establish-task-identity` 仅加入 planned registry metadata；没有 package/interface/command/active
  ...
  production 仍为 22 invokes / 98 exits；C6-C7、D443、D436、E434 与 Release matrix 未完成。（此行为 `.63` C5 历史状态。）

## #454 C6/C7 `.64` Current Inventory Delta

- canonical shared creation/activation input schemas 与 C6 composition runtime 已交付，C7 零新 mapping/path
  read/write 证据进入 Architecture/RDT current；这不是可调用的 task creation package。
- `guru-create-task`、`guru-establish-task-identity`、`guru-establish-task-branch-binding`、
  `guru-ensure-task-checkout`、`guru-rebind-task-branch`、`guru-activate-task` 共六个 ID 仅 planned。
- active package/exit/command 保持 `32/142/102`；production workflow `22/98`，installed/platform
  projection 与旧 predecessor 未切换。D443/D436/E434 与 #434 activation 仍为后续边界。

## #454 D443 `.65` Current Inventory Delta

- `guru-bind-task-session` canonical source 已迁移 TaskId/generation、official session adapter、六条成功出口
  （含 explicit_task_mode）及 ReasonDTO blocked；五个原出口 ID 不变。
- active registry 仍指向 predecessor major，32 packages / 142 exits / 102 commands 与六个 planned IDs 不变。
  Production 仍为 22 invokes / 98 exits。D436/E434、installed/platform 和 #434 activation 未执行。

## #454 D436 `.66` Current Inventory Delta

- `guru-review-task-completion`、`guru-complete-task-closure`、`guru-finish-task`、
  `guru-cleanup-task-resources`、`guru-reactivate-task` 的 canonical source majors 承接 current
  TaskId/generation、source、scope、binding、ledger 和 Result/Transaction/Seal refs。
- Cleanup 增加 manual 与 machine-handoff profiles；Finish 增加 Closure refresh/manual cleanup exits；
  Reactivate 增加 session/recovery/source routes。新增 source exits 不是 active selector 或 production edge。
- registry 仍指向 predecessor：32 packages / 142 exits / 102 commands + 六个 planned IDs，production
  仍为 22 invokes / 98 exits。E434/#434 才安装新的 manifest、workflow 和平台投影。
