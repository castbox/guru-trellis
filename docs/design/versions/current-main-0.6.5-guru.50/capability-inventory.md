...
派生日期：2026-09-13；source：`trellis/skills/guru-team/registry.json` 与各 package `interface.json`；source baseline：containing Git object + inherited immutable `.49` authority + reviewed #247 contribution；精确 revision 由包含本 authority 的 Git commit/tree identity 绑定，正文不自引用可变 HEAD；provenance：`code_recovered` + inherited reviewed authority + #247 Issue reference/closure ownership contract。
...
| `guru-merge-task-pr` | `merged`, `phase2_reentry_required`, `merge_blocked`, `closure_mismatch` |
...
- Registry 标记 23 个 package 为 `active`；22 个 workflow-integrated，`guru-verify-extension-installation` 为 `standalone_only`；合计 97 个 external exits / 78 commands。
...
- Architecture current authority 是 `docs/architecture/README.md` / `current-main-0.6.5-guru.50` / `active`；设计宪法是 `docs/architecture/00-foundation/design-constitution.md` / `guru-trellis-design-constitution-v1` / `current`。
...
`release-guru-trellis-version` 不属于上表 23 个 public Skill，也不改变其 97 个 external exits。
...
- framework source 固定为 `castbox/Trellis@a2003296b4c4ce46c50d72ead3b2ec9c317f69fc`，
