...
- Architecture current authority 是 `docs/architecture/README.md` / `current-main-0.6.5-guru.41` / `active`；设计宪法是 `docs/architecture/00-foundation/design-constitution.md` / `guru-trellis-design-constitution-v1` / `current`。
...
- extension candidate 仍为 `0.6.5-guru.37`；`.41` 是 RDT/Architecture knowledge identity，不构成 version bump、tag 或 Release。
