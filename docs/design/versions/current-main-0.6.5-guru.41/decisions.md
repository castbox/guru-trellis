...
- `DDEC-014`（accepted）：`.41` 是 current knowledge identity，extension candidate 继续为 `0.6.5-guru.37`；knowledge promotion 不隐式授权 version bump、push 或 release。
...
- `DDEC-016`（accepted）：Finalizer 使用独立 target/source checkout 与 closed mode binding，删除
