...
完整继承 immutable `.60` 业务合同，当前增量为 #454 C3 checkout acquisition provenance；旧 pin、release mapping、计数及 promotion/evidence 叙述只绑定其明确历史版本。
...
当前 inventory 完整继承 `.60` 并增加 reviewed #454 C3 provenance contribution；registry/interface/command cardinality不变。`guru-bind-task-session` 为 active/deferred，`guru-ensure-task-checkout` 为 planned ID，production workflow graph 尚未由 #434 激活。
