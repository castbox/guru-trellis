...
派生日期：2026-08-21；source：`trellis/skills/guru-team/registry.json` 与各 package `interface.json`；source baseline：task head `d4165f26…` + #290 serialized promotion delta（精确 revision 为当前 Git HEAD）；provenance：`code_recovered` + #260 exact matrix evidence + #283 architecture contract evidence + #290 reviewed base-routing evidence。
...
- Architecture current authority 是 `docs/architecture/README.md` / `current-main-0.6.5-guru.39` / `active`；设计宪法是 `docs/architecture/00-foundation/design-constitution.md` / `guru-trellis-design-constitution-v1` / `current`。
