...
派生日期：2026-09-09；source：`trellis/skills/guru-team/registry.json` 与各 package `interface.json`；source baseline：containing Git object + inherited `.46` authority + reviewed #378 pinned-fork-runtime contribution；精确 revision 由包含本 authority 的 Git commit/tree identity 绑定，正文不自引用可变 HEAD；provenance：`code_recovered` + #260 exact matrix evidence + #283 architecture contract evidence + #290/#295 reviewed evidence + #240/#348/#332 reviewed authority + #376 current continuity authority。
...
- Architecture current authority 是 `docs/architecture/README.md` / `current-main-0.6.5-guru.47` / `active`；设计宪法是 `docs/architecture/00-foundation/design-constitution.md` / `guru-trellis-design-constitution-v1` / `current`。
...
current source 由 `trellis/presets/guru-team/source/trellis-source.json` 固定到 `castbox/Trellis@ad332e3fe5a19d7274cb03e7c2f3e2128f8de291`，CLI/core `0.6.16`；Guru extension manifest 仍为 `0.6.15-guru.40`。本次不新增公共 Skill/exit/command，继承 23/97/78 graph。来源、session 隔离、source-record 投影与 verifier composition 分别见 D378-01..04。
