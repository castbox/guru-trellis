...
| `guru-reconcile-task-base` / `current_pair`, `base_reentry`, `repair_reentry`, `scope_reentry`, five qualification-return profiles | semantic | current pair=`task_ref,task_head_ref,base_before_ref,base_after_ref,resume_target`, with `resume_target=plan\|implement\|check\|branch_review\|delivery_route\|delivery_readiness`; base re-entry=`task_ref,base_pair_ref`; repair=`continuation_ref,repair_input`; scope re-entry=`task_ref,requirements_result_ref,caller_continuation_ref`; qualification returns use the Section 3 closed shapes. The owner resolves the bound pair plus common-dir/cwd/runtime mapping/original task-worktree-branch identity and classifies per-path ownership/state itself | current-base tracked and path-clean same-task files remain in the original active worktree; source/task-worktree unrelated dirty remains untouched and is not a same-task blocker. Dirty/untracked same-task paths, review/check metadata or any cwd/runtime/worktree/task/branch mismatch remain exact blockers and cannot be bypassed by a source-clean switch. `reconciled` -> `resume_target,base_result_ref` -> `guru-base-resume-router`; `review_continuity_required` -> `base_ref,commit_ref,range_ref` -> `guru-review-branch`; `implementation_required` -> `finding_refs` -> `guru-implement-task`; `planning_required` -> `finding_refs` -> `guru-plan-task:finding_reentry`; `scope_confirmation_required` -> `scope_ref,caller_continuation_ref` -> `guru-clarify-requirements:base_reconciliation_scope`; recoverable `blocked` -> `continuation_ref,repair_input` -> `task-base-reconciliation-blocked` |
| `guru-review-branch` / `full_diff`, `continuity`, `scope_reentry`, `authority_reentry`, `base_reentry`, five qualification-return profiles | semantic | full-diff/continuity=`task_ref,base_ref,commit_ref,range_ref`; scope re-entry=`task_ref,requirements_result_ref,caller_continuation_ref`; authority re-entry=`task_ref,authority_result_ref`; base re-entry=`task_ref,base_result_ref`; qualification returns use the Section 3 closed shapes. The Branch Review owner restores and live-validates its bound committed range | `passed` uses independent closed discriminators `promotion_kind=none\|rdt\|architecture\|rdt_then_architecture` and `projection_kind=none\|authority_only\|with_code_spec`, each computed from outstanding work after comparing every in-range contribution with live authority/projection identities. All schemas require `reviewed_range_ref,promotion_kind,projection_kind`; applicable promotion schemas require exact `rdt_promotion_ref` and/or `architecture_promotion_ref`, each binding contribution content, this range and an expected-current identity that still matches live authority. A contribution already current at its reviewed target identity emits no promotion ref. `projection_kind=none` is valid only with `promotion_kind=none` and means no outstanding authority promotion plus no missing/stale authority locator/usage/freshness or in-range code-spec contribution in the current projection, even when the full diff still contains original already-current contribution/projection changes. `authority_only` forbids the code-spec ref and is valid with any promotion kind, including `promotion_kind=none` when shared authority is current but its locator/usage/freshness projection needs repair. `with_code_spec` is valid with any promotion kind and alone requires a same-range `code_spec_contribution_ref` whose target identity is still missing from the current projection. Authority/contribution/expected-current/projection material drift cannot be classified as none and returns the affected owner. A raw `trellis-update-spec` result is invalid -> `guru-post-review-authority-router`; `implementation_required` -> `finding_refs` -> `guru-implement-task`; `planning_required` -> `finding_refs` -> `guru-plan-task:finding_reentry`; `scope_confirmation_required` -> `scope_ref,caller_continuation_ref` -> `guru-clarify-requirements:branch_review_scope`; `authority_reentry_required` -> `authority_kind,revision_ref` -> `guru-authority-reentry-router`; `base_reconciliation_required` -> `base_pair_ref` -> `guru-reconcile-task-base:base_reentry`; recoverable `blocked` -> `continuation_ref,repair_input` -> `branch-review-blocked` |
...
| `guru-accept-task` / `github_pr`, `none`, `scope_reentry`, `authority_reentry` | semantic | initial profiles=`task_ref,readiness_ref`; the ref fixes route and reviewed-range identity. Scope re-entry=`task_ref,requirements_result_ref,caller_continuation_ref`; authority re-entry=`task_ref,authority_result_ref`; this owner restores and live-validates that readiness | profile-specific `accepted`: both `github_pr` and `none` emit only `acceptance_ref` -> `guru-accepted-route-router`; Acceptance never emits a delivery fact. `implementation_required` -> `finding_refs` -> `guru-implement-task`; `planning_required` -> `finding_refs` -> `guru-plan-task:finding_reentry`; `scope_confirmation_required` -> `scope_ref,caller_continuation_ref` -> `guru-clarify-requirements:task_acceptance_scope`; `authority_reentry_required` -> `authority_kind,revision_ref` -> `guru-authority-reentry-router`; recoverable `blocked` -> `continuation_ref,repair_input` -> `task-acceptance-blocked` |
| `guru-publish-task-pr` / `provenance_prepare`, `provenance_confirmation_reentry`, `provenance_reentry`, `push_prepare`, `push_confirmation_reentry`, `push_provider_reentry`, `pr_create_prepare`, `pr_creation_confirmation_reentry`, `pr_provider_reentry` | semantic | provenance prepare=`task_ref,acceptance_ref`; push prepare=`acceptance_ref,publication_head_ref`; PR-create prepare=`acceptance_ref,published_head_ref`; each confirmation re-entry=`continuation_ref`; each repair/provider re-entry=`continuation_ref,repair_input`. Every continuation fixes exactly one stage and the bound task/acceptance/reviewed-content/publication/remote identity; no input accepts confirmation or authorization state | `provenance_prepare` selects closed `self_hosted\|installed`, resolves separate target/source checkouts, validates immutable source plus allowlisted normalized reviewed-content equality and either emits no-mutation `publication_head_current -> acceptance_ref,publication_head_ref -> push_prepare`, or displays only the metadata-tail preparation and emits `provenance_confirmation_required -> continuation_ref -> task-publication-provenance-confirmation-wait`; refusal is `publication_preparation_not_executed -> task_ref,acceptance_ref -> task-publication-preparation-not-executed`. `push_prepare` emits no-mutation `branch_published -> acceptance_ref,published_head_ref -> pr_create_prepare` when the exact publication head is already remote-current, otherwise it displays only branch push and uses `task-branch-push-confirmation-wait`; refusal is `branch_push_not_executed -> task_ref,acceptance_ref,publication_head_ref -> task-branch-push-not-executed`. `pr_create_prepare` accepts an exact existing Draft or READY base/head PR as no-mutation `draft_pr_current`, otherwise displays only Draft PR creation and uses `task-pr-creation-confirmation-wait`; refusal is `pr_creation_not_executed -> task_ref,acceptance_ref,published_head_ref -> task-pr-creation-not-executed`; success is always `draft_pr_current -> task_ref,acceptance_ref,pr_ref,publication_head_ref -> guru-finalize-task:github_pr`, never direct Merge. The three action continuations/refusals/recoveries are mutually incompatible; every prepare/re-entry first rereads its own live state, recognizes completed work, and never replays metadata tail, push or PR creation. Source/target/lineage/postimage failure uses only `provenance_reentry`; branch divergence uses only `push_provider_reentry`; PR mismatch/provider failure uses only `pr_provider_reentry` |
| `guru-merge-task-pr` / `initial`, `provider_reentry` | semantic | initial=`task_ref,ready_for_merge_ref`; provider re-entry=`continuation_ref,repair_input`. The archive-bound ref fixes PR/base/expected archive head, publication/reviewed-content chain, durable result and closure scope | only after live validation confirms the exact PR is READY, archive head/summary/scope are current and no earlier mutation remains does the owner display the merge plan. Any clear semantic affirmative under `EVO-REQ-081` is sufficient without a fixed `合并PR` phrase; `merged` -> `delivery_fact_ref` binding merge plus durable result -> `guru-resolve-issue-closure:github_pr`; `pr_not_merged` -> `ready_for_merge_ref` -> `task-pr-not-merged`; `merge_recovery_required\|blocked` re-enter only `provider_reentry`. Merge never creates/archive/pushes/marks Ready or consumes an earlier publication/Finish confirmation |
...
`delivery_fact_ref` and `closure_ref|reason_ref` is a closed chained identity; routers never reconstruct provider,
archive or durable facts. `guru-delivery-terminal-router` creates no durable artifact: from the closure exit DTO alone
it validates that route, acceptance, Finish and closure bindings match, projects the consumer-minimal
`delivery_terminal_ref`, and invokes Cleanup directly. Cleanup alone authors its private `resource_plan_ref`; there is
no post-terminal second Finish call.
For `none`, the accepted DTO contains no `delivery_fact_ref`, and the router projects only the accepted task identity
into Finish. A route-local `delivery_fact_ref` has exactly one producer after its delivery fact is current: Merge for
`github_pr`, or successful Finish for `none`; no pre-Finish input or duplicate stage identity is valid.

## 6. Distribution And Stock Skills

| Public Skill / profiles | Mode | Minimal public input | Exit -> minimal output -> unique consumer |
| --- | --- | --- | --- |
| `guru-validate-extension-projection` / `standalone`, `standalone_reentry`, `stock_policy_reentry` | semantic | standalone=`candidate_ref,target_ref`; blocked re-entry=`continuation_ref,repair_input`; stock-policy re-entry=`policy_ref`, whose closed identity binds the same candidate/target and standalone route. This owner resolves the candidate-declared surface and verifier execution boundary itself | `stock_maintenance_required` -> `candidate_ref,target_ref` -> `guru-maintain-stock-projection:standalone`, and only that fixed child edge may perform standalone policy maintenance; `stock_policy_reentry` freshly validates the complete declared surface after stock current. Only then may `projection_current -> candidate_ref -> workflow-completed`. Any pre-matrix/matrix/post-matrix execution failure must first bind schema-valid non-null `failure_ref` containing actual stage/cell/command/exit/bounded credential-safe tail and stdout/stderr hash-size; matrix-external failures use `postcheck_failure`. Temporary cleanup may run only after that binding. Recoverable `projection_validation_blocked -> gate_kind,mismatch_refs,failure_ref,continuation_ref,repair_input -> projection-validation-blocked`; the stop consumes `failure_ref` for current diagnosis but projects only `continuation_ref,repair_input` to `standalone_reentry`. Embedded exits are schema-invalid, no raw output is public/durable, and no fifth top-level distribution action is created |
...
| `guru-maintain-stock-projection` / `standalone`, `standalone_action_reentry`, `standalone_suppression_reentry`, `standalone_provider_reentry` | semantic | standalone=`candidate_ref,target_ref`, with the fixed caller `guru-validate-extension-projection:standalone`; action re-entry=`continuation_ref`; suppression/provider repair=`continuation_ref,repair_input`, with the selected profile fixed by the blocked exit. The stock owner resolves the selected `action_set_ref` and bound candidate/target from its current policy/continuation sections rather than requiring a caller replay | `stock_policy_current` -> `policy_ref`, whose identity binds candidate/target and the fixed standalone projection route -> `guru-validate-extension-projection:stock_policy_reentry`; it never completes the top-level invocation itself. `stock_policy_action_required`/`stock_policy_action_partial`/`stock_policy_action_unknown` each -> `continuation_ref` -> the same Stock owner's action-state resolver. It may perform only non-mutating live resolution; once one exact pending mutation is current, it displays that plan and enters `stock-policy-action-confirmation-wait`. A current clear affirmative alone projects `continuation_ref` to `standalone_action_reentry`; missing confirmation stays in the wait, material drift returns with zero mutation to `standalone_action_reentry` for fresh resolution/redisplay, and refusal emits `stock_policy_action_not_executed` -> `completed_action_refs,pending_action_refs,unknown_action_refs,continuation_ref` -> `stock-policy-action-not-executed`, which may project only `continuation_ref` to `standalone_action_reentry` after a later explicit request to continue. A deterministic action that needs no confirmation may complete before the same re-entry, but an action-state continuation alone never authorizes mutation. Recoverable `upstream_suppression_blocked` -> `asset_refs,continuation_ref,repair_input` -> `stock-suppression-maintenance-blocked`; recoverable `provider_boundary_blocked` -> `asset_refs,continuation_ref,repair_input` -> `stock-provider-maintenance-blocked`; embedded/retained exits are schema-invalid |
...
| `guru-maintain-stock-projection` / `embedded_clean`, `embedded_clean_action_reentry`, `embedded_migration`, `embedded_migration_action_reentry`, `embedded_release`, `embedded_release_action_reentry`, `reapply`, `reapply_action_reentry` | semantic | initial=`candidate_ref,target_ref`; action re-entry=`continuation_ref`; every profile fixes one exact caller, and the stock owner resolves the selected `action_set_ref` from its current policy/continuation section | only `returned_to_caller` with one disjoint schema -> `guru-stock-result-router`: current=`policy_result=current,policy_ref`; all-pending=`policy_result=action_required,action_state_ref,continuation_ref`; completed+pending=`policy_result=action_partial,action_state_ref,continuation_ref`; any-unknown=`policy_result=action_unknown,action_state_ref,continuation_ref`; explicit refusal=`policy_result=action_not_executed,action_state_ref,continuation_ref`; embedded role-local failure=`policy_result=caller_finding,finding_refs`. Fields from every other variant are forbidden. The exact caller's fixed stock-action edge returns an action-required/partial/unknown variant to the same Stock owner for non-mutating state resolution; once one exact pending mutation is current, Stock displays its plan and enters `stock-policy-action-confirmation-wait`. A current clear affirmative alone projects `continuation_ref` to the matching `embedded_*_action_reentry` or `reapply_action_reentry`; missing confirmation stays in the wait, material drift returns with zero mutation to that same profile for fresh resolution/redisplay, and refusal returns `policy_result=action_not_executed` to the exact caller. A deterministic action needing no confirmation may complete before the same re-entry. No action-state continuation directly authorizes mutation, and standalone completion/role-local block/retained exits are schema-invalid, so the caller alone owns its route terminal |
...
| `guru-post-review-authority-router` | Branch Review outstanding `promotion_kind`, outstanding `projection_kind`, exact applicable promotion refs, variant-specific `code_spec_contribution_ref` and `reviewed_range_ref` | first invoke only selected not-yet-current authority `promotion` profiles, ordered RDT then Architecture when both. Only `promotion_kind=none,projection_kind=none` goes to `guru-select-delivery-route`; this exact pair means all in-range authority contributions are already current or absent and no authority locator/usage/freshness or in-range code-spec projection work remains. `authority_only\|with_code_spec` invokes the matching disjoint input of the single `.trellis/spec` owner `guru-bootstrap-repository-ssot:projection_refresh`, selecting/renaming only fields declared by that variant and only after every selected authority, if any, is current. `promotion_kind=none,projection_kind=authority_only` is the closed normal repair route when shared authority is already current but its minimal projection is missing or stale; it carries no promotion or code-spec ref. The resulting `spec_projection_ref` is renamed to `authority_result_ref` -> `guru-check-task:authority_reentry`; Check success proceeds through a new Commit and committed full-diff Branch Review. That fresh review compares the still-complete diff with live current identities, emits exact double-none when prior promotion/projection is already current, and reopens only materially drifted work. The router never reruns current promotion/projection and cannot write shared authority prose |
| `guru-accepted-route-router` | `github_pr` or `none` | `github_pr` -> `guru-publish-task-pr:provenance_prepare`; `none` -> `guru-finalize-task:none` |
| `guru-delivery-terminal-router` | route/acceptance/closure tuple | deterministic `delivery_terminal_ref` projection -> `guru-clean-task-resources:prepare` only |
| `guru-embedded-projection-router` | `clean`, `migration` or `release` caller | that exact distribution caller only |
| `guru-stock-result-router` | exact embedded clean/migration/release caller, or reapply fixed to Existing Migrator's ordered update/preset-reapply cell, plus one disjoint `policy_result` schema | current/finding/refusal -> that exact caller only; action-required/partial/unknown -> that caller's fixed stock-action edge back to the same Stock owner. After non-mutating resolution, only an exact pending mutation plan may enter `stock-policy-action-confirmation-wait`; a current clear affirmative projects only `continuation_ref` to the matching `*_action_reentry`, while missing confirmation waits, refusal returns to the caller and drift re-enters the same profile with zero mutation. It rejects direct continuation-to-mutation authority, fields from another variant, standalone/retained-host output and workflow completion |
...
| `guru-publish-task-pr` | `provenance_blocked` -> `task-publication-preparation-blocked` | `provenance_reentry` / `continuation_ref,repair_input` |
...
terminals/pauses, not hidden Skills. Publish uses three distinct refusal consumers:
`task-publication-preparation-not-executed`, `task-branch-push-not-executed` and
`task-pr-creation-not-executed`; none is a provider block or an alias for another action. A Section 7.1 block retains producer Skill/profile plus its declared repair
...
`release-confirmation-wait`, `stock-policy-action-confirmation-wait`,
`active-lifecycle-cleanup-confirmation-wait`,
`task-publication-provenance-confirmation-wait`,
...
publication provenance, branch push, Draft PR creation, Stock action, Release and the two cleanup waits consume an AI-judged
semantic confirmation for exactly their displayed action;
...
| `provider_context` | exact action/provider caller | Sync; task-free or standard local write; Commit; publication provenance preparation; branch Publish; Draft PR Publish; Finish archive commit/push/Ready; Merge; Closure; clean Install; existing Migration; standalone verifier; Stock/Reapply; Release; controlled adapters, each only under its closed action profile | target/action/contract/live outcome change |
...
| --- | --- | --- | --- |
| Git | `base_sync`, `local_write`, `task_commit`, `publication_tail_commit`, `task_push`, `finish_archive_commit`, `finish_archive_push` | base sync=`selected_base_ref,authority_checkout_ref,base_before_ref,base_after_ref`; local write=`checkout_ref,changed_path_refs,write_result_ref`; ordinary commit=`selected_commit_ref,diff_ref,commit_result_ref`; provenance tail additionally binds `reviewed_content_ref,source_binding_ref,allowlisted_field_refs,normalized_tree_ref,direct_parent_ref`; either push=`remote_ref,expected_head_ref,push_result_ref`; archive commit additionally binds `task_ref,summary_ref,archive_locator,publication_head_ref` | Sync, task-free/implementation, Commit, Publish provenance/branch-push, or Finish archive owner fixed by profile |
| GitHub | `pr_publish_draft`, `pr_ready`, `pr_merge`, `issue_closure_read`, `issue_closure_write` | all require `repository_ref,remote_ref,target_kind,target_identity,remote_state_ref`; Draft/Ready/Merge add their stage-specific `expected_head_ref`; Ready also binds `archive_ref,summary_ref`; closure read adds `closure_result_ref`; closure write adds `close_scope_ref,closure_preimage_ref,closure_result_ref` | exact Publish, Finish, Merge, or Closure owner fixed by profile |
| Standalone verifier | `pre_matrix`, `matrix_cell`, `post_matrix` | `candidate_ref,target_ref,actual_stage,matrix_cell,command_label,exit_code,failure_class,bounded_safe_tail,stdout_hash,stdout_size,stderr_hash,stderr_size`; matrix-external failures require `failure_class=postcheck_failure` | `guru-validate-extension-projection:standalone\|standalone_reentry`; failure binding precedes temporary cleanup and embedded callers cannot select these profiles |
...
1. Every external exit above has exactly one named consumer/router/stop; routers validate profile and perform only
   ...
   `guru-implement-task:initial` as its sole consumer. Publication provenance preparation, branch push, Draft PR,
   Finish/archive/Ready, archive-bound Merge, Closure and Cleanup form one ordered chain; no action accepts an earlier
   stage's continuation or confirmation. Stock action-state continuations likewise enter the named Stock confirmation
   wait before any pending mutation and can reach only their profile-fixed action re-entry. The three Publish refusal exits project only their action-local identity to
   their three distinct named zero-side-effect consumers. Standalone verifier failure binds non-null evidence before cleanup and only
   its Projection-owned re-entry consumes the continuation.
...
5. The old `guru-review-task-publication` and standalone `guru-verify-extension-installation` shapes are replaced by
   ...
   behavior is split without capability loss: Publish owns immutable source/target preparation, metadata tail, branch
   push and Draft PR; Finish retains summary/archive/archive-push/Ready; Merge consumes only archive-bound
   `ready_for_merge`; Closure and Cleanup remain separate. Standalone verifier evidence remains Projection-owned and
   is never a Finish input. Migration must prove zero legacy consumer before activation.
