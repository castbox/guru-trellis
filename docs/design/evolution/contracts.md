...
publication provenance, branch push, Draft PR creation, Release and the two cleanup waits consume an AI-judged
...
1. Every external exit above has exactly one named consumer/router/stop; routers validate profile and perform only
   ...
   stage's continuation or confirmation. The three Publish refusal exits project only their action-local identity to
