...
状态：`design_ready_for_delivery_planning`。本文件 closes the Design successor decision for every
`CUR-CAP-001..023` and `TARGET-DELTA-001..013`. Current facts remain owned by
...
| `CUR-CAP-006` | preserve no-mutation current convergence; when an action remains, preserve its exact plan, semantic affirmation and owner-specific zero-side-effect refusal without identity repetition | all semantic Skills that may own a side effect, including Workspace and active disposition cleanup; `EVO-DES-022,030,035..036,041,044..055,069` | `EVO-FIX-NO-ISSUE`, `EVO-FIX-TASK-FREE`, `EVO-FIX-ACTIVE-DISPOSITION`, `EVO-FIX-SEMANTIC-CONFIRMATION` |
...
| `CUR-CAP-012` | preserve exact commit/base reconciliation/full-diff review | Commit/Reconcile/Review; `EVO-DES-030..032` | `EVO-FIX-BRANCH-FINDING`, `EVO-FIX-BASE-EVOLUTION` |
| `CUR-CAP-013` | preserve truthful delivery readiness and two routes | delivery owners with separate branch-push/PR-create/merge actions and already-current no-mutation exits; `EVO-DES-033..035` | `EVO-FIX-FULL-NORMAL`, `EVO-FIX-NONE`, `EVO-FIX-SEMANTIC-CONFIRMATION` |
| `CUR-CAP-014` | preserve expected-head/closure/terminal correctness | push/PR-create/merge/closure owners with stage-local recovery and live completed-state convergence; `EVO-DES-034..036` | `EVO-FIX-PROVIDER-RECOVERY`, `EVO-FIX-FINISH-RECOVERY` |
...
| `CUR-CAP-016` | preserve RDT/Architecture bootstrap, contribution, promotion and minimal spec projection | three repository authority owners plus outstanding-only, already-current-aware Bootstrap `projection_refresh`; `EVO-DES-023..024,031,060..064` | `EVO-FIX-SSOT-BOOTSTRAP`, `EVO-FIX-RDT-LIFECYCLE`, `EVO-FIX-ARCH-PROMOTION` |
| `CUR-CAP-017` | preserve canonical/dogfood/installed/host consistency | projection/install/migration owners; standalone policy maintenance is produced only inside the Projection route and returns to fresh Projection validation; `EVO-DES-043..047,056..059` | `EVO-FIX-PROJECTION`, `EVO-FIX-STOCK-MAINTENANCE`, `EVO-FIX-CLEAN-INSTALL`, `EVO-FIX-MIGRATION` |
| `CUR-CAP-018` | preserve live-derived inventory and separate loss/consistency gates | projection validator; `EVO-DES-043,059` | `EVO-FIX-PROJECTION`, `EVO-FIX-RELEASE` |
| `CUR-CAP-019` | preserve evidence layers and minimal validation ownership | Check/projection/Release owners; `EVO-DES-030,043..048,065..067` | `EVO-FIX-PLAN-NORMAL`, `EVO-FIX-FULL-NORMAL`, `EVO-FIX-RELEASE` |
...
exits` is a `.40` source identity, not a target magic count.
...
| `TARGET-DELTA-007` | target-native distribution, atomic migration and immutable Release replace legacy verifier/finalizer shapes | `EVO-DES-043..048,059`; projection/install/migration/Release fixtures |
...
| Finalizer-owned publication/merge/closure/cleanup aggregate | branch push, PR create, merge, closure, Finish and Cleanup exact action owners | each confirmation/refusal/recovery is stage-local and each failure re-enters only its exact owner |
