...
| `CUR-CAP-013` | preserve truthful delivery readiness, two routes and exact installed platform selection | delivery owners with separate provenance-tail/push/Draft-PR/Finish/archive/Ready/Merge actions and already-current no-mutation exits; installed provenance prepare validates three exact selected lists before source checkout/apply/commit and never expands a subset; none Acceptance carries only acceptance current and enters Finish without a delivery fact; `EVO-DES-033..036,072` | `EVO-FIX-FULL-NORMAL`, `EVO-FIX-NONE`, `EVO-FIX-SEMANTIC-CONFIRMATION`, `EVO-FIX-INSTALLED-PROVENANCE-PUBLICATION` |
...
| `CUR-CAP-017` | preserve canonical/dogfood/installed/host consistency, immutable installed source and exact installed platform set | projection/install/migration/Publish owners; standalone policy maintenance returns to fresh Projection validation, while installed publication separates immutable extension source from reviewed target and always projects repeated exact `--platform`; `EVO-DES-043..047,056..059,072` | `EVO-FIX-PROJECTION`, `EVO-FIX-STOCK-MAINTENANCE`, `EVO-FIX-CLEAN-INSTALL`, `EVO-FIX-MIGRATION`, `EVO-FIX-INSTALLED-PROVENANCE-PUBLICATION` |
