...
状态：`design_ready_for_delivery_planning` / `issue_suggestions_only`。This plan decomposes the reviewed target candidate into serial,
independently reviewable implementation slices. It does not create Issues, activate this task, implement, commit,
publish or Release.
...
| `EVO-DEL-04` implementation and review | `guru-implement-task`, separate platform/channel caller-bound implementation adapters, check workers, Check/Commit/Reconcile/Branch Review, contribution freshness and Bootstrap `projection_refresh` return to fresh Check/Commit/Review | 01,03 | Full Normal through candidate review, Branch Finding, Base Evolution, Gitlink/submodule and downstream freshness targeted fixtures pass; a reviewed code-spec contribution has exactly one `.trellis/spec` writer, only outstanding work is selected, and the fresh complete-diff review converges already-current authority/projection targets to exact double-none before delivery |
| `EVO-DEL-05` delivery, Finish and cleanup | delivery route/readiness/acceptance, separate branch-push and PR-create confirmation/recovery subgraphs, merge, closure, semantic Finish owner with deterministic transitions, semantic Cleanup and durable history | 01,04 | GitHub-provider fake/controlled integration and `none` route, Semantic Confirmation, Provider Recovery and Finish Recovery pass; push and PR create have separate prepare/wait/re-entry/refusal/recovery, exact already-current remote branch/READY PR exits require no mutation confirmation, and push confirmation cannot create a PR; READY merge has no lexical gate; Merge/Closure recovery carries continuation; push/PR-create/merge/closure/Finish/post-Finish Cleanup/disposition-cleanup refusal targets are disjoint; no remote live claim required |
...
| `EVO-DEL-07` projection and clean install | canonical/dogfood/installed generator, preset transaction, reference manifest projection, code-spec projection-contract projection, host setup partition, projection validator and clean installer | 01-06 | Projection plus one representative clean throwaway pass; standalone policy maintenance follows only `Projection -> Stock -> Projection fresh validation` with no fifth distribution action; manifest source precedence, one-writer ownership, reapply/drift and executable/mode checks pass; preset reapply does not perform code-spec promotion/write; full matrix explicitly unverified |
...
| `EVO-DEL-09` cumulative compatibility matrix | execute all declared Codex/Claude/Cursor clean/existing/update/reapply/workflow-switch cells, setup partition and normal A/B lifecycle against the frozen Slice-08 candidate; implementation/contract writes are forbidden | immutable candidate from 01-08 | complete exact matrix passes from that identity; any finding returns to an owning earlier slice and invalidates the matrix rather than being patched or deferred into Release |
...
| provider outcome | exact action owner; branch push and PR create remain separate stages and already-current live state advances without mutation | retry/fallback route chosen by child provider, current action repeated, or push confirmation reused for PR creation |
