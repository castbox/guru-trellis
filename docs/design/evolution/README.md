...
既有 Publish/Finish/Projection responsibilities。Requirements 现为 52 UC / 84 REQ / 34 NFR / 23 current
...
`evolution_refactor_eligible`。#311/#312 merge、`.42`
RDT/Architecture/inventory fresh rebind 与 requirement/normal-path fixture successor 零差集 projection 已同步，
fresh Requirements semantic、Strict technical 与确定性闭包审核已通过；`.42` 相对 `.41` 只增加 `.3/.39/CLI 0.6.15` candidate/target alignment，
...
Design review 前。它不表示 target runtime、`.42` as-built Design（`.41`/`.40` 仅为历史 comparison evidence）、preset、平台投影或
Release 已经改变。
...
本目录与 `docs/design/versions/current-main-0.6.5-guru.42/` 并存是 current/target 文档状态分离，
