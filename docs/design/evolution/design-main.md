...
版本：`evolution-design-candidate-2026-08-30`；状态：`design_ready_for_delivery_planning` /
`fresh_design_review_passed` / `evolution_refactor_eligible`。本文件已从 current
`requirements_ready_for_design` identity 投影
`REQ-REV-011..142`、`EVO-001..007`、`EVO-CAP-001..004`、`EVO-REQ-001..084`、
`EVO-NFR-001..034` 与 `TARGET-DELTA-001..013`。`EVO-DES-070..073` 分别拥有两阶段
Evolution eligibility、#312 original-worktree/base continuity、#311 installed publication terminal
continuity 与 standalone verifier failure evidence；PR #317 exact platform-set preservation 由既有
`EVO-DES-035/072` 承接，不新增 responsibility。它们与既有 `EVO-DES-001..069` 共同构成本轮已审核
planning candidate。该 target Design 不改变 selected-base current `.42` runtime 事实；`.42` 相对
`.41` 的 `.3/.39/CLI 0.6.15` candidate/target alignment 是 fact-only；PR #317 把 latest stable current 修正为
`.2/.38/CLI 0.6.15` 且 `.3/.39` 仍 unverified，这些 authority facts 都不授权实现或发布；`.41`/`.40` 仅为
历史 comparison evidence。PR #318 只刷新 caller-inventory identity、Issue disposition、dogfood provenance
与 archive/merge facts，不改变 target Design、current `.42` behavior 或任何 owner/contract allocation。
...
- `EVO-DES-035`：`guru-accept-task` 是两条 route共享的 pre-delivery semantic acceptance owner，按 route
  ...
  mutation前进入publication owner recovery。installed mode 还必须在 extension source checkout 前读取 parent
  manifest 的 `install.selected_platforms` / `install.all_platforms` / `skill_packages.selected_platforms` /
  `overlays.selected_platforms`；三处 list 必须 non-empty/sorted/unique/canonical/equal。只有
  `all_platforms=true` + full canonical set 映射 `--all-platforms`，其它合法 selection 以 repeated
  `--platform` exact 保留；invalid identity 在 source checkout/preset apply/commit 前进入同一
  `provenance_reentry`，target managed mutation 为 0。缺tail时owner展示且只确认该metadata-only preparation；精确tail
...
- `EVO-DES-060`：本Design与Evolution Test当前是 target contribution candidate；只有完整
  ...
  必须同时读取 selected-base `.42` current 与 target delta，通过 task-local contribution 说明本 slice 的
...
- `EVO-DES-070`：Evolution prerequisite采用两道不可互换的gate。Design入口只消费同一exact selected-base
  ...
  Design owner展开84个Requirement、34个NFR、23个current capability、13个target delta与50个fixture，
...
- `EVO-DES-072`：#311 installed publication preservation由现有owner分层承接而不增加public Skill：
  ...
  direct-parent lineage与exact generated bytes。installed parent/post-reprepare platform set 必须 exact equal，
  subset expansion/loss/reorder 为 0；invalid list/`all_platforms` 必须在 source checkout/apply/commit 前 fail
  closed。self-hosted与installed均可达同一terminal；missing/mutable/
...
The pending candidate may become `design_ready_for_delivery_planning` / `fresh_design_review_passed` only when：

1. 本目录与Evolution Test全稿完成且导航/链接/manifest current；
2. `EVO-REQ-001..084`、`EVO-NFR-001..034`、`CUR-CAP-001..023`、`TARGET-DELTA-001..013`均有
...
3. 50个`EVO-FIX-*`均有owner、candidate kind、evidence layer、blocked/re-entry和验收结果；其中Evolution
   ...
   `EVO-DES-072`，verifier failure命中`EVO-DES-073`；installed platform selection 通过既有
   `EVO-DES-035/072` 命中同一 installed-publication fixture，不新增第 51 个 fixture；
...
The current Design gate has passed against this revised candidate. #311/#312 merge and exact `.42` authority rebind are
current; `origin/main@5650df47…` includes PR #317 platform-selection preservation plus PR #318 fact-only
caller-inventory/provenance correction, and the 84/34/13/50 Requirements-stage successor projection plus fresh
Requirements gates are current. The complete Design/Test/Architecture/task projection preserves 73 responsibilities
and 50 fixture mappings. Pre-`REQ-REV-142` `DES-REV-001..052` remains stale historical evidence; current fresh semantic
review and deterministic closure passed with P1/P2/P3 findings and high-risk open questions all zero. Current status is
`design_ready_for_delivery_planning` / `fresh_design_review_passed` / `evolution_refactor_eligible`. This continuation
remains in Phase 1 and stops after Design gate closure; no implementation or delivery action is authorized.
