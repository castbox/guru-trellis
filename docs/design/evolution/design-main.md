...
- `EVO-DES-035`：`guru-accept-task` 是两条 route共享的 pre-delivery semantic acceptance owner，按 route
  ...
  manifest 的 `install.selected_platforms` / `skill_packages.selected_platforms` /
  `overlays.selected_platforms`；三处 list 必须 non-empty/sorted/unique/canonical/equal。所有合法
  selection 都以 repeated `--platform` exact 保留；invalid identity 在 source checkout/preset apply/commit 前进入同一
...
- `EVO-DES-072`：#311 installed publication preservation由现有owner分层承接而不增加public Skill：
  ...
  subset expansion/loss/reorder 为 0；invalid selected list 必须在 source checkout/apply/commit 前 fail
