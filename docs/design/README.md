...
唯一 current Design authority：[`versions/current-main-0.6.17-guru.66/design-main.md`](./versions/current-main-0.6.17-guru.66/design-main.md)。版本状态与 source binding 见同目录 `manifest.yaml`；#454 D436 非激活 terminal lifecycle packages 与 registry（32 packages / 142 exits / 102 commands + 6 planned IDs）见 [`capability-inventory.md`](./versions/current-main-0.6.17-guru.66/capability-inventory.md)，引用链见 `traceability.md`，决策见 `decisions.md`。production workflow 保持 22 mandatory invokes / 98 exits。Architecture 为 `.66` / `active`；`.65` 及更早版本保持 immutable。
...
| 状态 | 版本 | Locator |
| --- | --- | --- |
| `active` | `current-main-0.6.17-guru.66` | [`design-main.md`](./versions/current-main-0.6.17-guru.66/design-main.md) |
