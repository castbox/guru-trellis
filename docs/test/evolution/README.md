...
Current as-built Test is [`current-main-0.6.5-guru.46`](../versions/current-main-0.6.5-guru.46/test-strategy.md).
Its `TST-046..063` / `SCN-054..066` additions define #332 exact-candidate Release Gate and original-entry verification ownership and
...
`TST-036..039` / `SCN-048` 保持历史/current fact-only checks；`.45` graph 为 23 Skills / 97 exits /
77 commands，`.46` 的 `TST-067..073` / `SCN-070..076` 把 #376 continuity/projection 与 23/97/78 graph
映射到既有 Evolution fixtures，不增加 fixture row；
