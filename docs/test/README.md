...
当前 authority：[`versions/current-main-0.6.17-guru.52/test-strategy.md`](./versions/current-main-0.6.17-guru.52/test-strategy.md) 与 [`test-plan.md`](./versions/current-main-0.6.17-guru.52/test-plan.md)。`traceability.md` 连接 Requirements、Design、Architecture `.52/active` 和 evidence；`.51` 保持 immutable。

状态：`active`。#408 source evidence 区分真实 installed、native、mock 与未验证远端边界；历史结果不标为本次 PASS，知识提升不证明正式 promotion 校验、后续 gate 或 Release 完成。

| 状态 | 版本 | Locator |
| --- | --- | --- |
| `active` | `current-main-0.6.17-guru.52` | [`test-strategy.md`](./versions/current-main-0.6.17-guru.52/test-strategy.md) |
