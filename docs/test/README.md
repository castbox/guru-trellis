...
当前 authority：[`versions/current-main-0.6.17-guru.58/test-strategy.md`](./versions/current-main-0.6.17-guru.58/test-strategy.md) 与 [`test-plan.md`](./versions/current-main-0.6.17-guru.58/test-plan.md)。`traceability.md` 连接 Requirements、Design、Architecture `.58/active` 和 evidence；`.57` 及更早版本保持 immutable。

状态：`active`。`.58` 完整继承 immutable `.57` 并承接 #452 两层平台 authority：pinned upstream 22-platform inventory 与目标仓库 exact installed selection。`T452-01..12` 是 current acceptance authority；promotion 时全部为 `not_executed`，截至 2026-09-20 fresh evidence 已使 `T452-01..11` 通过，`T452-12` 为 `in_progress`。#434 production activation、完整 Release Gate、正式 tag/Release 与业务生产升级仍 `unverified`。

| 状态 | 版本 | Locator |
| --- | --- | --- |
| `active` | `current-main-0.6.17-guru.58` | [`test-strategy.md`](./versions/current-main-0.6.17-guru.58/test-strategy.md) |
...
`.58/active` 完整继承 immutable `.57` 并吸收 reviewed #452 contribution；Architecture 为 `.58/active`。三平台 default/dogfood 不代表完整 22-platform inventory，公开 CLI 不提供全集安装选项；OpenCode 只在显式选择时进入安装。fresh RDT 与 Phase 2 evidence 已建立，Task Commit 和完整 Branch Review 仍待执行；#434 production activation 与完整发布矩阵保持独立且当前未验证。
