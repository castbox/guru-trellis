...
版本：`current-main-0.6.17-guru.53`；状态：`active`；predecessor：`current-main-0.6.17-guru.52`；source baseline：reviewed #418 contribution + inherited immutable `.52` authority；精确 revision 由 containing Git object/tree identity 绑定。
...
| `TST-051` | current graph closure | registry/interface/workflow/preset exact 派生 23 Skills / 100 exits / 78 commands，22 integrated + 1 standalone；业务 22 invokes/98 exits | 历史固定计数替代 current inventory |
...
## #247 Issue scope ledger retirement strategy
...
| `T247-04` | completed、reference-only empty close set、no-Issue、non-default-base四路Publication决定；Finalizer archive/Ready与normal/terminal handoff生成同一reviewed body SHA-256，Merge在scope推导和mutation前拒绝body-only drift | static keyword scan不替代owner behavior，Merge不新增reprepare typed exit |
| `T247-05` | Finalizer archive-month、existing PR、lost-result、reprepare、terminal recovery、Merge四 exits与archived Restore只消费current owner facts | 不建立ledger fallback或替代aggregate |
...
| `T247-08` | canonical/dogfood/installed/Shared/Codex/Claude/Cursor parity、`guru-ledger-free-runtime@1.0.0`、apply/reapply/update、ownership/drift/sidecar与真实wrapper旧流程fixture | 不等于完整Release matrix |

- `SCN-085 issue-backed completed`：Publication默认判定关闭；默认分支PR body写入closing keyword，
  GitHub自动执行，Merge只验证live facts。
- `SCN-086 issue-backed remain-open`：live Issue含合并后仍待完成条件或当前交付不完整；Publication仅引用
  并记录具体current-authority原因。
- `SCN-087 no-Issue`：Commit/PR不制造Issue reference、primary Issue或关闭效果。
- `SCN-088 inert legacy present`：absent/present-A/present-B不影响current lifecycle；active runtime无read，
  preset/update不主动触碰。
- `SCN-089 recovery`：Finalizer archive/Ready、existing PR、lost-result、reprepare、terminal与archived
  restore/re-entry使用current task/Git/PR/provider facts，不重复副作用且不读取ledger。
- `SCN-090 distribution, non-default publication and body continuity`：canonical、dogfood、installed与
  四平台package/capability一致；非默认分支PR只引用，后续目标默认分支Publication fresh判断并编码closing keyword；
...
| `T408-04` | 安装入口/示例可达；完整 AI 语义输入省略 recorder 私有派生字段仍成功，缺判断及普通 stale 内容拒绝；native 按合同执行原链，23/97/78 与 developer/ledger active-zero 保持 | 预填语义 fixture 不替代 native，native 不借 eval runtime 或手工 private plan |
...
需求与实现分别引用 [Requirements](../../../requirements/versions/current-main-0.6.17-guru.53/requirement-main.md)
和 [Design](../../../design/versions/current-main-0.6.17-guru.53/design-main.md)，完整双向关系见
[traceability.md](./traceability.md)。继承 T329/T247 的 retired-zero 与旧 lifecycle；历史 pin 证据
...
| T418-11 | 原 active profiles、status、anchors、普通成功/失败出口和 mutation path 不回归，#247 ledger-free/Restore 保持 |
...
[test-plan.md](./test-plan.md)。本 .53 promotion-created diff 仍须 fresh Phase 2、Task Commit、完整 Branch Review。
