...
`T454-C5-01..09` 承接 session adapter、单一 resource ledger、Finish seal 与 Cleanup resolution；focused lifecycle
`113/113` 为提升前 C5 candidate evidence。重点以真实 Git ancestor 验证 remote H1→H2 单 incarnation 单调推进、
equal-HEAD 只读、rebind seal 最新 HEAD、caller-owned 保守恢复不改变 ownership；unique candidate 自动选择，
零/多个及显式未发现 target 走人工选择后同一 live validation。Draft 2020-12 schema、Python compile、task
validator 与 `git diff --check` 为 supporting evidence，不替代 promotion-created diff 的 fresh Phase 2、Task
Commit、完整 Branch Review。installed/platform、E434 activation、Release matrix 仍未验证。

## #454 C6/C7 当前计划

`T454-C6C7-01..07` 采用 [reviewed test contribution](../../../requirements-design-test-contributions/454-task-lifecycle-state-model-c6-c7/test.md)
中的逐项验证责任。提升前 focused lifecycle `125/125` 与 planned inventory `3/3` 已通过，package integration
`19/20`、managed verifier fixture 未通过；Fixed Fork 本地 build、完整 E434 package、installed/platform 与
#410 Release matrix 均不声明通过。selected-base drift、caller-owned checkout 保留、task/session result-loss
恢复和 activation input freshness 是本次关键回归。Docs promotion 产生的新 diff 必须重新通过 Phase 2、Task
Commit 与独立完整 Branch Review，随后才能进入 Publication。

## #454 D443 当前验收计划

`T454-D443-01..04` 由 [reviewed test contribution](../../../requirements-design-test-contributions/454-task-lifecycle-d443/test.md)
进入 `.65` current。以 Fixed Fork source-lock 的实际 session port 与真实 Git checkout 验证 generation 0、
TaskRef 移动、resume/rebind/switch/Reactivate、pointer-only recovery、explicit mode 和 stale/conflict
零写入；真实 Git fixture 保留旧 checkout 并换绑，先复现全 worktree resolver 的歧义，
再验证 current checkout 唯一选择后的 rebind/resume。审查五个旧成功出口、显式出口和 ReasonDTO、
source package 闭包及非激活边界。

提升前 Bind tests 12/12、source package check 与 compile 通过；全局 package integration 19/20，唯一
错误为未修改的 Closure relative `$ref`，不声明通过。Docs promotion diff 的 Phase 2、Task Commit、
独立完整 Branch Review 需要在本版本写入后重新执行。E434 installed/platform 与 #410 Release matrix
尚未验证。

## #454 D436 当前验收计划

`T454-D436-01..06` 执行五个 package test directories 与 shared task lifecycle runtime，使用
`--import-mode=importlib` 合跑以隔离重复 `tests` 包名；再跑 package source/integration、task validator、
`git diff --check`，核对整个 D436 切片未修改 active graph/selector/manifest/installed/platform。
重点以真实 Git merge/ledger 验证 Finish 对 PR head 的 resource seal 和 merge SHA 的 archive 校验；
Cleanup 覆盖正常删除、已不存在、失败重试、手动选择与 machine-handoff；Reactivate 检查旧代
receipt/binding 不成为新代 authority。

测试输出需分别记录通过、失败与未运行；D436 非激活 package evidence 不等于 E434 installed/platform
或完整 Release matrix proof。提交后独立复核 `origin/main...HEAD` 的完整 diff。
