...
当前 .51 来源：`castbox/Trellis@db4ca1dfbb5abaf9be62b2a01b70dda3f80df0f0` / CI `34838784963` / CLI/core `0.6.17`；Guru manifest `0.6.16-guru.41`；repository release target `v0.6.16-guru.1`。Architecture public inheritance：`docs/architecture/README.md` / `current-main-0.6.5-guru.51` / `active`。
继承段落中的旧版本映射、矩阵及历史 promotion 只绑定其原版本，不声明当前 Fork 的完整兼容或 Release；当前 #408 增量见同版本主定义与 traceability；完整继承 immutable `.50`，#247 及更早段落按原版本保留，不代表本次重新验证。


版本：`current-main-0.6.5-guru.51`；状态：`active`；predecessor：`current-main-0.6.5-guru.50`；source baseline：reviewed #408 contribution + inherited immutable `.50` authority；精确 revision 由 containing Git object/tree identity 绑定。
...
需求与实现分别引用 [Requirements](../../../requirements/versions/current-main-0.6.5-guru.51/requirement-main.md)
和 [Design](../../../design/versions/current-main-0.6.5-guru.51/design-main.md)，完整双向关系见
