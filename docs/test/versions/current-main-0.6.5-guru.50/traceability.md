...
本 trace记录 `reviewed_promoted` stable contract；`.49` 是 immutable predecessor，`.50` 是 current active
authority。promotion-created diff、Publication与远端Release/Issue结果仍由后续owner fresh验证。
