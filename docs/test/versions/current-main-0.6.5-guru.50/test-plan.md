...
## #247 Issue scope ledger retirement evidence contract

| Gate | Reviewed evidence | Stable boundary |
| --- | --- | --- |
| fresh Phase 2 | Architecture `baseline_current / architecture_impact / target_native / reviewed_candidate`、ADR required；qualifiers `qualified_current`；`guru-check-task:passed` | 绑定promotion前完整candidate，不替代promotion-created diff review |
| package/runtime integration | 2026-09-13相关 Workspace、Commit、Review、Publication、Finalizer、Merge、Restore、Planning、qualification与runtime共`392 tests / PASS` | earlier `203 tests / OK (skipped=1)`仅为此前完整回归事实 |
| active-zero and distribution | ledger writer/reader/precondition/schema registration/aggregate DTO consumer为零；ownership与dogfood drift `status=ok`；canonical/installed/platform parity通过 | 历史archive/ADR/version命中排除；完整Release matrix未运行 |
| Publication body continuity finding-fix | Finalizer输出与Merge workflow input绑定`publication_body_sha256`；normal/terminal materializer、body-only drift pre-mutation rejection、standalone exclusion及cross-Skill integration通过 | 只传递body identity，不恢复Issue数组/ledger，不新增Merge reprepare typed exit |
| independent committed review | exact range `ec016827fac81d33faeacb307b0db76d5259dc28...9c3c00908446ac0fa86974cb9886f37917ac40ca`；395 tests、456 changed JSON parse、103 changed Python AST、shell/task/diff/ownership/drift/active scan通过；P0-P3 findings zero | reviewer因只读约束未写fresh gate artifact；结论不证明远端动作 |
| authority promotion | expected `.49`、reviewed RDT/Architecture contributions、accepted `ADR-009`、唯一 active `.50`、R247/D247/T247/BEH-019..020/SCN-085..090与Architecture refs闭合 | `.49` immutable；promotion-created combined diff须fresh Phase 2/commit/review |
| remote and release transactions | Publication、push、PR、merge、tag、GitHub Release、Issue closure、生产业务仓验证 | 全部 `unverified`，由各live owner独立处理 |

该 stable Test authority只记录reviewed promotion输入与明确边界，不把测试数量冒充唯一 gate，也不写入
用户授权、动态checkpoint或可变HEAD。完整多平台 exact-candidate Release matrix继续deferred。
