...
完整继承 immutable `.54` 业务合同，当前增量为 #435；旧 pin、release mapping、计数及 promotion/evidence 叙述只绑定其明确历史版本。当前 registry 为 26 packages / 114 exits / 96 commands，production workflow 保持 22 mandatory invokes / 98 exits；旧验证不证明本次 candidate。

`.55` 完整继承 `.54`，吸收 reviewed #435 contribution；RDT current 前进到 `.55/active`，Architecture 同步为 `.55/active`。promotion-created diff 必须重新通过 fresh Phase 2、Task Commit、完整 Branch Review 后才能进入 Publication；本文不声明下游门禁已通过。
...
实现见 [D418-01..06](../../../design/versions/current-main-0.6.17-guru.55/design-main.md)，
验收见 [T418-01..14](../../../test/versions/current-main-0.6.17-guru.55/test-strategy.md)，
...
#436 独占 Completion、Closure、Finish、Cleanup 与 Reactivate。当前 registry 为 26 packages / 114 exits /
96 commands；完整多平台 clean/existing/update/workflow-switch/release-candidate Release matrix 保持
`unverified`，由专门 Release owner 在 exact candidate 上执行。
