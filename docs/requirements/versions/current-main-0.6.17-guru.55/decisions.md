...
- `RDEC-001`（accepted）：current knowledge authority 为 `current-main-0.6.17-guru.55`，与 extension `0.6.17-guru.42`、CLI/core `0.6.17`、repository target `v0.6.17-guru.1` 独立；旧 release mapping 仅为继承历史。
...
- `RDEC-028`（accepted）：#435 以 `target_native` 绑定 expected immutable `.54`，由 RDT serialized owner
