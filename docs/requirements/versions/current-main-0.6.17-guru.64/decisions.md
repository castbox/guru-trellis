...
- `RDEC-001`（accepted）：current knowledge authority 为 `current-main-0.6.17-guru.64`，与 extension `0.6.17-guru.42`、CLI/core `0.6.17`、repository target `v0.6.17-guru.1` 独立；旧 release mapping 仅为继承历史。
...
- `RDEC-044`（accepted）：#454 C4 以 expected immutable `.61` serialized promotion 建立唯一 active RDT `.62`；TaskBranchBinding、establishment 与 rebind substrate 使用 epoch/revision/expected-HEAD closed contract，三个 owner IDs 保持 planned，32/142/102 active registry 与 22/98 production graph 不变。C5-C7、D443、D436、E434 与 #434 activation 保持后续边界。
- `RDEC-045`（accepted）：#454 C5 以 expected immutable `.62` 串行提升 RDT `.63`，继承 Architecture `.63/active`；official session store 单写、Git common-dir 唯一 resource ledger 与宽松最小充分 candidate 选择各守其责。C5 新 ID 仅 planned，active 32/142/102 与 production 22/98 不变；C6-C7、D443、D436、E434 与 #434 activation 保持后续边界。
- `RDEC-046`（accepted）：#454 C6/C7 以 expected immutable `.63` 串行提升 RDT `.64`，继承 Architecture `.64/active`。Reviewed C6 task creation/activation inputs 与 C7 subtraction 仅形成非激活 current substrate；六个 ID 仍 planned，active 32/142/102 和 production 22/98 不变。D443/D436/E434、#434 activation 与 #410 matrix 保持独立。
