...
定义定位：同版本 Requirements requirement-main.md、Design design-main.md、Test test-strategy.md；R378/D378/T378 均为 accepted/current，关系为 implements/verifies。原 contribution 保留 promotion 来源引用，不再重复 current 正文。
...
predecessor，`.50` 是 current active authority。promotion-created diff与远端动作仍需各 owner fresh验证。
