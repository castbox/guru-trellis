...
版本：`current-main-0.6.5-guru.50`；状态：`active`；predecessor：`current-main-0.6.5-guru.49`；source baseline：reviewed #247 contribution `architecture-contribution-247-remove-issue-scope-ledger-v1` + accepted `ADR-009` + inherited immutable `.49` authority；精确 revision 由 containing Git object/tree identity 绑定。
...
来源：reviewed #378 contribution；稳定 ID 在 promotion 前后不变，状态为 accepted/current。
...
`BEH-009` 的当前来源路径由 R378-01/03 承接；REQ-016/017、BEH-011 的旧 official-version matrix 保留为历史版本合同，不再作为当前安装入口。
