...
完整继承 immutable `.58` 业务合同，当前增量为 #454 C2 shared lifecycle kernel 与 D0 stage-evidence correction；旧 pin、release mapping、计数及 promotion/evidence 叙述只绑定其明确历史版本。当前 registry 保持 32 packages / 142 exits / 102 commands，production workflow 保持 22 mandatory invokes / 98 exits。

`.59` 完整继承 `.58`，吸收 reviewed #454 contribution；RDT current 前进到 `.59/active`，Architecture inheritance 为 `.59/active`。本次只提升已验证的 C2+D0 slice；promotion-created diff 必须重新通过 fresh Phase 2、Task Commit、完整 Branch Review 后才能进入 Publication，C3-C7、D443、D436、E434、Release 与生产验证仍未完成。
