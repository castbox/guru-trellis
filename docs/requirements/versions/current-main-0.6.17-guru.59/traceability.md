...
完整继承 immutable `.58` 业务合同，当前增量为 #454 C2+D0；current registry 保持 32 packages / 142 exits / 102 commands，production workflow 保持 22 mandatory invokes / 98 exits。

`.59` 完整继承 `.58`，吸收 reviewed #454 contribution；RDT 与 Architecture current 均为 `.59/active`。promotion-created diff 必须重新通过 fresh Phase 2、Task Commit、完整 Branch Review 后才能进入 Publication；剩余 lifecycle migration、#434 activation、tag 与 Release 未完成。
