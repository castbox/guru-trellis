...
`.59` 完整继承 `.58`，吸收 reviewed #454 contribution；RDT 与 Architecture current 均为 `.59/active`。promotion-created diff 必须重新通过 fresh Phase 2、Task Commit、完整 Branch Review 后才能进入 Publication；本文不声明剩余 lifecycle migration、#434 activation、tag 或 Release 已完成。

- `RDEC-001`（accepted）：current knowledge authority 为 `current-main-0.6.17-guru.59`，与 extension `0.6.17-guru.42`、CLI/core `0.6.17`、repository target `v0.6.17-guru.1` 独立；旧 release mapping 仅为继承历史。
