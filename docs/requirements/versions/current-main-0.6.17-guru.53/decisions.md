...
- `RDEC-001`（accepted）：current knowledge authority 为 `current-main-0.6.17-guru.53`，与 extension `0.6.17-guru.42`、CLI/core `0.6.17`、repository target `v0.6.17-guru.1` 独立；旧 release mapping 仅为继承历史。
...
原 Merge 四出口与 ledger-free/Restore 不变；新增出口只接续新审查，不复制旧 pass。
