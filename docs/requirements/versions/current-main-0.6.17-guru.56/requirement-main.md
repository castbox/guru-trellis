...
完整继承 immutable `.55` 业务合同，当前增量为 #436；旧 pin、release mapping、计数及 promotion/evidence 叙述只绑定其明确历史版本。当前 registry 为 31 packages / 136 exits / 101 commands，production workflow 保持 22 mandatory invokes / 98 exits；旧验证不证明本次 candidate。

`.56` 完整继承 `.55`，吸收 reviewed #436 contribution；RDT current 前进到 `.56/active`，Architecture 同步为 `.56/active`。promotion-created diff 必须重新通过 fresh Phase 2、Task Commit、完整 Branch Review 后才能进入 Publication；本文不声明下游门禁已通过。
