...
- `RDEC-001`（accepted）：current knowledge authority 为 `current-main-0.6.17-guru.65`，与 extension `0.6.17-guru.42`、CLI/core `0.6.17`、repository target `v0.6.17-guru.1` 独立；旧 release mapping 仅为继承历史。
...
- `RDEC-047`（accepted）：以 reviewed D443 contribution 与 expected immutable `.64` 建立 RDT `.65` current，继承 Architecture `.65/active`。新 Bind canonical package 用 TaskId/generation 与 official schema-2 pointer 替代旧 mapping/path 行为；历史 `R443-*` 不可覆盖本 successor。32/142/102 selector 与 production 22/98 不变；D436/E434、installed/platform 和 #410 Release matrix 仍独立。

## #454 D436 `.66` Decision

- `RDEC-048`（accepted）：以 reviewed D436 contribution 与 immutable `.65` 建立 RDT `.66` current，继承 Architecture `.66/active`。五个终态 lifecycle canonical packages 以 current generation、Closure/Finish result、单一 ledger 和 live Git facts 取代旧 mapping/receipt/resource-list authority；Finish seal 使用 PR head，目标 merge SHA 单独验证 archive，binding 在 terminal persistence 后退役。32/142/102 selector 与 production 22/98 不变；E434/#434 激活、installed/platform 和 #410 Release matrix 仍独立。
