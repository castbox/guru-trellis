...
C5-C7、D443、D436、E434、#434 activation 与完整 Release matrix 继续保持独立后续边界（此句是继承自 `.62` 的 C4 历史边界）。

## #454 C5 Session And Resource Control

- `R454-C5-01`：session 领域值仅为 TaskId 与 lifecycle generation；official schema-2 record 不携带 TaskRef、branch、path、HEAD、ownership 或授权。
- `R454-C5-02`：adapter 复用 Fixed Fork official repository/common-dir session store；缺 context key 返回 `explicit_task_mode` 而不写记录，session 写失败不回滚已完成 lifecycle。
- `R454-C5-03`：多 session 可指向同一 task，同一 session 可受控 A-to-B-to-A；Reactivate 使旧 generation 失效，TaskRef 每次 fresh 解析。
- `R454-C5-04`：Git common-dir 的唯一 resource ledger 以 TaskId/generation 隔离 portable incarnation、origin、ownership、binding epoch/revision、state 与 responsibility role，不保存 checkout path authority。
- `R454-C5-05`：unknown ownership 是 caller-owned；active ledger 缺失在 current binding/live facts 验证后保守重建；conflict 单独拒绝，terminal 缺失只走人工 Cleanup selection。
- `R454-C5-06`：同一 ledger 实现 C4 `OwnershipPort`；只严格验证已建立 DTO/generation/epoch/revision/branch/incarnation authority，不从 path、branch、Issue 或目录推断 task，不增加 strict selector。
- `R454-C5-07`：rebind 保留旧 incarnation；Guru-owned 进入 `cleanup_pending`，caller-owned retained/manual-only，未收敛 portable ref 不复用。
- `R454-C5-08`：Finish 以 exact `finish_head` seal 全 generation inventory；普通 Cleanup 仅删除已 seal 的 Guru-owned cleanup-pending resource，排除 caller-owned、unknown 与 retained control ref。
- `R454-C5-09`：remote identity 包含 remote name、repository 与完整 branch ref；当前 delivery remote 可为空或唯一。同一 incarnation 正常 Publication HEAD 仅按 Git ancestry 单调推进，equal HEAD 只读，回退/身份漂移拒绝；Guru-owned 初次记录须有 exact HEAD。
- `R454-C5-10`：唯一合法 candidate 自动选择；零/多个要求人工选择；用户可显式指定未发现 target，并经相同 fresh live validator。
- `R454-C5-11`：`guru-establish-task-identity` 仅为 planned ID；E434 前不创建 package/interface/command/active edge/installed/platform projection。

C6-C7、D443、D436、E434、#434 activation 与完整 Release matrix 仍未完成；C5 current authority 不授权执行 Cleanup。（此句是继承自 `.63` 的 C5 历史边界。）

## #454 C6/C7 非激活创建基座与验证

`R454-C6C7-01..09` 完整继承 [reviewed contribution](../../../requirements-design-test-contributions/454-task-lifecycle-state-model-c6-c7/requirements.md)，
并将它纳入 `.64` current acceptance。Task creation 的 source 与 accepted scope 分离，reviewed
`existing_issue | standalone_request` 走同一创建组合；Issue creation 仍归独立 owner。C3 acquisition 的
adopt/provision 使用同一 live validator，五种 ownership 投影只标记 Guru 实际创建的资源。C4 initial binding
与 C5 resource ledger、session adapter 由 C6 shared runtime 组合；无 context key 进入 explicit-task mode，
结果丢失只读恢复同一 task identity。Activation inputs 只验证 planning status、current branch/ledger、
selected base ref/head 与 task-head ancestry；未来 `guru-activate-task` 才独占 status mutation。

六个稳定 ID 只处于 planned inventory，active 32/142/102、production workflow 22/98、installed/platform
投影均未切换。C7 只证明新 Phase C substrate 零使用旧 mapping/path；已有生产 predecessor 的退役、
D443/D436/E434 package migration 和 #434 graph activation 留在后续 owner。`ARCH-GAP-011` 保持 partial/open，
#410 完整 Release matrix 未验证。

## #454 D443 非激活 Bind package

`R454-D443-01..04` 由 [reviewed D443 contribution](../../../requirements-design-test-contributions/454-task-lifecycle-d443/requirements.md)
进入 `.65` current。Bind 对 TaskId 与 lifecycle generation（包括 0）执行 resume、rebind、switch、
Reactivate rebind 和 pointer-only manual recovery；session record 仍只由 Fixed Fork schema-2 API 持有。
任务、checkout、branch binding 与 resource ownership 均从 Phase C 当前事实解析；先选唯一 current checkout，
再让官方 TaskId resolver 检查其 task artifact，保留的旧 branch checkout 不参与 current 候选。旧 mapping/path/HEAD 不再
成为 target Bind 权威。五个成功出口只交付 TaskLifecycleDTO 与 resume_target；缺 context key 进入
`explicit_task_mode`，阻塞只交付 ReasonDTO。旧 #443 当前段落是 immutable predecessor 的历史行为，
其中重建 mapping 的语义被本增量取代。

本增量不切换 registry selector、active manifest、production workflow 或 installed/platform 投影。
D436、E434 与 #434 才承担后续 package/graph activation 和旧 production edge 退休；`ARCH-GAP-011`
仍为 partial/open，#410 多平台 Release matrix 未验证。

## #454 D436 非激活终态生命周期 packages

`R454-D436-01..06` 由 [reviewed D436 contribution](../../../requirements-design-test-contributions/454-task-lifecycle-d436/requirements.md)
进入 `.66` current，替代历史 #436 package 的目标语义，不改写 immutable `.65`。Completion 只以当前
TaskLifecycleKey、accepted scope、准确的 Delivery merge lineage 与 evidence slots 判断七种结果；PR Ready、
merge 或 archive 不自动构成完成。Closure 以 current Completion 和 source/scope/target/binding/evidence
冻结精确 action set；no-Issue/reference-only 不关闭 Issue，外部状态变化返回 semantic re-entry。

Finish 在 archive/ledger mutation 之前重读 required-closed Issue，验证当前 Closure，持久化本代 archive
和 lifecycle-only bookkeeping；terminal seal 锁定 PR head 所在的资源分支与目标 merge SHA 的归档事实，
随后退役本代 TaskBranchBinding，不执行删除。Cleanup 的 normal profile 仅消费本代 ResourceSealRefDTO，
从 common-dir ledger 定位 Guru-owned 资源；caller-owned 保留，terminal ownership 缺失进入 manual selection，
machine-handoff 单独承接，旧代 Finish 不能授权新代删除。Reactivate 沿用 TaskId/source/scope，建立严格递增
generation 的 checkout/binding/ledger/session；Planning 是正常重入，session recovery、同事务 resume、source
correction 是明确出口，不复用历史 receipts。

五个 canonical package 的 I/O、schema、consumer 与 runtime 是 package-ready source，不改变 active
registry selector、production workflow、active manifest 或 installed/platform bytes。E434/#434 独占原子激活，
`ARCH-GAP-011` 仍 partial/open，#410 Release matrix 未验证。
