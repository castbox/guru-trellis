...
- `RDEC-001`（accepted）：current knowledge authority 使用 `current-main-0.6.17-guru.52`，明确区别于
  正式 predecessor `v0.6.15-guru.6` 与 current target `v0.6.16-guru.1` / extension
  `0.6.16-guru.41`；当前 Fork CLI 为 `0.6.17`，而 release mapping 的 CLI `0.6.16` 保持历史边界。
...
- `RDEC-025`（accepted）：#408 以 immutable `.50` 为 predecessor，`.51` 承接
  R408-01..08。仅 current source pin 前进，R329 developer-free、R247 ledger-free、R378 session
