...
## #247 Issue Scope Ledger retirement

来源：reviewed five-file RDT contribution、Architecture contribution
`architecture-contribution-247-remove-issue-scope-ledger-v1`、accepted `ADR-009` 与 expected `.49`
serialized promotion。以下 stable ID 在 `.50` 为 accepted/current；它们直接删除 task-local Issue
classification aggregate，不改变 23 Skills / 97 exits / 78 commands、四阶段顺序或 #305 Evolution target。

- `R247-01`：current active graph 必须移除 `issue-scope-ledger.json`、`guru-issue-scope-ledger-*`、
  `primary_issue` 与 Issue-array aggregate 的 writer、reader、precondition、schema registration 和 consumer。
- `R247-02`：失去唯一 ledger consumer 的 Skill Markdown、interface/consumer schema、eval/example JSON、
  DTO、runtime/script、fixture/test、manifest/registry 与 current docs 必须同 task 退出；不保留 nullable 壳、
  alias、adapter、fallback、dual-read/write 或替代 aggregate。
- `R247-03`：task/workspace creation 只创建 official task/worktree/mapping identity；Clarification/Readiness
  不再投影 Issue aggregate，Workspace 不写入、登记或恢复 ledger。
...
- `R247-05`：Publication 是 Issue reference 与 closure intent 的唯一 semantic owner。Issue-backed task
  完整解决 current Issue 时默认关闭；remain-open 必须绑定 live Issue 中合并后仍未完成的具体条件或本次
...
`BEH-020` Publication closure ownership：Issue-backed completed、Issue-backed remain-open、no-Issue 与
non-default-base 四路分别产生默认关闭、有 current-authority 原因的保持 open、无 Issue identity 与延迟到
后续默认分支 Publication 的决定；Finalizer、Merge 与其它 lifecycle owner不形成第二 closure authority。

`.50` promotion 只把 reviewed #247 contract 合并为 current authority。promotion-created combined diff仍须
fresh Phase 2、task commit 与独立完整 Branch Review；Publication、push、PR、merge、tag、GitHub Release、
生产业务仓验证与 Issue closure 均未由本正文声明完成。
...
- `R408-04`：正常 Agent 按实际安装合同调用现有 owner，不依赖 eval runtime、手算 recorder 私有
  ...
  的旧完整生命周期、23 Skills / 97 exits / 78 commands，以及 developer/ledger active-zero；
...
- `R408-07`：手动 Git/GitHub 成功与 task/runtime/Finalizer/archive residue 分别报告，不补写 Guru gate、
  archive 或完成标记；residue 不禁止独立确认的操作。R247 的 closure owner 限于 Guru lifecycle，
...
本增量不实施 #398/#407，不改变 Constitution、GAP、ADR、Evolution 或旧 ledger-bound task 不迁移
的边界。继承合同见前述 R247/R329/R378；双向关系见 [traceability.md](./traceability.md)。
