...
版本：`current-main-0.6.5-guru.48`；状态：`active`；predecessor：`current-main-0.6.5-guru.47`；source baseline：reviewed #392 contribution `architecture-contribution-392-release-v0616-guru1-v2` + inherited immutable `.47` authority；精确 revision 由 containing Git object/tree identity 绑定。
