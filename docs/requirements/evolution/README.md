...
| `requirements_trace_ready_for_design` | [`current-capability-inventory.md`](./current-capability-inventory.md) | current `.46` 可观察能力、24 个 `CUR-CAP-*`、13 个 `TARGET-DELTA-*` 与 50 个 normal-path fixture 的 successor/差集；#311 已折入 `CUR-CAP-013/014/017/018/019`，#312 与 #376 post-review base continuity 均折入 `CUR-CAP-012`，PR #317 平台保真折入 `CUR-CAP-013/014/017`；`.46` 的 23/97/78 graph 与 continuity/projection facts 不新增 capability、delta、fixture 或 target responsibility；Requirements-stage trace closure 与 fresh review 已通过，pre-`REQ-REV-142` Design review 仍 stale，但 current exact candidate 的 fresh Design review 与确定性闭包已通过 |
