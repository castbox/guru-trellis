...
| Same-entry recovery | 同一 finish entry 在 archive 前从 committed plan/readiness/evidence 与 active locator 恢复；official move 后只从 immutable plan、exact archive path/blob/commit lineage 与 remote/PR HEAD facts 恢复，不暴露 `--skip-archive`、publish recovery flag 或 artifact override。 |
