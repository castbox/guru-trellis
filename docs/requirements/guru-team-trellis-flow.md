...
| Archive destination / children preflight | shared prepare 要求计划 archive locator 不存在；`task.json.children` 缺失按空 list，否则严格为 `list[str]`。按官方 active task exact/suffix lookup，只有会被 archive 改写的 active child 阻塞；已归档 child 不阻塞 parent。dry-run/formal 均在 Git、GitHub、recorder mutation 前失败。 |
