...
9 Skills/35 exits graph；Stage 0 identity 仍为 6/24，mixed graph、缺失 case、平台字节漂移或
`.new`/`.bak` 均失败关闭。
...
`branch-review-or-finding-closure` 消费；本迁移不激活 #131。

三条 semantic handoff 使用 target-owned `skill_input_authoring_seed`：planning revision
...
`source_exit/task_ref`。Target package authoring example 只包含 fresh caller AI fields；seed
