...
| Exact archive lineage | evidence commit parent 必须是 reviewed work HEAD；archive commit parent 必须是该 evidence commit。`tracked_move_paths` 必须同时出现 active 删除和 archive 新增；evidence commit 后才生成的 `untracked_archive_outputs` 只能出现 archive 新增。partial、missing、extra、误分类 path 均阻塞。 |
...
| Same-entry recovery | 同一 finish entry 在 archive 前从 committed plan/readiness/evidence 与 active locator 恢复。official move 后、精确 archive commit 形成前，仍校验 archived working-tree layout、dirty/staged paths、blob continuity 与官方 `task.json` delta；commit 缺失或不匹配 fail closed。当前 `HEAD` 已是精确 archive commit 后，只校验 immutable plan、Git parent/path/tree/blob lineage 与 remote/PR HEAD facts，本地 archived 文件缺失/篡改不阻塞 push/ready。plan-only archived directory 仅供 finish-work recovery 解析；该入口从commit blob读取plan，并在GitHub/fast-path前校验Git toplevel、配置/effective repo、head branch、base ref、HEAD transaction、digest、task identity和locators，不无条件跳过boundary。raw locator在普通resolve前只允许basename/原active/精确archive语义；path-like输入先从repo root到final dir逐组件lstat，拒绝内外部、相对/绝对、ancestor/final、多层、dangling/loop alias，再调用普通resolver以保留显式`task.json`、active和普通archive优先级。只有ordinary not-found才进入plan-only fallback；精确archive只尝试该候选，basename/原active fallback要求唯一archive月份，多候选fail closed。plan-only resolved target仍须等于plan canonical locator，仅固定Darwin `/var -> /private/var`系统映射可重锚。普通命令仍要求 `task.json`，worktree mode仍要求`task-start-context.json`；不暴露 `--skip-archive`、publish recovery flag 或 artifact override。 |
...
| `finish-summary.json` | draft PR 后、archive 前 | archived task-local 完成摘要 | #98 历史检索；一次生成 canonical PR URL/ref，随 archive move 进入终态。 |
