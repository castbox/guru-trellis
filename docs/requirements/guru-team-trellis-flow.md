...
Production activation 以 `stage0-minimal-handoff-v1` 为唯一原子单元：
...
`guru-create-task-workspace` 六包及 24 exits 同时使用 Interface 1.3；planning/check/commit
三个 #146 Skills 保持 Interface 1.2 legacy。Migration manifest 把每个 input profile、exit、
...
graph；mixed graph、缺失 case、平台字节漂移或 `.new`/`.bak` 均失败关闭。
...
planning/check/commit 三个 legacy packages 的迁移与 coverage closure。
