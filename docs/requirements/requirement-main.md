...
- Issue #122：Phase 2 pass 后的 task work commit 收敛为公共
  ...
  path 使用既有 SHA-256/mode/delete identity，candidate self 使用 validated plan
  deterministic bytes，所有 hook/commit 先在 isolated transaction 完成再发布。
