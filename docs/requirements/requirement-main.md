...
- Issue #113：激活 semantic `guru-clarify-requirements`，统一 initial intake
  ...
  `mechanism_removed/replaced` 无 confirmation/trail/mutation，planning evidence 复用 shared
  schema 1.2 完整 validator。
...
| Change-request readiness review | `guru-review-change-request`、`guru-change-request-review-1.0`、`record-change-request-review.sh`、`check-change-request-review.sh` | Canonical Skill package 独占 target/prerequisite semantic readiness、十项 dimensions、findings、delivery unit、scope conclusion、AI Gate、confirmation 与五出口。Runtime 复用前置 objective validators并重建 portable projection/linkage/facts，不生成 semantic result或 route。Pre-task/standalone 只输出 stdout；planned #112 后续只能持久化同一 checker-passed `issue-review.json` bytes。 |
| Planning start gate | `planning-approval.json`、`record-planning-approval.sh`、`check-planning-approval.sh` | 消费 checker-validated `guru-review-contract-wording:planning_artifacts:pass` evidence，逐项复制已验证 planning dimensions 到兼容 `ambiguity_review` 审计字段，并记录三份 planning artifact 的 hash / size / mtime、reviewer、`review_prompt_presented_at`、`approved_at`、HEAD、dirty paths 和 `user_confirmation.source=explicit-post-planning-review`。Validator 先校验 current wording evidence/binding，再校验 approval projection 与三文档 digest；HEAD / dirty paths drift 不单独阻塞。`task.py start` 只是状态写入。Phase 0 handoff、旧 schema/source、缺失/过期/non-pass wording evidence，以及 schema id 仍为 1.0 但缺少 planning-only dimensions 的旧 evidence 均不能通过 gate；后者必须完整 AI 重审、重新展示三文档并取得 fresh confirmation，禁止手补布尔值。 |
...
| #101 | open | 当前任务 | `guru-review-change-request` semantic readiness closed loop、三类 target、current prerequisite linkage、十项 dimensions、五个唯一 consumer、stdout-only record/check、planned #112 fail-closed transition 与旧 readiness route replacement。 |
