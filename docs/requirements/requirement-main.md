...
- Issue #111：`synced` mandatory invoke semantic `guru-discover-change-context`。
  ...
  现有 workflow route `guru-clarify-requirements`、active Skill `guru-sync-base` 和
  fail-closed stop；#111 不实现 #113 package。Deep-read locator 必须按 task artifact、
  canonical GitHub issue/PR、exact Git object/ref 分型校验；`task_branch_stale` 可完整刷新，
  其他 malformed task facts 仍阻塞；全部 payload string 必须拒绝 machine-local absolute
  path 与 signed-query credential。
...
schema/digest/security/semantic shape、base-only live gate、fresh-base-only repo
...
必须按 oldest `context_ready` ancestor 到 direct prior 顺序，为每条 refresh history 重复
提供实际 snapshot 与独立 expected digest；长度 `N` 的 history 还必须按 hop 顺序提供
`N-1` 份上一轮真实 `refresh_base` receipt 与独立 expected digest，单次 refresh 的单对
ancestor CLI 保持兼容且无需 receipt。Pure gate 重算整条 ancestor/receipt identity，验证
数量/顺序/history prefix/每条 superseded link，证明每份 receipt 是前一 ancestor append
完整 entry 的唯一 projection 且其 history 与下一 ancestor prefix 完全一致，并证明当前
payload 只从 direct prior append 最新 entry 后重算新 identity。Receipt 只有在上一轮
production result 输出时连同 digest 被独立保留才有 authority，当前候选链不能自行生成。
缺失、重复、错序、跳过、改写或非父关系 fail closed；task 写后重读完整外部链，外部
bytes 不持久化。Task-local recorder 写前/写后和 checker 必须通过 `git check-ignore
...
deep Git locator、issue、reviewed blob 或 history。Portable gate 只豁免精确
`/<namespace>:<command>` span 的裸文本、inline code、bold、句中或中文标点形态，不能
放过真实 absolute/multi-segment path 或 signed URL。
