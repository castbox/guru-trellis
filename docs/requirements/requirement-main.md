...
| Reviewed draft creation | Draft invocation 只取得 `github_issue_mutation` confirmation，创建 exact reviewed title/body/labels；GitHub adapter 不 trim title/body，也不追加换行。重读 live issue并记录 `created_issue_binding` 后立即返回 `refresh_review`；同一 invocation 不创建 branch/worktree/task。 |
