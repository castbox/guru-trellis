...
| 能力 | Artifact / 脚本 | 说明 |
| --- | --- | --- |
| Contract wording review | `guru-review-contract-wording`、`contract-wording-review.json`、`record-contract-wording-review.sh`、`check-contract-wording-review.sh` | Canonical Skill package 是 vocabulary、classification、三个固定 profile、AI semantic loop、confirmation policy、schema 与三个 typed exits 的唯一 owner。Shared runtime 只重建 scope、扫描、派生 digest/unchecked projection 并校验 freshness；`planning_artifacts` 固定绑定当前 task 的三份 planning docs，passed evidence 写入 task-local artifact。 |
| Planning start gate | `planning-approval.json`、`record-planning-approval.sh`、`check-planning-approval.sh` | 消费 checker-validated `guru-review-contract-wording:planning_artifacts:pass` evidence，投影兼容 `ambiguity_review` 审计字段，并记录三份 planning artifact 的 hash / size / mtime、reviewer、`review_prompt_presented_at`、`approved_at`、HEAD、dirty paths 和 `user_confirmation.source=explicit-post-planning-review`。Validator 先校验 current wording evidence/binding，再校验 approval projection 与三文档 digest；HEAD / dirty paths drift 不单独阻塞。`task.py start` 只是状态写入，Phase 0 handoff、旧 schema/source、缺失/过期/non-pass wording evidence 均不能通过 gate。 |
