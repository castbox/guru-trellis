...
| Reviewed draft creation | Draft invocation 只取得 `github_issue_mutation` confirmation，创建 exact reviewed title/body/labels 并重读 live issue，记录 `created_issue_binding` 后立即返回 `refresh_review`；同一 invocation 不创建 branch/worktree/task。 |
| Workspace/task executor boundary | Open issue invocation 另行取得 `workspace_and_task_mutation` confirmation。Executor 每个 mutation boundary 重验 plan/base/target/prerequisites，仅创建或 `reuse_exact` identity 一致对象。 |
| Assignee route | 固定顺序为 explicit input、exactly one issue assignee、zero issue assignees 时 current GitHub login、multiple/unresolved 时 AI/user 选择一个 login；未解析前零 workspace/task 副作用，且始终显式传给 `task.py create --assignee`。 |
