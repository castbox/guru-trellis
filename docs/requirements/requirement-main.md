...
- Issue #144：保留 public Skill interface 1.2 legacy 语义并新增独立 1.3 minimal
  ...
  `discover-skill-contract`。Skill consumer 必须引用 target-owned input 并绑定 exact
  target interface id；非 direct projection 必须证明所有合法 producer output 可消费，
  public/private schema id 与 path 分别互斥，wrapper 必须可确定性证明只路由 shared
  dispatcher。当前九个 production Skills 全部维持 1.2 `legacy`，1.3
  只由 mixed test-only fixture 验证，#145/#146 负责后续 payload migration。
...
| Public Skill typed handoff | Interface 1.2 与 1.3 独立共存；registry active row exact 选择 `1.2+legacy` 或 `1.3+minimal_handoff`。1.3 structured input 使用 closed schema/discriminator/`oneOf`，deterministic scalar CLI 不制造 input JSON；每个 exit 独立 output schema/example，每个字段绑定直接 consumer use，consumer input 由 Skill/workflow/stop 自身拥有，projection 只允许 direct/select/rename/closed normalize。Runtime/private checkpoint 与 gate evidence 不进入 public DTO。`discover-skill-contract` 返回 closed legacy/minimal index 和 portable stable errors，并通过 representative wrapper 真正执行验证。#144 的 production registry 仍是九个 legacy，mixed fixture 不安装、不进入 manifest 或 mandatory route。 |
