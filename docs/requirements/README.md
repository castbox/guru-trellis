...
Skill 对交付/Issue scope/PR body/validation/Branch Review/Docs SSOT/
safety-deployment/finish-summary/metadata tail/freshness 做完整 AI review，
...
`guru-verify-extension-installation` 是 active Interface 1.3 semantic owner。
Workflow profile `verification_required` 的业务 seed 固定为 `task_ref`、`plan_ref`、
`repo_ref`、`reviewed_head`、`verification_target`；standalone 使用结构不同的
`standalone_verification`，只接收 repo、remote、ref、caller intent 与可选 task identity。
四个 minimal exits 是 `verified`、`not_required`、`return_to_task_work`、`blocked`，
profile、adequacy、command/log digest、asset/ownership inventory 与 findings 只保留在
task-local `marketplace-verification.json` 或 taskless session-only owner result。

Skill 选择 closed capability matrix，覆盖 marketplace index、new install、
