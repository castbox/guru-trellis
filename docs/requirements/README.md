...
| --- | --- | --- | --- |
| `active` | `current-main-0.6.17-guru.65` | [requirement-main.md](./versions/current-main-0.6.17-guru.65/requirement-main.md) | #454 D443 非激活 Bind major；完整继承 immutable `.64`，Architecture 为 `.65/active` |
...
Docs `.65` 完整继承 immutable `.64` 并承接 #454 D443 非激活 Bind major；current registry 保持 32 packages / 142 exits / 102 commands 与六个 planned IDs，production workflow 保持 22 mandatory invokes / 98 exits。D436、E434 与 #434 activation 未完成；Architecture 为 `.65/active`。Promotion-created diff 尚需 fresh Phase 2/commit/完整 Branch Review；Bind 12/12 只证明提升前 candidate，package integration 19/20、installed/platform 与完整 Release matrix 均未声明通过。
...
`R454-D443-01..04 -> D454-D443-01..04 -> T454-D443-01..04` 与 C6/C7、C5/C4/C3 继承链见 [current trace](./versions/current-main-0.6.17-guru.65/traceability.md)。实际证明边界由同版本 Test 层维护。
...
`.65/active` 完整继承 `.64` 并吸收 reviewed #454 D443 contribution；Architecture current 为 `.65/active`。promotion-created diff 必须重新通过 fresh Phase 2、Task Commit、完整 Branch Review，随后才进入 Publication；D436、E434 与 #434 production graph activation 保持后续边界，#410 独占 post-merge Release Gate matrix 且当前仍未验证。
