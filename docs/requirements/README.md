...
- Issue #144 建立 interface 1.2/1.3 共存、registry 1.1 exact migration
  ...
  原子迁移到 1.3；#131 激活 Branch Review，#116 再激活 publication review。当前
  live closure 为 11 Skills / 42 exits，Stage 0 的 6/24 identity 与独立 production
  manifest 的 3/11 membership 保持不变。
...
加后续完整 active rows 形成当前 11/42 closure。`committed` DTO 只含 `exit_id`、`task_ref`、`base_ref`、
...
10 Skills/39 exits；#116 激活后为 11 Skills/42 exits。#146 production migration
仍保持 3 Skills/11 exits 与原 activation id。
