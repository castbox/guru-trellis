...
- `R311-08`：matching post-bind existing-PR recovery 必须先于 pre-PR provenance inference；public
  profiles、typed exits、transaction、Draft/Ready/archive/Merge handoff 与 terminal freshness不变。
...
Finalizer public I/O，也不建立 legacy fallback、dual-read、shared source resolver 或 verifier re-entry。
