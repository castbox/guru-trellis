...
- `R454-C3-03`：candidate 与 resolution 使用封闭状态集合，并拒绝状态、candidate 集合、selection、
  authority conflict 或 remediation 彼此矛盾的 payload。
...
- `R454-C3-05`：checkout transaction/result 等 runtime identifier 必须使用共享语法
  ...
  及其它越界字符。
- `R454-C3-06`：public invocation error 只使用 dispatcher 共享字段 `code`、`field_path` 与
  `remediation`，三者语义必须精确；不得新增 message/details 别名或 package-private error shape。
- `R454-C3-07`：`guru-ensure-task-checkout` 必须作为完整 canonical package 存在，registry 只登记
  `state=planned`，canonical extension manifest 只加入 `planned_skill_ids`。Planned package 必须参与
  canonical ownership/source validation，但不得被 active selector 选中。
...
- `R454-C3-09`：C3 acceptance 只绑定 shared schema、lifecycle runtime、canonical package、planned ownership、
