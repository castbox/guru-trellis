...
本 reviewed promotion 按 r24 修订为 `dedicated_refactor_slice`，保持 23 Skills / 97 exits / 78 commands、
四阶段 workflow 顺序与旧 lifecycle 行为，直接删除 ledger aggregate 并同步 current consumer input。稳定设计已合并到
...
- `D247-04`：默认分支 PR 由 Publication 在 body 编码 closing keyword；非默认分支 PR 只引用，后续
  目标默认分支 Publication fresh 判断。Finalizer 保留 preparation、push、PR create/update、official
  archive、Ready、handoff 与 recovery，并向 Merge 投影 archive locator 和 exact reviewed body UTF-8
  bytes 的 SHA-256；正常、existing-PR 与 terminal recovery 使用同一 materializer。
- `D247-05`：Merge 独立重读 live PR/GitHub facts并完成 readiness semantic review、confirmation、
  ...
  决定、不调用 Issue close API；四个 declared exits、current terminal 和
  `phase2_reentry_required -> guru-restore-archived-task` 保持旧语义；Finish/Restore/Cleanup不消费 Issue aggregate。
...
- `D247-08`：legacy ledger path 不属于新 managed inventory；preset/update不主动触碰，current runtime
  不打开。旧 ledger-bound task不迁移、不转换；既有 ledger-free Restore/recovery 保留。canonical 与
  installed manifest 声明同一 `guru-ledger-free-runtime@1.0.0` projection identity；历史 archive/ADR/
  superseded/released RDT保持不变，旧 lifecycle 局限明确披露。
