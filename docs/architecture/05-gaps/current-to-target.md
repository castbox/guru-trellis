...
| `ARCH-GAP-008` | task-local Issue scope ledger 曾聚合 reference、closure intent、关闭执行与结果验证，并迫使多个 lifecycle owner共享 Issue classification arrays | `.50` 删除 ledger authority，同时保留 Publication、Finalizer archive/Ready、GitHub closing action、Merge四 exits/live verification与Restore旧语义 | #247 r24 `dedicated_refactor_slice` + accepted `ADR-009`；ledger debt `closed`，提前归档与多 PR 接续仍属后续工作 |
