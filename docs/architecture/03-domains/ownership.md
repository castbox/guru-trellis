...
| `ARCH-DOM-015` | Issue reference and closure ownership | current user/live external authority 与 lifecycle owner持有 requirement/source reference；Publication 独占 Issue reference/closure intent；Finalizer 只执行 reviewed payload；GitHub 执行默认分支 closing keyword；Merge 只验证 live result；无 task-local classification aggregate 或第二 closure owner |
