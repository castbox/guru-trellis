...
| `ADR-011` | draft | exact active task 的 detailed continuation 由当前 workflow 唯一区块拥有；adjacent DTO 直达 consumer，output loss 回原 producer，semantic result fresh 重跑；尚未 accepted |
...
`ADR-011` 草案见 [`011-active-task-continuation-authority.md`](./011-active-task-continuation-authority.md)；
在 independent committed review 与 serialized promotion 前不得作为 current decision 引用。
