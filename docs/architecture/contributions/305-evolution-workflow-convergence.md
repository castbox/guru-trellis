...
4. target-native delivery in the fixed order provenance preparation -> publication-head push -> Draft PR -> Finish
   summary/archive/archive-push/Ready -> archive-bound Merge -> Closure -> delivery terminal -> Cleanup, with exact
   owner-local failure re-entry and no confirmation reuse;
...
13. one `.trellis/spec` projection single writer replacing raw `trellis-update-spec`:
    ...
    work not already current at live target identities; `authority_only` also represents the normal zero-promotion
    repair when shared authority is already current but locator/usage/freshness projection is missing or stale.
    `guru-bootstrap-repository-ssot:projection_refresh` runs for the latter two only after all selected
    RDT/Architecture promotion, if any, is current, requires a same-range contribution ref that is still missing from
    current projection only for `with_code_spec`, writes only a missing
...
14. three independent publication actions: immutable-source/target provenance plus metadata-tail preparation, branch
    push and Draft PR creation have separate prepare/wait/confirmation-reentry/refusal/provider-recovery identities.
    Their refusal exits are respectively `publication_preparation_not_executed`, `branch_push_not_executed` and
    `pr_creation_not_executed`, each with one action-local minimal output and the distinct consumers
    `task-publication-preparation-not-executed`, `task-branch-push-not-executed` and
    `task-pr-creation-not-executed`.
    Every entry first rereads live state and exact current tail/remote head/Draft-or-READY base-head PR advances without
    mutation or confirmation. Draft PR always enters Finish; Finish alone produces the official archive, archive push,
    Ready state and archive-bound `ready_for_merge_ref`; only Merge consumes that ref.
...
| `.trellis/spec` projection | `guru-bootstrap-repository-ssot:projection_refresh` after outstanding reviewed promotion, authority locator/usage/freshness repair or code-spec work; zero-promotion `authority_only` is valid, already-current target returns without write, fresh committed review converges to double-none, and no raw Skill/preset/worker writer exists |
...
| post-review authority promotion | Branch Review supplies independent closed promotion/projection discriminators; the deterministic router invokes only not-yet-current authority promotion owners serially, supports zero-promotion `authority_only` missing-spec repair, invokes Bootstrap-only projection refresh, then forces fresh Check/Commit/Review whose already-current result is exact double-none |
...
- Planning project check: the pre-`REQ-REV-133` `DES-REV-001..014` result is stale. Current Phase 1 Requirements
  ...
  `DES-REV-001..043` with open `P1=0`, blocking `P2=0`, `P3=0` and high-risk question `=0`, and deterministic closure
