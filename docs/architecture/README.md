...
版本历史：`current-main-0.6.5-guru.35` 是唯一 active baseline；`v0.6.5-guru.9` 仅以 `EVD-002` 保留 released identity，其完整 Architecture 未从 tag 恢复，保持 `unverified`。未来 Trellis `0.6.15` 只在 TARGET/GAP 中出现。
