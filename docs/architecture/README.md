...
版本：`current-main-0.6.5-guru.37`；状态：`active`；source baseline：`5c059f4943edad7dfe25182a78af94759d41f9a1` + #260 compatibility task delta（精确 revision 为当前 Git HEAD）。
...
版本历史：`current-main-0.6.5-guru.37` 是唯一 active baseline；`current-main-0.6.5-guru.36` 与 `.35` 是 superseded current identities。`v0.6.5-guru.10` 只以 release evidence 保留 immutable released identity，不替代 current Architecture；`.37` stable tag/Release 仍只在 TARGET/GAP 中出现。
