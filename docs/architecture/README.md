...
版本：`current-main-0.6.17-guru.59`；状态：`active`；predecessor：`current-main-0.6.17-guru.58`；source baseline：reviewed [#454 contribution](./contributions/454-task-lifecycle-state-model.md) + inherited immutable `.58` authority；#305 的 `EVO-001..007` 仍是独立 target authority。精确 revision 由包含本 authority 的 Git commit/tree identity 绑定，正文不自引用可变 HEAD。
...
版本历史：`current-main-0.6.17-guru.59` 是唯一 active Architecture baseline；`.58` 及更早 identities 保持 immutable superseded history。`.59` 完整继承 `.58`，只吸收 #454 已审查的 C2 shared lifecycle kernel 与 D0 stage-evidence correction，决策与证据见 `ARCH-CUR-036`、`ARCH-DOM-021`、`ARCH-INT-024`、`ARCH-GAP-011`、`ADR-015`、`EVD-034`。active registry 保持 32 Skills / 142 package exits / 102 commands；production business workflow 仍为 22 mandatory invokes / 98 exits。

Repository `v0.6.17-guru.1`、extension `0.6.17-guru.42`、CLI/core `0.6.17` 与固定 Fork source `castbox/Trellis@eb370008c7689d4e272ae626bd002190ecbb3296` 仍保持独立版本轴。`.59` promotion 不是 production graph activation 或软件发布；C3-C7、D443、D436 与 E434 仍未完成，#434 继续独占 Delivery/lifecycle graph 激活与旧 edge retirement。promotion-created diff 必须重新通过 Phase 2、task commit 与独立完整 Branch Review，才可进入 Publication；完整 Release matrix 仍为 `unverified`。
