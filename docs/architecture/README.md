...
版本：`current-main-0.6.17-guru.55`；状态：`active`；predecessor：`current-main-0.6.17-guru.54`；source baseline：reviewed [#435 contribution](./contributions/435-active-task-delivery-loop.md) + inherited immutable `.54` authority；#305 的 `EVO-001..007` 仍是独立 target authority。精确 revision 由包含本 authority 的 Git commit/tree identity 绑定，正文不自引用可变 HEAD。
...
版本历史：`current-main-0.6.17-guru.55` 是唯一 active Architecture baseline；`.54` 及更早 identities 保持 immutable superseded history。`.55` 完整继承 `.54` 并吸收 #435 Active Task Delivery additive capability，决策见 [ADR-012](./adr/012-active-task-delivery-loop.md)，证据见 `EVD-030`。active registry 为 26 Skills / 114 package exits / 96 commands；production business workflow 仍为 22 mandatory invokes / 98 exits。

Repository `v0.6.17-guru.1`、extension `0.6.17-guru.42`、CLI/core `0.6.17` 与固定 Fork source 均继承 `.54`。本提升不是 production graph activation 或软件发布；#434 独占 Delivery graph 激活与旧 edge retirement，#436 独占 Completion、Closure、Finish、Cleanup 与 Reactivate。promotion-created diff 必须重新通过 Phase 2、task commit 与独立完整 Branch Review，才可进入 Publication；完整 Release matrix 仍为 `unverified`。
