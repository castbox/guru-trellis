...
版本：`current-main-0.6.17-guru.64`；状态：`active`；predecessor：`current-main-0.6.17-guru.63`；source baseline：reviewed [#454 C6/C7 contribution](./contributions/454-task-lifecycle-state-model-c6-c7.md) + inherited immutable `.63` authority；#305 的 `EVO-001..007` 仍是独立 target authority。精确 revision 由包含本 authority 的 Git commit/tree identity 绑定，正文不自引用可变 HEAD。
...
版本历史：`current-main-0.6.17-guru.64` 是唯一 active Architecture baseline；`.63` 及更早 identities 保持 immutable superseded history。`.64` 继承 C2/D0/C3/C4/C5，并吸收 #454 已审查的 C6 task creation composition、activation inputs 与 C7 subtraction/validation；见 `ARCH-CUR-041`、`ARCH-DOM-026`、`ARCH-INT-029`、`ARCH-GAP-011`、`EVD-039`。active registry 保持 32 Skills / 142 package exits / 102 commands，六个 planned IDs 不激活；production business workflow 仍为 22 mandatory invokes / 98 exits。
...
Repository `v0.6.17-guru.1`、extension `0.6.17-guru.42`、CLI/core `0.6.17` 与固定 Fork source `castbox/Trellis@eb370008c7689d4e272ae626bd002190ecbb3296` 仍保持独立版本轴。`.64` promotion 不是 production graph activation 或软件发布；D443、D436、E434 与 #434 activation 仍未完成。RDT 由独立 owner 从 `.63` 串行提升至 `.64`。promotion-created diff 必须重新通过 Phase 2、task commit 与独立完整 Branch Review；focused lifecycle `125/125` 只证明提升前 C6/C7 candidate，installed/platform 与完整 Release matrix 均未被声明为通过。
