...
版本：`current-main-0.6.17-guru.63`；状态：`active`；predecessor：`current-main-0.6.17-guru.62`；source baseline：reviewed [#454 C5 session/resource control contribution](./contributions/454-task-lifecycle-state-model-c5.md) + inherited immutable `.62` authority；#305 的 `EVO-001..007` 仍是独立 target authority。精确 revision 由包含本 authority 的 Git commit/tree identity 绑定，正文不自引用可变 HEAD。
...
版本历史：`current-main-0.6.17-guru.63` 是唯一 active Architecture baseline；`.62` 及更早 identities 保持 immutable superseded history。`.63` 完整继承 C2/D0/C3/C4 和 C4 bounded Finalizer provenance recovery guard，并吸收 #454 已审查的 C5 path-free official session adapter、resource ownership ledger、Finish seal input 与 Cleanup resolution substrate，决策与证据见 `ARCH-CUR-040`、`ARCH-DOM-025`、`ARCH-INT-028`、`ARCH-GAP-011`、`ADR-015`、`EVD-038`。active registry 保持 32 Skills / 142 package exits / 102 commands，planned IDs 不激活；production business workflow 仍为 22 mandatory invokes / 98 exits。
...
Repository `v0.6.17-guru.1`、extension `0.6.17-guru.42`、CLI/core `0.6.17` 与固定 Fork source `castbox/Trellis@eb370008c7689d4e272ae626bd002190ecbb3296` 仍保持独立版本轴。`.63` promotion 不是 production graph activation 或软件发布；C6-C7、D443、D436、E434 与 #434 activation 仍未完成。RDT 已由独立 owner 从 `.62` 串行提升至 `.63`。promotion-created diff 必须重新通过 Phase 2、task commit 与独立完整 Branch Review；focused lifecycle `113/113` 只证明提升前 C5 candidate，installed/platform 与完整 Release matrix 均未被声明为通过。
