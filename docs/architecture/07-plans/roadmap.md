...
| current Issue closure ownership | #247 | 移除 task-local Issue scope ledger，并由 Publication、Finalizer、GitHub 与 Merge分别承接 reference/intent、payload execution、closing action 与 live result | reviewed contribution 已由 expected `.49` serialized promotion 为唯一 active `.50`；accepted `ADR-009` 与 `ARCH-GAP-008` 记录长期 owner/exit。promotion-created diff 仍须 fresh Phase 2/commit/full Branch Review，不证明 push、PR、merge、tag、Release 或 Issue closure |
