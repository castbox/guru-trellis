...
默认安装命令 pin 到 repo release tag `#v0.6.5`，用于可复现的稳定安装。维护者刻意采样
最新 `main` / canary 时，可以去掉 `#ref` 或设置其它 branch/tag ref，但最终报告必须说明
安装来源是 mutable ref 还是 immutable release tag。
...
`gh:castbox/guru-trellis/trellis#v0.6.5`；需要验证其它 branch/tag 时，设置
...
```text
在当前 Repo 中安装 latest Trellis CLI，并安装 Guru Team extension stable v0.6.5。
...
- 如果当前 Repo 还没有 .trellis/，直接用 Guru Team workflow 的稳定非交互命令初始化：`trellis init -y -u <your-name> --codex --cursor --workflow guru-team --workflow-source gh:castbox/guru-trellis/trellis#v0.6.5`。
- 如果我明确要求交互式选择 spec template，才可以去掉 `-y`；默认安装和自动验收必须使用 `-y` 或显式 `--template <name>`。
- 获取与 workflow source 相同 release tag 的公开 preset 仓库内容，例如 `git clone --depth 1 --branch v0.6.5 https://github.com/castbox/guru-trellis.git <guru-trellis>`。只有明确要跟随 latest/canary 时，才复用 `main` 或不带 `#ref` 的远端 source，并在最终报告中说明来源是 mutable ref。
...
完成后告诉我：
- 安装到的 Trellis 版本；
- 安装到的 Guru Team extension 版本、source ref / commit、source tree state，以及是否来自 mutable ref；
- 使用的用户名；
- 实际启用并保留了哪些平台入口；
...
- 当前 Repo 已有 .trellis/ 时，先用 Guru Team stable marketplace 生成 workflow 预览：`trellis workflow --marketplace gh:castbox/guru-trellis/trellis#v0.6.5 --template guru-team --create-new`，再对比现有 `.trellis/workflow.md` 和 `.trellis/workflow.md.new`；确认风险后运行 `trellis workflow --marketplace gh:castbox/guru-trellis/trellis#v0.6.5 --template guru-team` 切换 active workflow。
- 获取与 workflow source 相同 release tag 的公开 preset 仓库内容，例如 `git clone --depth 1 --branch v0.6.5 https://github.com/castbox/guru-trellis.git <guru-trellis>`。只有明确要跟随 latest/canary 时，才复用 `main` 或不带 `#ref` 的远端 source，并在最终报告中说明来源是 mutable ref。
...
完成后告诉我：
- 升级前后的 Trellis 版本；
- 升级前后的 Guru Team extension 版本、source ref / commit、source tree state，以及是否来自 mutable ref；
...
- `trellis/guru-team-extension.json`：Guru Team extension 的 canonical SemVer 版本；
...
overlay entrypoints，以及 `.new` / `.bak` conflict handling。版本号按 SemVer 维护：
...
本仓库的 release tag 使用 repo 级 `vX.Y.Z`，例如 `v0.6.5`。tag 版本必须与
`trellis/guru-team-extension.json.version` 一致；稳定安装文档使用
`gh:castbox/guru-trellis/trellis#vX.Y.Z`。不带 `#ref` 的
...
annotated tag `vX.Y.Z`，验证 `trellis init` / `trellis workflow` 的 tag-pinned 安装后，
再退休旧 tag 名称。
