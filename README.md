...
`#v0.6.5-guru.1`，用于可复现的稳定安装。维护者刻意采样最新 `main` / canary
...
`gh:castbox/guru-trellis/trellis#v0.6.5-guru.1`；需要验证其它 branch/tag 时，设置
...
```text
在当前 Repo 中安装官方 Trellis CLI v0.6.5，并安装 Guru Team extension stable v0.6.5-guru.1。

要求：
- 先实时确认 npm 上 @mindfoldhq/trellis 的 latest 版本，不要凭记忆判断版本；如果 latest 已不是 0.6.5，本次仍按 Guru Team stable v0.6.5-guru.1 的目标基线安装 @mindfoldhq/trellis@0.6.5，除非我明确要求升级官方 Trellis 基线。
...
- 如果当前 Repo 还没有 .trellis/，直接用 Guru Team workflow 的稳定非交互命令初始化：`trellis init -y -u <your-name> --codex --cursor --workflow guru-team --workflow-source gh:castbox/guru-trellis/trellis#v0.6.5-guru.1`。
- 如果我明确要求交互式选择 spec template，才可以去掉 `-y`；默认安装和自动验收必须使用 `-y` 或显式 `--template <name>`。
- 获取与 workflow source 相同 release tag 的公开 preset 仓库内容，例如 `git clone --depth 1 --branch v0.6.5-guru.1 https://github.com/castbox/guru-trellis.git <guru-trellis>`。只有明确要跟随 latest/canary 时，才复用 `main` 或不带 `#ref` 的远端 source，并在最终报告中说明来源是 mutable ref。
...
要求：
- 先实时确认 npm 上 @mindfoldhq/trellis 的 latest 版本，并检查当前 trellis --version、which -a trellis、npm list -g @mindfoldhq/trellis --depth=0；如果 latest 已不是 0.6.5，本次仍按 Guru Team stable v0.6.5-guru.1 的目标基线安装 @mindfoldhq/trellis@0.6.5，除非我明确要求升级官方 Trellis 基线。
...
- 当前 Repo 已有 .trellis/ 时，先用 Guru Team stable marketplace 生成 workflow 预览：`trellis workflow --marketplace gh:castbox/guru-trellis/trellis#v0.6.5-guru.1 --template guru-team --create-new`，再对比现有 `.trellis/workflow.md` 和 `.trellis/workflow.md.new`；确认风险后运行 `trellis workflow --marketplace gh:castbox/guru-trellis/trellis#v0.6.5-guru.1 --template guru-team` 切换 active workflow。
- 获取与 workflow source 相同 release tag 的公开 preset 仓库内容，例如 `git clone --depth 1 --branch v0.6.5-guru.1 https://github.com/castbox/guru-trellis.git <guru-trellis>`。只有明确要跟随 latest/canary 时，才复用 `main` 或不带 `#ref` 的远端 source，并在最终报告中说明来源是 mutable ref。
...
例如 `v0.6.5-guru.1` 表示“针对官方 Trellis `0.6.5` 的 Guru Team 第 1 个稳定修订”。
...
本仓库的 release tag 使用 repo 级 tag，例如 `v0.6.5-guru.1`。tag 名称必须与
`trellis/guru-team-extension.json.version` 对应；该 manifest 同时用 `target_trellis_cli`
记录目标官方 Trellis CLI 版本。稳定安装文档使用
`gh:castbox/guru-trellis/trellis#v0.6.5-guru.1`。不带 `#ref` 的
...
annotated tag `v0.6.5-guru.1` 这类 release tag，验证 `trellis init` / `trellis workflow`
...
校验规划审查证据：主会话必须先展示 `prd.md`、`design.md`、`implement.md` 三个
task-local 链接，并在用户看到后获得 `explicit-post-planning-review` 确认；
Phase 0 handoff 确认或旧 `source=workflow` 不能通过 gate。`task.py start`
只写状态，不代表规划已审查。
...
`review.md`。main session 只协调、等待/恢复/替换、记录 evidence、commit 和运行
...
最终完成事件，不代表 sub-agent 卡死、失败或应该收口。主会话必须区分总运行时间长和
近期无可观察进展；只要 sub-agent 仍在输出、合理修改工作区、运行验证或产生 channel
event，就继续等待。stale 判断默认至少基于 5 分钟无进展，而不是固定总运行时长。
等待 timeout、progress observed、stale assessed、continue waiting、resume same agent、
replacement started、terminated unfinished、completed、failed 等状态处理写入
`agent-assignment.json.status_events[]`。未完成即被中断或终止的 sub-agent 必须恢复同一
`agent_id` 或由 replacement agent 继承前任输出、当前 diff、task artifacts、剩余工作和
gate 阻塞点，并直到 `completed` 或明确 `failed`；未闭环的部分输出不能作为实现完成、
Phase 2 check pass 或 Branch Review Gate pass 证据。
...
记录来源、review report digest、assignment digest、roles、review round 和 status event 摘要，并校验
