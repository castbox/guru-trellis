...
overlay 不读取或枚举 journal。preset 固定 materialize `session_auto_commit: false` 并忽略该目录。PR 创建后，
...
task-local `pr-readiness.json.publish_inputs` 在首次 create 前提交，固定
`repo/base/head/reviewed-head/title/body-sha256/draft/reviewed-source` 与 canonical digest；
recovery 在查询或创建 PR 前重新校验 committed blob/history、snapshot/body digest、
repo/base/head、review gate、当前与远端 HEAD，且不接受 title/body/draft/base override，
