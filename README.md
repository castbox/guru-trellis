...
`guru-sync-base` 与 `guru-create-task-commit`。Active package 的
...
解析：显式 `--base`、scalar `base_branch`（或单值兼容 candidates）、remote default、
本地或 remote-tracking refs 中恰好一个 fallback candidate；current branch 永远不是隐式
fallback。AI 先审查 invocation intent、resolution source 与 selected base，executor 再以
reviewed resolution digest 绑定重解析和 fetch/`ff-only`。成功证据必须证明 decision
checkout HEAD、local base HEAD 与 remote-tracking HEAD 三方相等且 checkout clean，随后
由 validator 校验 schema、digest 和 live Git facts。Stable exits 只有 `synced`、
...
manifest 已递增到下一待发布版本 `0.6.5-guru.7`；在对应 merge commit 创建并验证
...
```text
guru-sync-base
  synced -> guru-discover-change-context -> check-env / prepare-task
...
`prepare-task` 必须消费前序 AI-reviewed resolution file 与 expected digest，并在
...
之前。`--base-branch` 只做相等断言，不得改写 config/remote-default/fallback provenance。每次成功都必须
