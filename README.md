...
`guru-team-skill-interface-1.2 + legacy`。当前
...
`guru-approve-task-plan`、`guru-check-task`、`guru-create-task-commit` 仍由 #146
保持 legacy。
...
`trellis/skills/guru-team/migrations/stage0-minimal-handoff.json`；source/installed
validator 会把它与 registry、Interface、workflow markers、extension inventories、六份
corpus及 selected-platform copies 双向比较。每个 migrated package 的
...
runtime raw read 必须真实失败。六个 Stage 0 production packages 维护唯一 canonical
corpus 并覆盖全部 24 exits/input profiles；#146 继续负责 planning/check/commit 三包。
