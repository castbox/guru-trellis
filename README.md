...
完成的 AI/human review 结论，不替代真正的文档 + 代码 review。
`publish-pr.sh` 默认拒绝普通直接调用，避免 `trellis-continue` 或未完成
`trellis-finish-work` 时提前 push 分支并创建 PR；日常发布必须由
`trellis-finish-work` 在 archive task、记录 journal 并提交允许的 Trellis metadata
之后内部触发。只有 finish-work 已完成但 publish 需要重试时，才使用显式
recovery/debug flag。
